...
import { listingExpectedPublicationLocale, type ListingPublicationIntent, type VerifiedListingRemoteState } from "./listing-publication-state";
import { verifyListingUpdateReadback } from "./listing-update";
import { lazadaPrimaryCategory, lazadaUpdateCommerceReadbackVerified } from "./lazada-listing-update";

import { lazadaRequest, textValue, type RemoteResponse, type SecretPayload } from "./protocols";
...
function lazadaImmutableRemoteAssetsVerified(sourceRemoteData: UnknownRecord, currentRemoteData: UnknownRecord) {
...
function lazadaVisibility(statuses: string[], qcStatuses: string[]): VerifiedListingRemoteState["visibility"] | undefined {
 ...
 */
export function lazadaListingArgumentsForPublicationIntent(argumentsValue: UnknownRecord, intent: ListingPublicationIntent) {
 ...
 */
export function lazadaListingArgumentsForRemoteItem(argumentsValue: UnknownRecord, remoteIdValue: string) {
...
export function normalizeLazadaListingPublicationReadback(input: {
  ...
  const localeVerified = Boolean(market
  ...
  const contentVerification = input.operation === "listing.stop"
    ...
    : input.contentVerificationMode === "immutable_source_readback"
      ? lazadaImmutableRemoteAssetsVerified(recordValue(input.immutableSourceRemoteData), input.remoteData)
        ? { ok: true, mismatches: [] as string[] }
...
export async function readLazadaListingPublicationState(input: {
  ...
  contentVerificationMode?: LazadaContentVerificationMode;
  immutableSourceRemoteData?: UnknownRecord;
}): Promise<LazadaPublicationReadbackVerification & {
