...
export async function executeElevenstInquiry(
  ...
  if (input.operation === "inquiries.list") {
    ...
    const listStep = inquiryStep("inquiries", remote);
    const remoteData = remote.data as Record<string, unknown>;
    const observedRows = remoteData.sellerpilotElevenstProductQnaObservedRows;
    const rows = remoteData.productQnas;
    const parseIncomplete = remoteData.sellerpilotElevenstProductQnaParseIncomplete === true;
    const parserReady = remoteData.sellerpilotElevenstProductQnaParseContract
        === "sellerpilot-elevenst-product-qna-parser/1"
      && remoteData.sellerpilotElevenstProductQnaDocumentReady === true
      && Array.isArray(rows)
      && typeof observedRows === "number"
      && Number.isSafeInteger(observedRows)
      && observedRows === rows.length
    ...
    listStep.data = {
      ...
      sellerpilotProductQnaParserReady: parserReady,
    };
    return { steps: [listStep] };
  }
