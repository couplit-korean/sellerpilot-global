import { qoo10InquiryReplyParams } from "./contracts";
...
export function resolveQoo10ReplyLineage(input: {
  ...
  const scoped = /^qoo10:conversation:([a-f0-9]{64})$/u.exec(input.externalTicketId);
  const providerExternalTicketId = scoped
    ? text(input.providerContext.providerExternalTicketId)
    : input.externalTicketId;
  const thread = /^qoo10:v2:(MSG|HELP|ITEM):(\d{1,40})$/u.exec(providerExternalTicketId);
  ...
  if (!thread || input.providerContext.ticketIdentityVersion !== "qoo10-thread-v2"
      ...
      || scoped && (
        input.providerContext.conversationIdentityDigest !== scoped[1]
        || !/^[a-f0-9]{64}$/u.test(text(input.providerContext.accountIdentityDigest))
        || !/^[a-f0-9]{64}$/u.test(text(input.providerContext.messageIdentityDigest))
      )) {
    throw new Error("QOO10_REPLY_TARGET_INVALID");
  }
  return {
    ...
    ...(scoped ? { messageIdentityDigest: text(input.providerContext.messageIdentityDigest) } : {}),
    ticketIdentity: scoped ? "account-scoped-v2" as const : "thread-v2" as const,
  };
}
...
export function prepareQoo10Reply(input: Qoo10ReplyPreparationInput) {
  ...
  if (lineage.ticketIdentity === "account-scoped-v2"
      && input.selectedInboundKey !== `qoo10:inbound:${lineage.messageIdentityDigest}`) {
    throw new Error("QOO10_REPLY_TARGET_INVALID");
  }
