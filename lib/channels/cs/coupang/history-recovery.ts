import { inquiryHistorySyncRequests } from "../../sync-arguments";
...
export function coupangHistoryRecoveryCheckpoint(run: CoupangHistoryRunState) {
  ...
  const expectedInitialJobs = Math.ceil(run.historyDays / 7) * 5;
  const legacyExpectedInitialJobs = Math.ceil(run.historyDays / 7) * 8;
  if (![expectedInitialJobs, legacyExpectedInitialJobs].includes(run.expectedInitialJobs)
      ...
      || run.totalJobs < run.expectedInitialJobs) {
