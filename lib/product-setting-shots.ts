...
const settingShotRetryProfiles = [
  {
    ...
    forbiddenTopology: "the base camera axis; blank architecture extending materially outside the reserved quiet rectangle with only one ambiguous functional-room sliver; a one-sided daylight strip without a second fixed room cue; centred axial symmetry; any major doorway, cabinet or fixture edge cutting through the reserved quiet rectangle; or a crop, mirror, recolour or small lateral shift of any rejected plate",
  },
  {
...
export function buildSettingShotRetryVariant(
  ...
  return {
    ...
    surface: contactMode === "surface-supported"
      ? `an integrated ${retrySurface}; this is the continuous physical support plane whose horizontal boundary crosses the complete reserved rectangle with its gradient-ridge centre within two 256-by-256 sampled pixels of the nominal reserved-zone bottom, and whose assigned material continues below that boundary; the previous candidate's material and grain direction are fully blacklisted`
...
export function buildSettingShotRetryGuidance(
  ...
  const reservedZoneContactGate = contactMode === "surface-supported"
    ? `Render exactly one continuous horizontal support boundary across the complete reserved x-span. ${nominalSupportContactInstruction}. Within that x-span, the assigned support material must continue from the boundary through the frame bottom, leaving no wall, vertical panel, open-air or diagonal-boundary pixels in the below-boundary region; no other major perspective edge may enter the reserved rectangle.`
