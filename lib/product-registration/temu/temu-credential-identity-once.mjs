...
async function readTemuIdentity({ appKey, appSecret, accessToken }) {
  ...
  if (parsed.success !== true || !parsed.result) {
    ...
    // A whitelist rejection is an egress problem, not a credential problem, so
    // the operator sees a stable code instead of a generic read failure.
    if (String(code) === "5000003"
      || /not[ _-]?in[ _-]?ip[ _-]?white[ _-]?list|ip[ _-]?white[ _-]?list/iu.test(message)) {
      throw new Error("TEMU_EGRESS_IP_NOT_ALLOWLISTED");
    }
