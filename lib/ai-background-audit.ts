...
export function buildBackgroundSemanticAuditPrompt(input: BackgroundSemanticAuditPromptInput) {
  ...
  const reservedRight = Number((reservedZone.left + reservedZone.width).toFixed(4));
  const reservedBottom = Number((reservedZone.top + reservedZone.height).toFixed(4));
  ...
  if (input.reviewProfile === "catalog-scenes") {
    return [
      ...
      `Empty product placement rectangle: ${JSON.stringify(reservedZone)}. ${input.contactMode === "surface-supported"
        ? `reservedZoneClear requires physically unobstructed product placement and a visible horizontal shelf, counter or tabletop beneath its base at y=${contactLine}, extending on both sides and below the base. Inspect the base at the rectangle's horizontal center, not a surface somewhere else. The base can sit WITHIN the support plane; its back edge or wall seam does not have to coincide with the product base. A table's vertical front fascia, vertical wall, empty air, room floor, ambiguous plane, product-shaped shadow or obstructing object at the base fails. A quiet surface seam elsewhere is allowed.`
        : contactAudit}`,
      "Judge occlusion by physical scene depth, not by 2D overlap with the placement rectangle. A fixed window frame or balcony railing clearly BEHIND the intended product placement is background architecture: the later verified source-pixel foreground will cover the pixels it actually occupies. Those rear architectural lines alone do not fail reservedZoneClear. Do not assume the entire rectangle will become opaque: unused margins and silhouette gaps remain visible, so a dominant object cluster or clutter that would compete with the product still fails. A foreground object occupying the product footprint, a rail or frame physically IN FRONT OF the intended product, an interruption of the required support or backing plane, or uncertain foreground/background depth must fail. This exception never replaces the selected contact mode's support or backing-plane requirements and never permits merchandise, containers, labels, barcodes or people. For a rejection, identify the physical foreground obstruction, remaining clutter or missing support in findings; mere projected line overlap is not an obstruction.",
  ...
  return [
    ...
    `Outer-band audit map outside the declared zone: left x=0..${reservedZone.left}, right x=${reservedRight}..1, top y=0..${reservedZone.top}, bottom y=${reservedBottom}..1. The top and bottom bands exclude the zone interior even where their horizontal ranges overlap it.`,
    "MANDATED QUIET-ZONE EXCLUSION: when reservedZoneClear is true, ignore blankness, low detail and the absence of perspective junctions caused solely by pixels inside the exact declared zone when deciding assignedLocationSatisfied, assignedCameraSatisfied, spatialDepthPresent and the series*Distinct fields. That intentional quiet rectangle will be occupied by a verified source-pixel product after this audit, so its large screen share is not by itself a dominant-blank-wall, shallow-layout or narrow-room-reveal failure.",
    "OUTER-BAND EVIDENCE GATE: the candidate still fails assignedEnvironmentPresent, assignedLocationSatisfied, assignedCameraSatisfied or spatialDepthPresent when the pixels outside the exact declared zone do not visibly prove the trusted room. Require room-recognition architecture across at least two non-collinear uncovered bands: one left/right side band and one top/bottom band. The outer evidence must include every hard room-recognition structure, the required slot-specific cue, and readable convergence or occlusion connecting foreground, middle and rear planes. If a blank wall continues materially beyond the zone into those outer bands, or all functional evidence is squeezed into one ambiguous side sliver, fail the relevant assigned and series dimensions.",
    ...
    input.comparisonAssetIds?.length
      ? `Image 1 is the candidate. The later trusted comparison plates, in order, are: ${input.comparisonAssetIds.join(", ")}. Safety flags, reserved-zone checks and assigned-dimension checks describe Image 1 only. Comparison plates may contain a neutral rectangular mask over an earlier product zone; ignore that mask and compare the remaining environment pixels. For same-slot rejected plates, ignore the same exact declared zone in every image even when no neutral mask was rendered. Compare outer-band pixels, not prompt wording or intentional quiet-zone occupancy. Set every series*Distinct field true only when the candidate is unmistakably different from every cross-slot comparison plate in that dimension outside the ignored zones. conflictingAssetIds must contain only comparison IDs that fail at least one required distinction; it must be empty when every required distinction is true, and every false series*Distinct field must have at least one relevant conflict ID. Merely changing one fixture, crop or product placement while retaining the same outer-band room, surface, light mood or depth counts as not distinct.${previousSameSlotIds.length ? ` Same-slot regeneration comparison ${previousSameSlotIds.join(", ")} intentionally shares the trusted location, moment-light, surface, palette and camera family; exclude it from those cross-slot dimension booleans. For that previous same-slot plate, require a materially different outer-band architectural layout, perspective composition, spatial arrangement and fixed-cue placement; mark seriesVisuallyDistinct and seriesCueDistinct false and list its ID only when that visual regeneration is still a near-repeat outside the ignored zone.` : ""}`
