...
export function buildBackgroundSemanticAuditPrompt(input: BackgroundSemanticAuditPromptInput) {
  ...
  return [
    ...
    `${contactAudit} A pre-rendered product-shaped shadow, reflection, silhouette, footprint or imprint also fails reservedZoneClear. A visible card, picture mat, poster, border, outline, lightbox, niche, pedestal block, display case or contrasting rectangle that traces or encloses the reserved zone also fails reservedZoneClear; the zone is coordinates only and must remain visually frameless.`,
