...
export async function executeCsOperation(
  ...
  const paginationDepth = finiteCount(input.arguments.sellerpilotPaginationDepth) ?? 0;
  const legacySmartstoreRollover = input.channel === "smartstore"
    && input.operation === "inquiries.list"
    && paginationDepth === MAX_PROVIDER_SYNC_CONTINUATIONS
    && finiteCount((input.arguments.query as Record<string, unknown> | undefined)?.page) === 51
    && input.arguments.sellerpilotPaginationEpoch === undefined
    && input.arguments.sellerpilotPaginationTrail === undefined;
  if (
    input.operation === "inquiries.list" &&
    paginationDepth >= MAX_PROVIDER_SYNC_CONTINUATIONS &&
    !legacySmartstoreRollover
  )
