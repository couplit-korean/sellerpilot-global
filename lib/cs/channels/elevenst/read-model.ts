...
export function buildElevenstReadonlyWebProjection(input: {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,119}$/u.test(input.provider.sellerId)
      || input.provider.sellerName.trim() !== input.provider.sellerName
      || input.provider.sellerName.length < 1
      || input.provider.sellerName.length > 160) {
