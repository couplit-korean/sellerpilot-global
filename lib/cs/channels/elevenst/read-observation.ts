...
import type { ElevenstCsAccountIdentity } from "./account-identity";
...
type BuildInput = {
  ...
  identity: ElevenstCsAccountIdentity;
};
...
export function buildElevenstCsReadObservation(input: BuildInput): {
  ...
  if (input.identity.contract !== "sellerpilot-elevenst-cs-account-identity/1"
      || input.identity.environment !== "production") {
    throw new Error("ELEVENST_READ_OBSERVATION_INVALID:identity");
  }
  ...
  if (requestedKind === "urgent_alimi") {
    ...
    parseIncomplete = data.sellerpilotElevenstProductQnaParseIncomplete === true;
    parserMarker = data.sellerpilotElevenstProductQnaParseContract
        === "sellerpilot-elevenst-product-qna-parser/1"
      ? "sellerpilot-elevenst-product-qna-parser/1"
      : null;
    ...
    if (observedRows === undefined) {
      providerRows = rows.length;
    } else if (typeof observedRows !== "number" || !Number.isSafeInteger(observedRows)
        || observedRows < 0) {
      throw new Error("ELEVENST_READ_OBSERVATION_INVALID:qnaObservedRows");
    } else {
      providerRows = observedRows;
    }
    if ((parseIncomplete && rows.length !== 0)
        || (!parseIncomplete && parserMarker !== null && providerRows !== rows.length)) {
      throw new Error("ELEVENST_READ_OBSERVATION_INVALID:qnaRowBinding");
    }
  ...
  const observation: ElevenstReadObservation = {
    ...
    sellerName: input.identity.sellerName,
