...
export const elevenstReadStateSchema = z.object({
  contractVersion: z.literal("sellerpilot-elevenst-authenticated-read-state/5"),
...
export const elevenstReadAccountSchema = z.object({
  credentialId: z.string().uuid(),
  sellerId: z.string().regex(/^[A-Za-z0-9][A-Za-z0-9._-]{0,119}$/u),
  sellerName: z.string().trim().min(1).max(160),
...
export const elevenstReadAccountsSchema = z.object({
  contractVersion: z.literal("sellerpilot-elevenst-authenticated-accounts/1"),
  accounts: z.array(elevenstReadAccountSchema).max(100),
}).strict();
...
export type ElevenstReadAccount = z.infer<typeof elevenstReadAccountSchema>;
