...
export const ebayCaseDisputeHistorySyncResponseSchema = z.object({
  ...
  if (value.availability === "readable") {
    if (value.httpStatus !== 200 || value.observedCount === null || value.insertedCount === null
        || value.insertedCount > value.observedCount) {
      context.addIssue({ code: "custom", message: "invalid readable sync receipt" });
    }
  } else if (value.total !== null || value.observedCount !== null || value.insertedCount !== null) {
    context.addIssue({ code: "custom", message: "unavailable sync cannot claim counts" });
  }
});
