-- Recovery preserves the actual Lazada IM reader wrappers as predecessors;
-- their owner checks and latest-message projection remain in the call chain.
-- Reviewed forward recovery. No jobs, approvals or provider actions are created.
begin;
set local lock_timeout='2s';set local statement_timeout='30s';
do $recovery_guard$ begin
if (select md5(p.prosrc) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_get_cs_workspace_snapshot' and pg_get_function_identity_arguments(p.oid)='') is distinct from '63f055ede50bc23243cedd43915921a8' then raise exception 'RECOVERY_PREIMAGE_DRIFT:sellerpilot_get_cs_workspace_snapshot';end if;
if (select md5(p.prosrc) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_get_ticket_reply_context_v2' and pg_get_function_identity_arguments(p.oid)='p_id uuid') is distinct from 'd1e26d75a608b9774ce06648df913b64' then raise exception 'RECOVERY_PREIMAGE_DRIFT:sellerpilot_get_ticket_reply_context_v2';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_read_cs_order_binding_health_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_read_cs_order_binding_health_v1';end if;
if (select md5(p.prosrc) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_service_ingest_orders' and pg_get_function_identity_arguments(p.oid)='p_credential_id uuid, p_channel text, p_orders jsonb') is distinct from '7657c4469226c8a0873628e9f029380d' then raise exception 'RECOVERY_PREIMAGE_DRIFT:sellerpilot_service_ingest_orders';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='coupang_cs_order_binding_candidate_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:coupang_cs_order_binding_candidate_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='coupang_cs_order_read_is_exact_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:coupang_cs_order_read_is_exact_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='coupang_order_revision_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:coupang_order_revision_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='cs_order_binding_is_exact') then raise exception 'RECOVERY_ALREADY_DEFINED:cs_order_binding_is_exact';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='reconcile_coupang_cs_order_bindings_from_credential_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:reconcile_coupang_cs_order_bindings_from_credential_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='reconcile_coupang_cs_order_bindings_from_lineage_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:reconcile_coupang_cs_order_bindings_from_lineage_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='reconcile_cs_order_binding_from_ticket') then raise exception 'RECOVERY_ALREADY_DEFINED:reconcile_cs_order_binding_from_ticket';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='reconcile_cs_order_bindings_from_credential') then raise exception 'RECOVERY_ALREADY_DEFINED:reconcile_cs_order_bindings_from_credential';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='reconcile_cs_order_bindings_from_order') then raise exception 'RECOVERY_ALREADY_DEFINED:reconcile_cs_order_bindings_from_order';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='reconcile_one_coupang_cs_order_binding_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:reconcile_one_coupang_cs_order_binding_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='reconcile_one_cs_order_binding') then raise exception 'RECOVERY_ALREADY_DEFINED:reconcile_one_cs_order_binding';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='record_coupang_order_lineage_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:record_coupang_order_lineage_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='reject_cs_lineage_commerce_mutation') then raise exception 'RECOVERY_ALREADY_DEFINED:reject_cs_lineage_commerce_mutation';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='validate_cs_order_binding') then raise exception 'RECOVERY_ALREADY_DEFINED:validate_cs_order_binding';end if;
end $recovery_guard$;
-- Reviewed source: 20260908001000_harden_cs_commerce_boundaries.sql
-- Source SHA256: 9dd479171e2a43c8ba2747a80149f4d494183d611253fd9493f44f658b1895dc
...
-- Reviewed source: 20260909123112_cs_coupang_order_revision_and_read_gate.sql
-- Source SHA256: 0ee4802e22131030977bec20c5958642bdefa926d4236f745b7dfb994aa79ee7
...
begin
  if to_regclass('sellerpilot_private.cs_reply_draft_jobs') is null
     ...
     or (select md5(prosrc) from pg_proc
        ...
        is distinct from '63f055ede50bc23243cedd43915921a8'
     or (select md5(prosrc) from pg_proc
          where oid='public.sellerpilot_get_ticket_reply_context_v2(uuid)'::regprocedure)
        is distinct from 'd1e26d75a608b9774ce06648df913b64'
     or (select md5(prosrc) from pg_proc
