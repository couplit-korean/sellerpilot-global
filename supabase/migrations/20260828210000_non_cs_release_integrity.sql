...
-- Product research and generated product assets are owned by the Vercel
-- server runtime and its Supabase recovery schedule. Keep the local AI worker
-- off those queues so it cannot race the server-owned model and cost boundary.
--
-- The legacy unscoped claimant is retained only for database compatibility;
-- deployed local workers enter through this non-product claimant. Its filter
-- names only the server-owned product kinds, so non-product behavior remains
-- unchanged and no customer-support contract is redefined here.
create or replace function public.sellerpilot_claim_local_ai_job(
  p_token_hash text,
  p_worker_version text default null
)
returns jsonb
...
begin
  ...
  update sellerpilot_private.ai_cli_jobs job
     set status = case when job.attempt_count >= 3 then 'failed' else 'queued' end,
         error_message = case
           when job.attempt_count >= 3 then 'Local AI worker lease expired three times.'
         ...
         updated_at = clock_timestamp()
   where job.kind not in ('product_studio', 'product_asset_regeneration', 'product_research')
     and job.status = 'running'
     and job.lease_expires_at < clock_timestamp();
  ...
  update sellerpilot_private.ai_cli_jobs job
   ...
   where job.id = v_job_id
     and job.status = 'queued';
  if not found then
    raise exception 'local AI job claim lost its row lock';
  end if;
  ...
  if not found then
    raise exception 'claimed local AI job ownership mismatch';
  end if;
  return v_result;
end;
$$;

revoke all on function public.sellerpilot_claim_local_ai_job(text, text)
  from public, anon, authenticated, service_role;
grant execute on function public.sellerpilot_claim_local_ai_job(text, text)
  to service_role;
comment on function public.sellerpilot_claim_local_ai_job(text, text) is
  'Claims local non-product AI jobs; product generation and research are server-owned.';
