-- Draft marketplace rows are preparation state, not an active remote write.
-- Keep the activity clock finite until a queue row or running attempt exists,
-- and keep product-research drafts out of the one-card-per-product history.
    ...
    with product_cards as (
      select
        ...
        case
          ...
          when coalesce(l.running_count, 0) = 0
            and coalesce(l.blocked_count, 0) = 0
            and coalesce(l.failed_count, 0) = 0
            and (j.status is null or j.status in ('succeeded', 'failed', 'cancelled')) then
            greatest(
              coalesce(j.completed_at, j.updated_at, p.updated_at),
              coalesce(l.completed_at, l.updated_at, p.updated_at)
            )
          else null
        end as completed_at,
      ...
      left join lateral (
        select
          ...
          count(*) filter (
            where pl.status in ('queued', 'draft') and a.status = 'running'
          ...
          jsonb_agg(jsonb_build_object(
            ...
            'market', pl.market,
            'status', case
              when pl.status = 'failed' and pl.failure_class = 'external_action' then 'blocked'
              when pl.status = 'queued' and a.id is null then 'draft'
              else pl.status
            end,
      ...
      select
        ...
        left(coalesce(nullif(j.request_payload->'manual_fields'->>'productName', ''), '상품 분석'), 160) as product_name,
