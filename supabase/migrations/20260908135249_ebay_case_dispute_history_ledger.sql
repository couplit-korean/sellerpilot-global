...
begin
  ...
  if p_resource_kind is null or p_resource_kind not in ('resolution_case','payment_dispute')
     ...
     or pg_catalog.jsonb_typeof(p_normalized_safe) is distinct from 'object'
     or pg_catalog.octet_length(p_normalized_safe::text) > 32768 then
  ...
  -- Safe fields must also be scalar; known keys cannot smuggle nested raw data.
  if exists (
    select 1 from pg_catalog.jsonb_each(p_normalized_safe) field
     where field.key <> v_amount_key and (
       pg_catalog.jsonb_typeof(field.value) not in ('string','number','null')
       or (pg_catalog.jsonb_typeof(field.value) = 'string' and pg_catalog.length(field.value #>> '{}') > 240)
       or (field.key in ('revision','evidenceRequestCount') and field.value <> 'null'::jsonb
         and (pg_catalog.jsonb_typeof(field.value) <> 'number' or field.value::text !~ '^[0-9]{1,10}$'))
       or (field.key not in ('revision','evidenceRequestCount') and pg_catalog.jsonb_typeof(field.value) = 'number')
     )
  ) then
    raise exception 'EBAY_CASE_DISPUTE_HISTORY_UNSAFE_PAYLOAD';
  end if;
