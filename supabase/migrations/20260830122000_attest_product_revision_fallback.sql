-- Authorize the deterministic Studio fallback for product revisions only when
-- the claimed job is backed by the exact pending private revision row. Request
-- JSON alone is not an authority because the generic admin job RPC can enqueue
-- ordinary product_studio payloads.
...
comment on function public.sellerpilot_claim_product_ai_job(text, text) is
  'Claims one bounded server product job and attests deterministic revision fallback only from an exact pending private revision row.';
