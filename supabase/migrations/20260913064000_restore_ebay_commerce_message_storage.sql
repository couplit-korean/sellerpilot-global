...
-- Extend only verified eBay system-message storage; customer/seller limits stay at 20k.
do $guard$ begin
 if (select pg_get_constraintdef(oid) from pg_constraint where conrelid='sellerpilot_private.support_inbound_messages'::regclass and conname='support_inbound_messages_body_check') is distinct from $expected$CHECK (((length(body) >= 1) AND (length(body) <= 20000)))$expected$ then
  raise exception 'EBAY_COMMERCE_BODY_CONSTRAINT_DRIFT';
 end if;
end $guard$;
alter table sellerpilot_private.support_inbound_messages drop constraint support_inbound_messages_body_check;
alter table sellerpilot_private.support_inbound_messages add constraint support_inbound_messages_body_check check (
 length(body)>=1 and (length(body)<=20000 or (
  channel_key='ebay' and sender_role='system'
  and (provider_context->>'kind'='conversation') is true
  and (provider_context->>'conversationType'='FROM_EBAY') is true
  and length(body)<=200000 and octet_length(body)<=600000
 ))
);

-- Forward restoration of two reviewed eBay branches; preserve every current non-eBay wrapper.

do $$ begin if md5(pg_get_functiondef('public.sellerpilot_service_ingest_inquiries(uuid,text,jsonb)'::regprocedure)) is distinct from '99f74b1fe5bfd5728e766cd63821e8fd' then raise exception 'EBAY_COMMERCE_PREIMAGE_DRIFT:sellerpilot_service_ingest_inquiries';end if;end $$;

alter function public.sellerpilot_service_ingest_inquiries(uuid,text,jsonb) rename to sellerpilot_130640_ingest_before_ebay_commerce;

revoke all on function public.sellerpilot_130640_ingest_before_ebay_commerce(uuid,text,jsonb) from public,anon,authenticated,service_role;
...
begin
  if p_channel <> 'ebay'
    ...
    return public.sellerpilot_130640_ingest_before_ebay_commerce(
  ...
  for v_inquiry in select value from jsonb_array_elements(p_inquiries) item(value) loop
    ...
    if jsonb_typeof(v_context) is distinct from 'object'
       ...
       or v_message='' or length(v_message)>(case when v_conversation_type='FROM_EBAY' then 200000 else 20000 end) or octet_length(v_message)>(case when v_conversation_type='FROM_EBAY' then 600000 else 60000 end)
...
do $$ begin if md5(pg_get_functiondef('public.sellerpilot_enqueue_inquiry_reply_gateway_job(uuid,text,text,jsonb)'::regprocedure)) is distinct from 'f2f8fc96c64f9166f173f4f15ba441b9' then raise exception 'EBAY_COMMERCE_PREIMAGE_DRIFT:sellerpilot_enqueue_inquiry_reply_gateway_job';end if;end $$;

alter function public.sellerpilot_enqueue_inquiry_reply_gateway_job(uuid,text,text,jsonb) rename to sellerpilot_130640_reply_before_ebay_commerce;

revoke all on function public.sellerpilot_130640_reply_before_ebay_commerce(uuid,text,text,jsonb) from public,anon,authenticated,service_role;
...
begin
  if p_channel<>'ebay' or p_request_payload#>>'{arguments,kind}' is distinct from'conversation' then
    return public.sellerpilot_130640_reply_before_ebay_commerce(
...
revoke all on function public.sellerpilot_enqueue_inquiry_reply_gateway_job(uuid,text,text,jsonb) from public,anon,authenticated;
