...
declare
  ...
  current_provider_body jsonb;
  current_provider_body_sha256 text;
  current_credential_secret jsonb;
...
begin
  ...
  select row.* into credential from sellerpilot_private.channel_credentials row
   ...
   for key share;
  if not found then raise exception 'COUPANG_CREATE_OFFICIAL_SOURCE_MISMATCH' using errcode = '23514'; end if;
  select decrypted.decrypted_secret::jsonb into current_credential_secret
    from vault.decrypted_secrets decrypted
   where decrypted.id = credential.vault_secret_id;
  if current_credential_secret is null then
    raise exception 'COUPANG_CREATE_OFFICIAL_SOURCE_MISMATCH' using errcode = '23514';
  end if;
  current_credential_secret_sha256 :=
    sellerpilot_private.coupang_create_official_sha256(current_credential_secret);
  select pg_catalog.count(*) into assignment_count
    from sellerpilot_private.product_category_assignments row
  ...
  current_provider_body := new.request_payload#>'{arguments,body}';
  current_provider_body_sha256 :=
    sellerpilot_private.coupang_create_official_sha256(current_provider_body);
  perform sellerpilot_private.assert_coupang_create_official_semantics(
    new.request_payload->'arguments',
  ...
  if snapshot.owner_id is distinct from new.created_by
     ...
     or snapshot.source_provider_body is distinct from current_provider_body
     or snapshot.provider_body_sha256 is distinct from current_provider_body_sha256
     or snapshot.credential_vault_secret_id is distinct from credential.vault_secret_id
     or snapshot.credential_secret_sha256 is distinct from current_credential_secret_sha256
     ...
     or snapshot.snapshot_digest_sha256 is distinct from
       sellerpilot_private.coupang_create_official_snapshot_digest_r8(
         ...
         current_provider_body, current_provider_body_sha256,
         credential.vault_secret_id, current_credential_secret_sha256,
