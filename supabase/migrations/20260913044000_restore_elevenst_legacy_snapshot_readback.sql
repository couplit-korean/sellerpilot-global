...
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_service_enqueue_elevenst_listing_snapshot_recovery') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_service_enqueue_elevenst_listing_snapshot_recovery';end if;
if (select md5(p.prosrc) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_service_listing_publication_verification_source' and pg_get_function_identity_arguments(p.oid)='p_token_hash text, p_job_id uuid, p_claim_token uuid') is distinct from 'e5460d75af36eb1e4810b6441e515ac2' then raise exception 'RECOVERY_PREIMAGE_DRIFT:sellerpilot_service_listing_publication_verification_source';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_service_prepare_elevenst_listing_snapshot_recovery') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_service_prepare_elevenst_listing_snapshot_recovery';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='apply_elevenst_snapshot_recovery') then raise exception 'RECOVERY_ALREADY_DEFINED:apply_elevenst_snapshot_recovery';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='elevenst_legacy_snapshot_immutable_product_matches') then raise exception 'RECOVERY_ALREADY_DEFINED:elevenst_legacy_snapshot_immutable_product_matches';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='elevenst_listing_snapshot_recovery_context') then raise exception 'RECOVERY_ALREADY_DEFINED:elevenst_listing_snapshot_recovery_context';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='elevenst_snapshot_recovery_completion_valid') then raise exception 'RECOVERY_ALREADY_DEFINED:elevenst_snapshot_recovery_completion_valid';end if;
end $recovery_guard$;
-- Reviewed source: 20260831054000_recover_elevenst_listing_snapshot.sql
-- Source SHA256: 5ebd66ed2359d64ff180bb70b07c0e3a04e6b9a35f390647820bb8d13491c99d
...
-- Historical one-product attestation seed intentionally omitted.
-- No source/job/product is vouched for by this function recovery.
...
begin
  ...
  if v_context->>'status' is distinct from 'ready' then return v_context; end if;
