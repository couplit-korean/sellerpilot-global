-- Preserve marketplace revenue when an order-list response omits its total.
-- The fallback uses the already-published listing price for the same product and
-- channel; provider-supplied order amounts always take precedence.
...
update sellerpilot_private.commerce_orders o
   set amount=l.price * o.quantity,
       currency=l.currency,
       amount_krw=case when upper(l.currency)='KRW' then l.price * o.quantity else o.amount_krw end,
       updated_at=now()
  from sellerpilot_private.product_listings l
 where o.owner_id=l.owner_id
   and o.product_id=l.product_id
   and o.channel_key=l.channel_key
   and not o.demo
   and o.amount=0
   and l.status='published'
   and l.price>0;
