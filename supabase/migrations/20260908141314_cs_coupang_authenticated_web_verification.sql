...
-- Exposes only exact identifiers, order lineage, status and conversation text
-- for one explicitly selected Coupang credential and one <=31 day kind scope.
-- Customer name, phone, address and provider_context are never returned.

begin;

create function public.sellerpilot_read_coupang_cs_verification_v1(
...
begin
  ...
  select count(*)::integer into v_total
   ...
   where ticket.owner_id = v_owner
     ...
     and not ticket.demo
     and ticket.external_ticket_id like p_kind || ':%'
...
comment on function public.sellerpilot_read_coupang_cs_verification_v1(
  uuid,date,date,text,integer
) is 'Authenticated exact-ID Coupang CS DB-to-web verification read without customer contact or address fields.';
