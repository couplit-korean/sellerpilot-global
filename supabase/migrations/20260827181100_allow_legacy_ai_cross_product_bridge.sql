-- Keep the read-only cross-product image lookup aligned with the bounded
-- legacy AI-worker bridge used by claim/heartbeat/completion. The grandfathered
-- combined token is accepted only until its original fixed rollout expiry;
-- newly issued gateway or scheduler tokens remain ineligible.

begin;

create or replace function public.sellerpilot_service_get_cross_product_setting_comparisons(
  p_token_hash text,
  p_job_id uuid,
  p_claim_token uuid,
...
begin
  ...
  select token.id
   ...
   where token.token_hash = p_token_hash
     ...
     and sellerpilot_private.worker_token_has_scope(p_token_hash, 'ai', true)
     and (
       token.scope = 'ai'
       or (
         token.scope = 'legacy_combined'
         and token.rotation_set_id is null
         and token.created_at < timestamptz '2026-08-26 09:00:00+00'
         and token.expires_at <= timestamptz '2026-09-01 17:44:00+00'
         and clock_timestamp() < timestamptz '2026-09-01 17:44:00+00'
       )
     );
