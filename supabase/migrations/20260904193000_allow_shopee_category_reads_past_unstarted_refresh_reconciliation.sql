-- Operator-verified 2026-09-04: after category reads were allowed past a
-- failed orders.list reconciliation, the next categories.suggest claimed and
-- then died as reconciliation_required with credential_refresh_in_flight
-- true, recovery vault null, and provider_mutation_started_at null (error
-- serverless_cs_execution_failed). Token refresh starts that flag before
-- any Shopee HTTP. Leave that row unchanged. Allow Shopee category reads
-- past any non-OAuth reconciliation that never started a provider mutation,
-- including an unstarted refresh. oauth.exchange remains fully fenced.
