...
do $guard$ begin
  if md5(pg_get_functiondef('public.sellerpilot_service_gateway_completion_context(text,uuid,uuid)'::regprocedure))
    is distinct from 'f138360fbf393c759017f16c64ee0804' then raise exception 'GATEWAY_COMPLETION_CONTEXT_PREIMAGE_CHANGED'; end if;
end $guard$;
...
begin
  ...
  if result is not null then
    -- The existing wrapper chain has already authenticated this exact claim or
    -- replay receipt. Enrich only its matching job; never synthesize a context.
    select job_row.* into job from sellerpilot_private.channel_gateway_jobs job_row
     where job_row.id = p_job_id
       and result->>'id' = job_row.id::text
       and result->>'channel' = job_row.channel
       and result->>'operation' = job_row.operation;
    if job.id is null then raise exception 'GATEWAY_CONTEXT_JOB_MISMATCH'; end if;
    return result || jsonb_build_object('request',job.request_payload,
      'started_at',job.started_at,'environment',job.environment);
  end if;
...
$function$;
revoke all on function public.sellerpilot_service_gateway_completion_context(text,uuid,uuid) from public,anon,authenticated,service_role;
grant execute on function public.sellerpilot_service_gateway_completion_context(text,uuid,uuid) to service_role;
