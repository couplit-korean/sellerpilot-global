-- The exact Lazada MY OAuth recovery is currently fenced by two independent
-- legacy reconciliation rows: one uncertain OAuth exchange and one read-only
-- orders refresh.  The existing recovery path intentionally accepts exactly
-- one blocker, so it must remain unchanged for every other account and job.
--
-- This forward-only exception admits one fresh, claim-bound OAuth job through
-- this exact pair.  It never creates a job, exchanges a token, rotates a
-- credential, calls Lazada, or grants a listing permit.  Both legacy rows stay
-- immutable until a distinct provider-certified credential has completed the
-- OAuth transaction and a later claim-bound /seller/get readback proves the
-- active MY seller.  Only then are both rows superseded together with one
-- audit record per row.
...
select pg_catalog.pg_advisory_xact_lock(193674993, 902091500);

do $verify_lazada_dual_blocker_preimage$
declare
  v_oauth_signature constant regprocedure :=
  ...
  v_refresh_signature constant regprocedure :=
    'sellerpilot_private.safe_lazada_oauth_refresh_blocker(uuid)'::regprocedure;
  v_oauth_source text;
  v_refresh_source text;
begin
  select proc.prosrc into v_oauth_source
    from pg_catalog.pg_proc proc
   where proc.oid = v_oauth_signature
     and proc.prosecdef
     and proc.provolatile = 's'
     and proc.proconfig is not distinct from array['search_path=""']::text[];
  select proc.prosrc into v_refresh_source
    from pg_catalog.pg_proc proc
   where proc.oid = v_refresh_signature
     and proc.prosecdef
     and proc.provolatile = 's'
     and proc.proconfig is not distinct from array['search_path=""']::text[];
  ...
  if v_oauth_source is null
     or pg_catalog.strpos(
          v_oauth_source,
          'faee01e1-2d68-4f99-951c-15684822fc43'
        ) = 0
     or pg_catalog.strpos(v_oauth_source, 'reconciliation_required') = 0
     or pg_catalog.strpos(v_oauth_source, ') = 1') = 0
     or v_refresh_source is null
     or pg_catalog.strpos(
          v_refresh_source,
          'safe_lazada_oauth_exchange_reauthorization_blocker'
        ) = 0
     or pg_catalog.strpos(
          v_refresh_source,
          'safe_lazada_read_refresh_reauthorization_blocker'
        ) = 0 then
    raise exception 'Lazada dual-blocker helper preimage mismatch'
      using errcode = '55000';
  end if;
end;
$verify_lazada_dual_blocker_preimage$;

create function sellerpilot_private.lazada_exact_dual_blockers_intact(
...
revoke all on function
  sellerpilot_private.lazada_exact_dual_blockers_intact(uuid,uuid,text)
  from public, anon, authenticated, service_role;

create function
  sellerpilot_private.safe_lazada_exact_dual_oauth_exchange_blocker(
    p_source_credential_id uuid,
    p_owner_id uuid,
    p_environment text,
...
set search_path = ''
as $$
  select 'faee01e1-2d68-4f99-951c-15684822fc43'::uuid
   where p_source_credential_id =
     ...
     and sellerpilot_private.lazada_exact_dual_blockers_intact(
...
alter function
  sellerpilot_private.safe_lazada_oauth_exchange_reauthorization_blocker(
    uuid,uuid,text,text,timestamptz
  ) rename to sellerpilot_0209020915_oauth_blocker_pre_dual;

revoke all on function
  sellerpilot_private.sellerpilot_0209020915_oauth_blocker_pre_dual(
    uuid,uuid,text,text,timestamptz
  ) from public, anon, authenticated, service_role;
...
create function sellerpilot_private.safe_lazada_exact_dual_oauth_refresh_blocker(
  p_oauth_job_id uuid
)
returns uuid
language sql
...
as $$
  select sellerpilot_private.safe_lazada_exact_dual_oauth_exchange_blocker(
   ...
   where oauth.id = p_oauth_job_id
     ...
     and oauth.oauth_request_fingerprint <>
           '8a0f1f27e3b168ace4dd70a416b898caa92ef5ac4725fc08e1ea798fb28a6bfa'
     and not exists (
       select 1
         from sellerpilot_private.channel_gateway_jobs other_oauth
...
revoke all on function
  sellerpilot_private.safe_lazada_exact_dual_oauth_refresh_blocker(uuid)
  from public, anon, authenticated, service_role;

alter function sellerpilot_private.safe_lazada_oauth_refresh_blocker(uuid)
  rename to sellerpilot_0209020915_refresh_blocker_pre_dual;

revoke all on function
  sellerpilot_private.sellerpilot_0209020915_refresh_blocker_pre_dual(uuid)
  from public, anon, authenticated, service_role;
...
as $$
  select coalesce(
    sellerpilot_private.sellerpilot_0209020915_refresh_blocker_pre_dual(
      p_oauth_job_id
    ),
    sellerpilot_private.safe_lazada_exact_dual_oauth_refresh_blocker(
      p_oauth_job_id
    )
  );
$$;
...
create function sellerpilot_private.exact_lazada_dual_readback_proof(
  p_target_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  if not sellerpilot_private.lazada_exact_dual_blockers_intact(
    'e54fa95d-ddfd-414f-82e9-636a0d9ab07c'::uuid,
    '768ce4ac-0ef2-4e01-89dc-05aa4fa8543c'::uuid,
    'production'
  ...
  with candidates as materialized (
    select job.*
     ...
     where job.oauth_source_credential_id = v_source.id
       ...
       and job.oauth_request_fingerprint <>
             '8a0f1f27e3b168ace4dd70a416b898caa92ef5ac4725fc08e1ea798fb28a6bfa'
...
revoke all on function
  sellerpilot_private.exact_lazada_dual_readback_proof(uuid)
  from public, anon, authenticated, service_role;

create function sellerpilot_private.guard_exact_lazada_dual_blockers()
returns trigger
language plpgsql
security definer
set search_path = ''
...
declare
  v_marker text := coalesce(current_setting(
    'sellerpilot.exact_lazada_dual_blocker_supersession', true
  ), '');
...
begin
  ...
  if tg_op = 'DELETE' then
    raise exception 'exact Lazada reconciliation blockers are immutable'
      using errcode = '55000';
  end if;
  v_expected_error := case old.id
  ...
  if v_marker !~
     ...
     or new.error_message <> v_expected_error
...
revoke all on function
  sellerpilot_private.guard_exact_lazada_dual_blockers()
  from public, anon, authenticated, service_role;

drop trigger if exists guard_exact_lazada_dual_blockers
  on sellerpilot_private.channel_gateway_jobs;
create trigger guard_exact_lazada_dual_blockers
before update or delete on sellerpilot_private.channel_gateway_jobs
for each row execute function
  sellerpilot_private.guard_exact_lazada_dual_blockers();

create function
  sellerpilot_private.supersede_exact_lazada_dual_blockers_after_readback()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  begin
    ...
    v_proof := sellerpilot_private.exact_lazada_dual_readback_proof(new.id);
    ...
    if v_updated <> 2 then
      raise exception 'exact Lazada dual-blocker supersession lost'
        using errcode = '55000';
    end if;
...
drop trigger if exists supersede_exact_lazada_dual_blockers_after_readback
  on sellerpilot_private.channel_market_targets;
create trigger supersede_exact_lazada_dual_blockers_after_readback
after insert or update on sellerpilot_private.channel_market_targets
for each row execute function
  sellerpilot_private.supersede_exact_lazada_dual_blockers_after_readback();

do $verify_lazada_dual_blocker_postimage$
declare
  v_signature regprocedure;
  v_trigger_count integer;
begin
  foreach v_signature in array array[
    'sellerpilot_private.lazada_exact_dual_blockers_intact(uuid,uuid,text)'::regprocedure,
    'sellerpilot_private.safe_lazada_exact_dual_oauth_exchange_blocker(uuid,uuid,text,text,timestamp with time zone)'::regprocedure,
    'sellerpilot_private.sellerpilot_0209020915_oauth_blocker_pre_dual(uuid,uuid,text,text,timestamp with time zone)'::regprocedure,
    'sellerpilot_private.safe_lazada_oauth_exchange_reauthorization_blocker(uuid,uuid,text,text,timestamp with time zone)'::regprocedure,
    'sellerpilot_private.safe_lazada_exact_dual_oauth_refresh_blocker(uuid)'::regprocedure,
    'sellerpilot_private.sellerpilot_0209020915_refresh_blocker_pre_dual(uuid)'::regprocedure,
    'sellerpilot_private.safe_lazada_oauth_refresh_blocker(uuid)'::regprocedure,
    'sellerpilot_private.exact_lazada_dual_readback_proof(uuid)'::regprocedure,
    'sellerpilot_private.guard_exact_lazada_dual_blockers()'::regprocedure,
    'sellerpilot_private.supersede_exact_lazada_dual_blockers_after_readback()'::regprocedure
  ] loop
    if not exists (
      select 1
      ...
      raise exception 'Lazada dual-blocker function postimage mismatch'
   ...
   where not installed_trigger.tgisinternal
     ...
     and (
       (
         ...
         and installed_trigger.tgname = 'guard_exact_lazada_dual_blockers'
       ...
       or (
         ...
         and installed_trigger.tgname =
               'supersede_exact_lazada_dual_blockers_after_readback'
         and installed_trigger.tgfoid =
           'sellerpilot_private.supersede_exact_lazada_dual_blockers_after_readback()'::regprocedure
       )
     );
  if v_trigger_count <> 2 then
    raise exception 'Lazada dual-blocker trigger postimage mismatch'
      using errcode = '55000';
  end if;
end;
$verify_lazada_dual_blocker_postimage$;

comment on function
  sellerpilot_private.safe_lazada_oauth_refresh_blocker(uuid)
is 'Preserves the general one-blocker Lazada OAuth fence and admits only the exact legacy v5 two-blocker recovery while its fresh OAuth job is queued or running.';
comment on function
  sellerpilot_private.exact_lazada_dual_readback_proof(uuid)
is 'Returns opaque identifiers only after exact provider-certified OAuth and a later active MY seller/get readback prove the Lazada dual-blocker supersession.';
