-- Extend the owner-scoped CS archive with account/shop/kind/source filters and
-- project bounded channel-native attachment metadata into the conversation UI.
begin;

create function public.sellerpilot_search_cs_archive_v2(
...
begin
  ...
  with events as (
    select 'i:'||m.id::text as key,m.sender_role as role,m.body,m.received_at as occurred_at,
      m.created_at as observed_at,'channel'::text as source,'remote_observed'::text as delivery_status,
      m.remote_message_id,null::uuid as job_id,
      case when jsonb_typeof(m.provider_context->'nativeMedia')='object'
    ...
    select 'd:'||d.id::text,'seller',case when d.channel_key='qoo10' then j.request_payload#>>'{arguments,params,contents}'
      ...
      d.provider_message_id,d.gateway_job_id,null::jsonb,'[]'::jsonb
    ...
    select 't:'||v_ticket.id::text,'customer',v_ticket.message,v_ticket.received_at,null::timestamptz,
      'legacy_ticket','recorded',null,null,null::jsonb,'[]'::jsonb
    ...
    'nativeMedia',native_media,'unsequencedAnswers',unsequenced_answers
