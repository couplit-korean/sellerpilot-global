-- Operator-verified 2026-09-04: Shopee categories.suggest stays queued for
-- the full 180s gateway wait even after authenticated Shopee work was taken
-- off the static-egress fence. The channel has a single running slot, so a
-- restarted orders.list / inquiries.list lease occupies production and the
-- interactive category job never claims. Keep oauth.exchange, listing
-- mutations, and identity diagnostics serialized. Do not cancel or rewrite
-- any reconciliation_required row.
