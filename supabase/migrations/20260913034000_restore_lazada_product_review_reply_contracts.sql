...
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_enqueue_lazada_product_review_readback_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_enqueue_lazada_product_review_readback_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_enqueue_lazada_product_review_reply_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_enqueue_lazada_product_review_reply_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_get_lazada_product_review_reply_capability_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_get_lazada_product_review_reply_capability_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_get_lazada_product_review_reply_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_get_lazada_product_review_reply_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_prepare_lazada_product_review_reply_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_prepare_lazada_product_review_reply_v1';end if;
if (select md5(p.prosrc) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_service_begin_gateway_provider_mutation' and pg_get_function_identity_arguments(p.oid)='p_token_hash text, p_job_id uuid, p_claim_token uuid') is distinct from '5a73b0c7ea4f41ba21794ab0d9c0fdbd' then raise exception 'RECOVERY_PREIMAGE_DRIFT:sellerpilot_service_begin_gateway_provider_mutation';end if;
if (select md5(p.prosrc) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_service_begin_serverless_gateway_provider_mutation' and pg_get_function_identity_arguments(p.oid)='p_token_hash text, p_job_id uuid, p_claim_token uuid') is distinct from '31465c874ece47ec0083126c91d6d059' then raise exception 'RECOVERY_PREIMAGE_DRIFT:sellerpilot_service_begin_serverless_gateway_provider_mutation';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_service_record_lazada_product_review_reply_grant_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_service_record_lazada_product_review_reply_grant_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='assert_lazada_product_review_reply_delivery_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:assert_lazada_product_review_reply_delivery_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='assert_lazada_product_review_reply_job_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:assert_lazada_product_review_reply_job_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='guard_lazada_product_review_reply_job_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:guard_lazada_product_review_reply_job_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='sync_lazada_product_review_reply_delivery') then raise exception 'RECOVERY_ALREADY_DEFINED:sync_lazada_product_review_reply_delivery';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='sync_lazada_product_review_reply_exact_result_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sync_lazada_product_review_reply_exact_result_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='sync_lazada_product_review_reply_receipt_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sync_lazada_product_review_reply_receipt_v1';end if;
end $recovery_guard$;
-- Reviewed source: 20260910061000_cs_lazada_product_review_reply.sql
-- Source SHA256: 698c4fc38a45bbbee426b8ca103ca2a6f548a2a77d2ffa8157cd0412b3a3a2e7
...
-- Reviewed source: 20260910064500_cs_lazada_product_review_reply_ui.sql
-- Source SHA256: 872daded747527a9c08b1532ee09fa9c34716ae801b12277b7b72e4f3552e1b0
...
-- Recovery: required lineage/acceptance NULL values cannot verify delivery.
-- Reviewed source: 20260910100300_cs_lazada_product_review_reply_exact_result.sql
-- Source SHA256: d52d46b7451af10058b27e77c26b0f62fa84cae1e2b8a3a09f8f3b57ad00d33f
...
declare
  ...
  v_acceptance jsonb;
begin
  if new.channel is distinct from 'lazada' or new.operation is distinct from 'inquiries.reply'
  ...
  exception when others then return new; end;
  if new.environment is distinct from 'production'
     or new.request_payload#>>'{arguments,deliveryId}' is distinct from v_delivery.id::text
     or new.request_payload#>>'{arguments,country}' is distinct from v_delivery.country
     or new.request_payload#>>'{arguments,sellerAccountKey}' is distinct from v_delivery.seller_account_key
     or new.request_payload#>>'{arguments,reviewId}' is distinct from v_delivery.review_id
     or new.request_payload#>>'{arguments,generation}' is distinct from v_delivery.review_generation::text
     or new.request_payload#>>'{arguments,identityFingerprint}' is distinct from v_delivery.identity_fingerprint
     or new.request_payload#>>'{arguments,reply}' is distinct from v_delivery.reply_text
     or encode(extensions.digest(new.request_payload#>>'{arguments,reply}','sha256'),'hex') is distinct from 
       v_delivery.reply_fingerprint then
    return new;
  end if;
  v_readback:=new.response_payload#>'{steps,0,data,sellerpilotLazadaProductReviewReadback}';
  v_acceptance:=new.response_payload#>'{steps,0,data,sellerpilotReplyAcceptance}';
  if v_kind='product_review' and (
       v_acceptance->>'contract'='sellerpilot-reply-acceptance/1'
       and v_acceptance->>'channel'='lazada'
       and v_acceptance->>'kind'='product_review'
       and v_acceptance->>'level'='provider_accepted') is not true then
    return new;
  end if;
  if v_readback->>'contract'='sellerpilot-lazada-product-review-reply-readback/1'
    ...
    update sellerpilot_private.lazada_product_review_reply_deliveries set
      ...
      reply_observed_at=clock_timestamp(),last_error=null,
      revision=revision+1,updated_at=clock_timestamp()
     where id=v_delivery.id and status is distinct from 'verified';
  end if;
  return new;
end $$;
