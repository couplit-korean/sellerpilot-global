-- Operator-verified 2026-09-04: a Shopee orders.list row is
-- reconciliation_required with credential_refresh_recovery_vault_id set,
-- credential_refresh_in_flight false, and provider_mutation_started_at
-- null (error serverless_cs_execution_failed). That fence currently blocks
-- every Shopee claim, so categories.suggest never leaves queued. Allow
-- Shopee category reads past a non-OAuth reconciliation that never started
-- a provider mutation or in-flight refresh. Do not cancel or rewrite the
-- orders.list row. oauth.exchange remains fully fenced.
       ...
       select 1
        ...
        where unresolved_credential.channel = credential.channel
          ...
          and not (
            ...
            and not coalesce(unresolved.credential_refresh_in_flight, false)
          )
     )
     and not (
       job.channel = 'ebay'
