-- Narrow the 2026-08-30 blanket Shopee static-egress fence
-- (require_static_egress_for_shopee.sql) so it only continues to gate
-- oauth.exchange -- the operation that Shopee actually rejected from the
-- Vercel serverless source address on 2026-08-30 09:38 UTC (job
-- 177eaf2e-3e28-4757-9521-16a517ee3b93). Every other Shopee gateway
-- operation (orders.list, categories.*, shops.get, diagnostic.test,
-- inquiries.list) uses the already-active, unexpired Shopee credential and
-- Shopee does not IP-restrict authenticated calls the same way it restricts
-- the OAuth token exchange itself; historical listing.create/update writes
-- for Shopee never depended on this queue at all and were never blocked.
-- oauth.exchange for Shopee remains fully gated behind
-- serverless_static_egress_allowed('shopee') exactly as before -- this
-- migration does not touch that path, does not enable the
-- serverless_static_egress_policy row for shopee, and does not change
-- coupang/smartstore/elevenst/temu at all. Recovery is intentionally
-- limited to the non-OAuth Shopee read/diagnostic backlog that has been
-- stuck in 'queued' since 2026-08-30.
  ...
  select job.id
   ...
   where job.status = 'queued'
     ...
     and (
       ...
       or (job.channel = 'shopee' and job.operation <> 'oauth.exchange')
     )
