...
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='sellerpilot_service_requeue_coupang_product_reply_readback_v1') then raise exception 'RECOVERY_ALREADY_DEFINED:sellerpilot_service_requeue_coupang_product_reply_readback_v1';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='bind_coupang_product_reply_readback_continuation') then raise exception 'RECOVERY_ALREADY_DEFINED:bind_coupang_product_reply_readback_continuation';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='enqueue_coupang_product_reply_readback_after_acceptance') then raise exception 'RECOVERY_ALREADY_DEFINED:enqueue_coupang_product_reply_readback_after_acceptance';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='enqueue_coupang_reply_readback_after_acceptance') then raise exception 'RECOVERY_ALREADY_DEFINED:enqueue_coupang_reply_readback_after_acceptance';end if;
if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='sellerpilot_private' and p.proname='mark_coupang_product_reply_readback_terminal') then raise exception 'RECOVERY_ALREADY_DEFINED:mark_coupang_product_reply_readback_terminal';end if;
end $recovery_guard$;
-- Reviewed source: 20260908143355_cs_coupang_reply_readback.sql
-- Source SHA256: 97a227621ccc055af2b1a4b88ad771ffd0a7019dcb15b12be088b4a3facc1518
...
-- Reviewed source: 20260909135332_cs_coupang_product_reply_readback.sql
-- Source SHA256: b7f72cdaec9b56433942b1429d575fa30d912de8e9e03280a2fbf819fcf632ab
