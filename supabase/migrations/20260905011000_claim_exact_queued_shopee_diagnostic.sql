-- Fail-closed claimant for one queued Shopee diagnostic.test job.
-- Does not sweep expired leases or select any other queued/recon job.

begin;

create function public.sellerpilot_claim_exact_shopee_diagnostic_job(
  p_token_hash text,
  ...
  begin
    ...
    select jsonb_build_object(
     ...
     where j.id = v_job_id
       ...
       and c.channel = 'shopee'
       and (
         d.decrypted_secret->>'shop_id' = '1719148844'
         or exists (
           select 1
             from pg_catalog.jsonb_array_elements(
               coalesce(d.decrypted_secret->'shopee_targets', '[]'::jsonb)
             ) target
