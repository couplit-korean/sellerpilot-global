-- The remote-edit route deliberately removes Coupang price, inventory, and
-- immutable SKU fields from a content-only patch. The exact closed-gate permit
-- still expected the pre-sanitized draft, so the permit armed successfully but
-- the enqueue transaction rolled back before creating a gateway job. Align the
-- database validator with the server-sanitized patch, then retire only the
-- expired, unbound permit and reclassify the proved pre-gateway attempt.
...
select pg_catalog.pg_advisory_xact_lock(193674993, 909000001);

do $patch_coupang_exact_sanitized_enqueue_contract$
declare
  ...
  v_before text := $body$
      ...
      and (v_item->>'maximumBuyCount')::integer = 1;$body$;
  v_after text := $body$
      and v_item->>'sellerpilotItemMatchId' = '95962393877'
      and v_item->>'modelNo' = 'QA-20260823-CC-001'
      and not (v_item ?| array[
        'externalVendorSku', 'originalPrice', 'salePrice',
        'maximumBuyCount'
      ]);$body$;
begin
  -- Focused migration-history tests intentionally replay subsets that predate
  -- the exact-permit subsystem. A clean subset may omit both the function and
  -- its permit table; a partially applied production schema must still fail
  -- closed instead of silently skipping this forward patch.
  if v_signature is null then
    ...
    raise exception 'Coupang exact enqueue validator is missing'
      using errcode = '55000';
  end if;
  ...
  if pg_catalog.strpos(v_definition, v_after) = 0 then
    if pg_catalog.strpos(v_definition, v_before) = 0 then
      raise exception 'Coupang exact sanitized enqueue patch target not found'
        using errcode = '55000';
    end if;
...
do $reconcile_coupang_exact_sanitized_enqueue_failure$
declare
  ...
  v_attempt_id constant uuid :=
    'a4af8e47-b820-40c4-96ff-67b6e6d930fe'::uuid;
  v_release_sha constant text :=
    'cf2528bf43714ef3394174bd530e65368acdee4e';
...
begin
  ...
  -- A clean migration replay must not manufacture this production-only tuple.
  if v_present_rows = 0 then return; end if;
  if v_present_rows <> 2 then
    raise exception 'COUPANG_EXACT_SANITIZED_ENQUEUE_RECONCILIATION_INCOMPLETE'
      using errcode = '55000';
  end if;
  ...
  perform 1
    ...
    join sellerpilot_private.exact_existing_update_permits permit
      on permit.channel = listing.channel_key
  ...
  if not found then
    raise exception 'COUPANG_EXACT_SANITIZED_ENQUEUE_RECONCILIATION_MISMATCH'
      using errcode = '55000';
  end if;
  ...
  if v_updated_rows <> 1 then
    raise exception 'COUPANG_EXACT_SANITIZED_ENQUEUE_PERMIT_RETIRE_FAILED'
      using errcode = '55000';
  end if;
  ...
  get diagnostics v_updated_rows = row_count;
  if v_updated_rows <> 1 then
    raise exception 'COUPANG_EXACT_SANITIZED_ENQUEUE_LISTING_RESTORE_FAILED'
      using errcode = '55000';
  end if;
end;
