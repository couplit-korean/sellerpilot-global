-- Project only undated answer bodies retained by the CS adapter; never invent event time.
-- The existing role, owner and pagination boundaries remain unchanged.
begin;
...
begin
  ...
  with events as (
    ...
    select 'd:'||d.id::text, 'seller',
      ...
      d.provider_message_id, d.gateway_job_id, '[]'::jsonb
    ...
    select 't:'||v_ticket.id::text,'customer',v_ticket.message,
      v_ticket.received_at,null::timestamptz,'legacy_ticket','recorded',null,null,'[]'::jsonb
    where not exists (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'unsequencedAnswers',unsequenced_answers
