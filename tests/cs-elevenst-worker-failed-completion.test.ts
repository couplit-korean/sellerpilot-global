...
function overflow(count: number): ChannelOperationResult {
  return {
    ok: false,
    channel: "elevenst",
    operation: "inquiries.list",
    steps: [{
      ...
      data: {
        ...
        sellerpilotElevenstProductQnaObservedRows: count,
        sellerpilotElevenstProductQnaParseIncomplete: true,
        sellerpilotElevenstProductQnaParseError: count > 5_000
    ...
    safeMessage: "raw provider message must not cross",
  };
}
...
test("actual worker failure payload preserves only bounded 11st Product Q&A overflow evidence", () => {
  for (const count of [501, 5_000, 5_001]) {
    const payload = buildGatewayWorkerFailedCompletionPayload({
      job,
      claimToken,
      error: "11st Product Q&A row limit exceeded.",
      result: overflow(count),
    });
    assert.equal(payload.result?.ok, false);
    assert.equal(payload.result?.steps[0]?.data.sellerpilotElevenstProductQnaObservedRows, count);
    assert.equal(payload.result?.steps[0]?.data.sellerpilotElevenstProductQnaParseIncomplete, true);
    assert.equal(
      payload.result?.steps[0]?.data.sellerpilotElevenstProductQnaParseError,
      count > 5_000
        ? "ELEVENST_PRODUCT_QNA_PARSER_ROW_LIMIT_EXCEEDED"
        : "ELEVENST_PRODUCT_QNA_DB_BATCH_LIMIT_EXCEEDED",
    );
    assert.deepEqual(payload.result?.steps[0]?.data.productQnas, []);
    assert.doesNotMatch(JSON.stringify(payload), /raw provider|memID|must-not-cross/u);
  }
});
