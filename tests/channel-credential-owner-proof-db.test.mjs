...
async function setup() {
  ...
  `);
  // Use the actual tracked admin function, not a mocked boolean implementation.
  ...
  const start = base.indexOf("create or replace function public.sellerpilot_is_admin()");
...
test("in-memory DB executes the actual migration and full positive/negative contract", async t => {
  ...
  try {
    ...
    await t.test("authenticated same-owner active credential returns exact safe fields", async () => {
      assert.deepEqual(await call(db), { contractVersion: 1, credentialId: id, ownerId: owner, channel: "coupang", environment: "production", credentialVersion: 7, expiresAt: null });
    ...
    await t.test("UID-less authenticated, non-admin, other admin owner and privileged owner sessions deny", async () => {
      for (const args of [{uid:null},{uid:"44444444-4444-4444-8444-444444444444"},{uid:other},{role:"none"},{role:"service_role",uid:null}]) await assert.rejects(call(db,args), {code:"42501"});
    ...
    await t.test("inactive, expired, boundary expiry and changed owner deny", async () => {
