...
import { temuCsReadiness, type TemuCsRuntimeState } from "../lib/channels/cs/temu/runtime-readiness";
...
test("Temu Buyer Chat remains unavailable without its own official contract and permission", () => {
  assert.deepEqual(temuCsReadiness("buyer_chat", readyState), {
    ready: false,
    kind: "buyer_chat",
    blockers: ["TEMU_BUYER_CHAT_CONTRACT_UNVERIFIED"],
  });
  const contract = {
    listApi: "verified-list-api",
    messageHistoryApi: "verified-history-api",
    webhookEvent: "verified-chat-event",
    replyApi: "verified-reply-api",
    permissionPackage: "Verified Buyer Chat Permission",
  };
  assert.deepEqual(temuCsReadiness("buyer_chat", { ...readyState, buyerChatContract: contract }), {
    ready: false,
    kind: "buyer_chat",
    blockers: ["TEMU_BUYER_CHAT_PERMISSION_MISSING"],
  });
  assert.equal(temuCsReadiness("buyer_chat", {
    ...readyState,
    buyerChatContract: contract,
    permissionPackages: [...readyState.permissionPackages, contract.permissionPackage],
  }).ready, true);
});
