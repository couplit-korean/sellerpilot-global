...
const [operationNamesSource, operationsSource, serverlessProviderSource, csProviderSource,
  serverlessGatewaySource, localBeginRouteSource, localCsWorkerSource, deliveryLedgerSource,
  draftRouteSource] = await Promise.all([
  'lib/channels/operation-names.ts',
  'lib/channels/operations.ts',
  'lib/channels/serverless-gateway-provider.ts',
  'lib/cs/operations/provider.ts',
  'lib/channels/serverless-gateway.ts',
  'app/api/channel-gateway/worker/begin-mutation/route.ts',
  'scripts/cs-gateway-job.mjs',
  'supabase/migrations/20260831033000_add_cs_message_delivery_ledger.sql',
  'app/api/admin/cs/drafts/route.ts',
...
async function fixture() {
  ...
  await db.exec(`
    ...
    create table sellerpilot_private.cs_reply_draft_jobs(
      id uuid primary key,ticket_id uuid not null,requested_by uuid not null,
      inbound_key text not null,request_payload jsonb not null,
      status text not null default 'queued',claim_token uuid,worker_token_hash text,
      lease_expires_at timestamptz,attempt_count integer not null default 0,
      result_payload jsonb,error_message text,
  ...
  await db.exec(`
    ...
    create function sellerpilot_private.worker_token_has_scope(
      p_token_hash text,p_scope text,p_live boolean default true
    ) returns boolean language sql stable set search_path='' as $$
      select exists(select 1 from sellerpilot_private.ai_cli_worker_tokens t
        where t.token_hash=p_token_hash and t.scope=p_scope and t.status='active'
          and (not p_live or t.expires_at>clock_timestamp()))
    $$;
    ...
    create function public.sellerpilot_create_cs_reply_draft(uuid,uuid,text,text,text)
      returns uuid language plpgsql security definer set search_path='' as $$
      declare t record;
      begin
        if auth.uid() is null or not public.sellerpilot_is_admin() then raise exception 'administrator access required'; end if;
        select * into t from sellerpilot_private.support_tickets where id=$2 and not demo for update;
        if not found then raise exception 'support ticket not found'; end if;
        if t.latest_inbound_key is distinct from $3 then raise exception 'INQUIRY_CONTEXT_STALE'; end if;
        insert into sellerpilot_private.cs_reply_draft_jobs(
          id,ticket_id,requested_by,inbound_key,request_payload
        ) values($1,t.id,auth.uid(),$3,jsonb_build_object(
          'ticket_id',t.id,'sellerpilotInboundKey',t.latest_inbound_key,
          'channel',t.channel_key,'target_locale',$4,'tone',$5,
          'subject','sensitive subject','message','sensitive original body','order',null));
        return $1;
      end $$;
    create function public.sellerpilot_get_cs_reply_draft(uuid)
      returns jsonb language plpgsql stable security definer set search_path='' as $$
      declare j record;
      begin
        if auth.uid() is null or not public.sellerpilot_is_admin() then raise exception 'administrator access required'; end if;
        select * into j from sellerpilot_private.cs_reply_draft_jobs where id=$1;
        if not found then return null; end if;
        return jsonb_build_object('id',j.id,'status',j.status,'result',j.result_payload,
          'error',j.error_message,'createdAt',j.created_at,'updatedAt',j.updated_at,
          'attemptCount',j.attempt_count);
      end $$;
    create function public.sellerpilot_claim_cs_reply_draft(text)
      returns jsonb language plpgsql security definer set search_path='' as $$
      declare j record;
      begin
        if sellerpilot_private.worker_token_has_scope($1,'ai',true) is distinct from true then raise exception 'CS worker authentication required'; end if;
        select * into j from sellerpilot_private.cs_reply_draft_jobs where status='queued'
          order by created_at,id for update skip locked limit 1;
        if not found then return null; end if;
        update sellerpilot_private.cs_reply_draft_jobs set status='running',claim_token=gen_random_uuid(),
          worker_token_hash=$1,lease_expires_at=clock_timestamp()+interval '90 seconds',
          attempt_count=attempt_count+1,started_at=clock_timestamp(),updated_at=clock_timestamp()
          where id=j.id returning * into j;
        return jsonb_build_object('id',j.id,'claim_token',j.claim_token,'request',j.request_payload,
          'attempt_count',j.attempt_count,'lease_expires_at',j.lease_expires_at);
      end $$;
    create function public.sellerpilot_complete_cs_reply_draft(text,uuid,uuid,text,jsonb,text)
      returns text language plpgsql security definer set search_path='' as $$
      declare j record;
      begin
        if sellerpilot_private.worker_token_has_scope($1,'ai',true) is distinct from true then raise exception 'CS worker authentication required'; end if;
        select * into j from sellerpilot_private.cs_reply_draft_jobs where id=$2 for update;
        if not found or j.claim_token is distinct from $3 or j.worker_token_hash is distinct from $1
          or j.status<>'running' then return 'lease_lost'; end if;
        update sellerpilot_private.cs_reply_draft_jobs set status=$4,result_payload=$5,
          error_message=$6,lease_expires_at=null,updated_at=clock_timestamp(),completed_at=clock_timestamp()
          where id=j.id;
        return 'completed';
      end $$;
    create function public.sellerpilot_create_support_reply_job(uuid,uuid,text,text,text)
      returns uuid language sql security definer set search_path='' as $$
      select public.sellerpilot_create_cs_reply_draft($1,$2,$3,$4,$5) $$;
    ...
    grant execute on function public.sellerpilot_create_cs_reply_draft(uuid,uuid,text,text,text) to authenticated;
    grant execute on function public.sellerpilot_get_cs_reply_draft(uuid) to authenticated;
    grant execute on function public.sellerpilot_claim_cs_reply_draft(text) to service_role;
    grant execute on function public.sellerpilot_complete_cs_reply_draft(text,uuid,uuid,text,jsonb,text) to service_role;
  ...
  await db.query(`insert into sellerpilot_private.admin_users values($1),($2)`, [owner, otherOwner]);
  ...
  await db.query(`insert into sellerpilot_private.ai_cli_worker_tokens values
    ...
    ($5,$6,'ai','active',clock_timestamp()+interval '1 day')`,
  [localToken, localHash, serverlessToken, serverlessHash, aiToken, aiHash]);
...
test('010 proposal uses the canonical V3 seller lock and removes the mutation-time cutoff', () => {
  ...
  assert.match(proposalSql, /alter function public\.sellerpilot_create_cs_reply_draft/);
  assert.match(proposalSql, /alter function public\.sellerpilot_get_cs_reply_draft/);
  assert.match(proposalSql, /alter function public\.sellerpilot_claim_cs_reply_draft/);
  assert.match(proposalSql, /alter function public\.sellerpilot_complete_cs_reply_draft/);
  assert.match(draftRouteSource, /rpc\("sellerpilot_create_cs_reply_draft"/);
});

test('both real worker call paths cross the provider boundary and reconciliation cannot be re-enqueued', () => {
  const writeSet = operationNamesSource.slice(
    operationNamesSource.indexOf('export const writeChannelOperations'),
    operationNamesSource.indexOf(']);', operationNamesSource.indexOf('export const writeChannelOperations')) + 3,
  );
  ...
  assert.match(localCsWorkerSource,
    /const beginProviderMutation = createGatewayMutationBoundary\([\s\S]{0,400}\/api\/channel-gateway\/worker\/begin-mutation/);
  assert.match(localCsWorkerSource,
    /executeProvider\(\{ job, signal, hooks: \{ assertLeaseHealthy, beginProviderMutation,/);
...
test('current dedicated draft queue preserves shared admins and never releases body or draft after recall', async () => {
  const db = await fixture();
  try {
    const sharedNormal = await addTicket(db, 'shared-normal');
    await db.query(`select set_config('request.jwt.claim.sub',$1,false)`, [otherOwner]);
    assert.equal((await db.query(`select public.sellerpilot_update_ticket($1,'waiting','shared admin draft',$2) ok`,
      [sharedNormal, 'inbound-shared-normal'])).rows[0].ok, true);
    const normalDraft = crypto.randomUUID();
    assert.equal((await db.query(`select public.sellerpilot_create_cs_reply_draft($1,$2,$3,'ko-KR','polite') id`,
      [normalDraft, sharedNormal, 'inbound-shared-normal'])).rows[0].id, normalDraft);
    await db.query(`update sellerpilot_private.cs_reply_draft_jobs set status='cancelled' where id=$1`,
      [normalDraft]);

    const blockedCreate = await addTicket(db, 'shared-blocked-create');
    await ingest(db, revision('shared-blocked-create', 'recalled'));
    await assert.rejects(db.query(`select public.sellerpilot_update_ticket($1,'resolved','blocked draft',$2)`,
      [blockedCreate, 'inbound-shared-blocked-create']), /LAZADA_IM_LATEST_MESSAGE_NOT_ACTIONABLE/);
    await assert.rejects(db.query(`select public.sellerpilot_create_cs_reply_draft($1,$2,$3,'ko-KR','polite')`,
      [crypto.randomUUID(), blockedCreate, 'inbound-shared-blocked-create']), /LAZADA_IM_LATEST_MESSAGE_NOT_ACTIONABLE/);

    const blockedClaim = await addTicket(db, 'blocked-claim');
    const blockedClaimDraft = crypto.randomUUID();
    await db.query(`select public.sellerpilot_create_cs_reply_draft($1,$2,$3,'ko-KR','polite')`,
      [blockedClaimDraft, blockedClaim, 'inbound-blocked-claim']);
    await ingest(db, revision('blocked-claim', 'recalled'));
    await db.exec('set role service_role');
    const noClaim = await db.query(`select public.sellerpilot_claim_cs_reply_draft($1) claim`, [aiHash]);
    await db.exec('reset role');
    assert.equal(noClaim.rows[0].claim, null);
    const failedClaim = (await db.query(`select status,result_payload from sellerpilot_private.cs_reply_draft_jobs where id=$1`,
      [blockedClaimDraft])).rows[0];
    assert.equal(failedClaim.status, 'failed');
    assert.equal(failedClaim.result_payload, null);

    const blockedComplete = await addTicket(db, 'blocked-complete');
    const blockedCompleteDraft = crypto.randomUUID();
    await db.query(`select public.sellerpilot_create_cs_reply_draft($1,$2,$3,'ko-KR','polite')`,
      [blockedCompleteDraft, blockedComplete, 'inbound-blocked-complete']);
    await db.exec('set role service_role');
    const claim = (await db.query(`select public.sellerpilot_claim_cs_reply_draft($1) claim`, [aiHash])).rows[0].claim;
    await db.exec('reset role');
    assert.equal(claim.id, blockedCompleteDraft);
    assert.equal(claim.request.message, 'sensitive original body');
    await ingest(db, revision('blocked-complete', 'recalled'));
    await db.exec('set role service_role');
    const completion = await db.query(`select public.sellerpilot_complete_cs_reply_draft(
      $1,$2,$3,'succeeded',$4::jsonb,null) result`, [aiHash, blockedCompleteDraft,
      claim.claim_token, JSON.stringify({ mode: 'support-reply', targetLocale: 'ko-KR', draft: 'secret generated draft' })]);
    await db.exec('reset role');
    assert.equal(completion.rows[0].result, 'lease_lost');
    const completedState = (await db.query(`select status,result_payload from sellerpilot_private.cs_reply_draft_jobs where id=$1`,
      [blockedCompleteDraft])).rows[0];
    assert.equal(completedState.status, 'failed');
    assert.equal(completedState.result_payload, null);
    const readback = (await db.query(`select public.sellerpilot_get_cs_reply_draft($1) draft`,
      [blockedCompleteDraft])).rows[0].draft;
    assert.equal(readback.status, 'failed');
    assert.equal(readback.result, null);
    assert.doesNotMatch(JSON.stringify(readback), /sensitive original body|secret generated draft/);
  } finally { await db.close(); }
});
  ...
  try {
    for (const role of ['anon', 'authenticated', 'service_role']) {
      const acl = (await db.query(`select
        ...
        has_function_privilege($1,'public.sellerpilot_09103500_create_cs_reply_draft_unsafe(uuid,uuid,text,text,text)','EXECUTE') unsafe_draft_create,
        has_function_privilege($1,'public.sellerpilot_09103500_get_cs_reply_draft_unsafe(uuid)','EXECUTE') unsafe_draft_get,
        has_function_privilege($1,'public.sellerpilot_09103500_claim_cs_reply_draft_unsafe(text)','EXECUTE') unsafe_draft_claim,
        has_function_privilege($1,'public.sellerpilot_09103500_complete_cs_reply_draft_unsafe(text,uuid,uuid,text,jsonb,text)','EXECUTE') unsafe_draft_complete`,
      [role])).rows[0];
      assert.equal(acl.ticket_lock, false);
      assert.equal(acl.job_lock, false);
      assert.equal(acl.unsafe_enqueue, false);
      assert.equal(acl.unsafe_draft_create, false);
      assert.equal(acl.unsafe_draft_get, false);
      assert.equal(acl.unsafe_draft_claim, false);
      assert.equal(acl.unsafe_draft_complete, false);
    }
  } finally { await db.close(); }
});
