...
import { shippingSnapshotSchema } from "../lib/shipping/snapshot.ts";
const source = (name) =>
...
async function fixture() {
 ...
 `);
  await db.exec(
    `create table sellerpilot_private.channel_sync_state(channel_key text,data_type text,status text,imported_count integer,last_started_at timestamptz,last_succeeded_at timestamptz,last_error text,updated_at timestamptz);`,
  );
  await db.exec(migration);
  return db;
}

test("product registration read model runs with shipping and CS tables absent", async () => {
  const db = await fixture();
  try {
    await db.exec(
      `drop table sellerpilot_private.commerce_orders;drop table sellerpilot_private.channel_sync_state;set role authenticated;set request.jwt.claim.sub='${admin}';`,
    );
    const value = (
      await db.query(
        "select public.sellerpilot_get_product_registration_snapshot() value",
      )
    ).rows[0].value;
    assert.equal(value.contract, "sellerpilot-product-registration-snapshot/1");
    ...
    assert.equal(value.summary.productCount, 1);
    assert.deepEqual(value.orders, []);
  } finally {
    await db.close();
  }
});
test("shipping snapshot and sales run with product registration and CS tables absent", async () => {
  const db = await fixture();
  try {
    await db.exec(
      `drop table sellerpilot_private.products;drop table sellerpilot_private.product_listings;drop table sellerpilot_private.ai_cli_jobs;drop table sellerpilot_private.channel_credentials;drop table sellerpilot_private.channel_operation_attempts;set role authenticated;set request.jwt.claim.sub='${admin}';`,
    );
    const value = (
      await db.query("select public.sellerpilot_get_shipping_snapshot() value")
    ).rows[0].value;
    assert.equal(value.contract, "sellerpilot-shipping-snapshot/1");
    assert.equal(value.orders.length, 1);
    assert.equal(value.orders[0].productName, "Fixture product");
    ...
    assert.equal(value.channelMetrics[0].readyToShipCount, 1);
    assert.equal(value.productSales[0].productId, product);
    const analytics = (
      await db.query(
        "select public.sellerpilot_get_shipping_sales_analytics(current_date-29,current_date) value",
      )
    ).rows[0].value;
    assert.equal(analytics.summary.revenueKrw, 40000);
    assert.equal(analytics.products[0].sold, 2);
    assert.equal(analytics.products[0].channels[0].channelKey, "elevenst");
    assert.equal(
      shippingSnapshotSchema.safeParse({ ...value, analytics }).success,
      true,
    );
  } finally {
    await db.close();
...
test("all new read models reject anonymous and non-admin sessions", async () => {
  const db = await fixture();
  try {
    for (const rpc of [
      "sellerpilot_get_product_registration_snapshot()",
      "sellerpilot_get_shipping_snapshot()",
      "sellerpilot_get_shipping_sales_analytics(current_date,current_date)",
    ]) {
      await db.exec("reset role;set role anon");
      await assert.rejects(
        db.query(`select public.${rpc}`),
        /permission denied/,
      );
      await db.exec(
...
test("shipping replay RPC only returns the exact credential/channel/operation attempt", async () => {
  const db = await fixture();
  try {
    const attempt = "00000000-0000-4000-8000-000000000701",
      credential = "00000000-0000-4000-8000-000000000702";
    await db.exec(`alter table sellerpilot_private.channel_operation_attempts add column id uuid,add column credential_id uuid;
 create table sellerpilot_private.channel_gateway_jobs(id uuid,attempt_id uuid,credential_id uuid,channel text,operation text,status text,response_payload jsonb,created_at timestamptz);
 insert into sellerpilot_private.channel_operation_attempts(id,credential_id,channel,operation)values('${attempt}','${credential}','qoo10','orders.get');
 insert into sellerpilot_private.channel_gateway_jobs values('${job}','${attempt}','${credential}','qoo10','orders.get','succeeded','{"ok":true,"channel":"qoo10","operation":"orders.get","steps":[]}',now());
 set role authenticated;set request.jwt.claim.sub='${admin}';`);
    const read = async (channel, operation) =>
      (
        await db.query(
          "select public.sellerpilot_get_shipping_attempt_result($1,$2,$3,$4) value",
          [attempt, credential, channel, operation],
        )
      ).rows[0].value;
    assert.equal((await read("qoo10", "orders.get")).response.ok, true);
    assert.equal(await read("ebay", "orders.get"), null);
    assert.equal(await read("qoo10", "shipment.confirm"), null);
    await assert.rejects(
      read("qoo10", "listing.create"),
      /shipping operation required/,
    );
  } finally {
    await db.close();
  }
