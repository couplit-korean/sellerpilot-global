...
test("contains the complete multi-channel operating storyboard and 175-item acceptance baseline", async () => {
  ...
  assert.match(manualSyncRoute, /if \(!parsed\.data\.includeImBootstrap\)[\s\S]{0,520}delegated: true[\s\S]{0,520}status: 409/);
  ...
  assert.match(cliWorker, /sellerpilot-cli-worker\/1\.27/);
