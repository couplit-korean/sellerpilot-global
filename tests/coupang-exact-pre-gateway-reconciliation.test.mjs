...
const migrationUrl = new URL(
  "../supabase/migrations/20260901082500_reconcile_coupang_exact_pre_gateway_failure.sql",
  import.meta.url,
);
...
const attemptId = "edea9f6b-6527-4367-96cb-08b368ea80f9";
...
async function createDatabase({ insertExactRows = true, insertGatewayJob = false } = {}) {
  ...
  await db.query(
    ...
    [attemptId, ownerId, credentialId, "1".repeat(64), safeMessage, sellerAccountKey],
  );
  await db.query(
...
test("exact Coupang pre-gateway failure becomes the existing external-action recovery candidate", async () => {
  const db = await createDatabase();
  try {
    await db.exec(migration);
    const listing = (await db.query(
      `select failure_class, operation_attempt_id, remote_id, market, target_id,
...
test("any gateway job makes the exact reconciliation fail closed", async () => {
  const db = await createDatabase({ insertGatewayJob: true });
  try {
    await assert.rejects(
      db.exec(migration),
      /COUPANG_EXACT_PRE_GATEWAY_RECONCILIATION_MISMATCH/,
    );
  } finally {
    await db.close();
  }
...
test("clean replay does not manufacture the exact production tuple", async () => {
