...
test("questionId and inquiryNo remain distinct even when their numeric values collide", () => {
  ...
  assert.deepEqual({
    identityContract: product[0]?.providerContext.identityContract,
    legacyExternalTicketId: product[0]?.providerContext.legacyExternalTicketId,
    providerTicketKind: product[0]?.providerContext.providerTicketKind,
    providerTicketId: product[0]?.providerContext.providerTicketId,
  }, {
    identityContract: "smartstore-provider-ticket-v1",
    legacyExternalTicketId: "smartstore:product-qna:701",
    providerTicketKind: "product",
    providerTicketId: "701",
  });
  assert.equal(customer[0]?.providerContext.legacyExternalTicketId, "customer:701");
...
test("product answer state uses explicit evidence across contradictory provider fields", () => {
  const base = {
    questionId: 950,
    question: "답변 상태 확인",
    createDate: "2026-09-08T09:00:00+09:00",
  ...
  const normalize = (fields: Record<string, unknown>) => normalizeSmartstoreInquiries({
    sellerpilotInquiryKind: "product",
    contents: [{ ...base, ...fields }],
  }, iso);

  const observedAnswer = normalize({
    answered: false,
    answers: [{ answer: "배열에 존재하는 판매자 답변", createDate: "2026-09-08T09:05:00+09:00" }],
  });
  assert.equal(observedAnswer[0]?.status, "resolved");
  assert.equal(observedAnswer[1]?.senderRole, "seller");

  const legacyAnswer = normalize({
    answered: false,
    answers: [{ answer: "   " }],
    answer: "legacy 판매자 답변",
  });
  assert.equal(legacyAnswer[0]?.status, "resolved");
  assert.deepEqual(legacyAnswer[0]?.providerContext.unsequencedAnswers, [{
    body: "legacy 판매자 답변",
    reason: "provider_timestamp_unavailable",
  }]);

  const negativeFixtures = [
    { name: "string flag", fields: { answered: "true", answers: [] } },
    { name: "empty answer", fields: { answered: false, answers: [{ answer: "   " }] } },
    { name: "non-string answer", fields: { answered: false, answers: [{ answer: 1 }] } },
  ];
  for (const fixture of negativeFixtures) {
    const normalized = normalize(fixture.fields);
    assert.equal(normalized[0]?.status, "waiting", fixture.name);
    assert.equal(normalized.length, 1, fixture.name);
  }

  const flagOnly = normalize({ answered: true, answers: [] });
  assert.equal(flagOnly[0]?.status, "resolved");
