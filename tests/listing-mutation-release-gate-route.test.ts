...
test("the post-image shipping contract is checked before any gateway enqueue", async () => {
  ...
  assert.equal(atomicEnqueueIndex, -1, "the retired product-specific enqueue path must stay deleted");
...
test("Qoo10 recovery capability is server-bound, survives normalization, and preserves the S1 ledger on pre-gateway retry", async () => {
  ...
  assert.match(route, /if \(boundQoo10RollbackUpdateRecovery\) effectiveArguments = bindQoo10RollbackUpdateRecoveryArguments\(effectiveArguments, boundQoo10RollbackUpdateRecovery\)/);
  ...
  assert.match(
    route,
    /if \(!\(preGatewayRetryable && boundQoo10RollbackUpdateRecovery\)\) \{\s*await completeListing\(\{ success: false/,
  );
});
