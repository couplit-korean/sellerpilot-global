...
const paginationEpochSql = await readFile(new URL("../supabase/migrations/20260909105857_cs_smartstore_pagination_epoch.sql", import.meta.url), "utf8");
...
async function fixture(instant) {
  ...
  await db.exec(paginationEpochSql);
  return db;
}
...
test("v7 preserves exact receipts and scope across a 55-page bounded-epoch lineage", async () => {
  const db = await fixture("2024-08-03 00:05:00+09");
  try {
    const run = await start(db, "2024-08-01", "2024-08-01");
    const initial = await roots(db, run.runId);
    ...
    for (let page = 1; page <= 55; page += 1) {
      await recordSucceededPage(db, current, page < 55);
      if (page === 55) break;
      if (depth >= 49) {
        depth = 0;
        epoch += 1;
      ...
      depth += 1;
      trail = [...trail, page.toString(16).padStart(64, "0")].slice(-50);
      current = await insertContinuation(db, current, {
        depth,
        arguments: {
          sellerpilotPaginationDepth: depth,
          sellerpilotPaginationEpoch: epoch,
          sellerpilotPaginationTrail: trail,
        },
      });
    }
    assert.equal((await refresh(db, run.runId)).status, "succeeded");

    const receiptsBefore = (await db.query(`select job_id,claim_token,worker_token_id
      from sellerpilot_private.gateway_completion_receipts order by job_id`)).rows;
    const reused = await start(db, "2024-08-01", "2024-08-01");
    assert.equal(reused.reused, true);
    assert.equal(reused.retriedJobs, 0);
    assert.equal(reused.totalJobs, 56);
    assert.deepEqual((await db.query(`select job_id,claim_token,worker_token_id
      from sellerpilot_private.gateway_completion_receipts order by job_id`)).rows, receiptsBefore);

    const last = (await db.query(`select credential_id,created_by,environment,request_payload
      from sellerpilot_private.channel_gateway_jobs where id=$1`, [current])).rows[0];
    assert.equal(last.credential_id, credential);
    assert.equal(last.created_by, seller);
    assert.equal(last.environment, "production");
    assert.equal(last.request_payload.arguments.query.page, 55);
    assert.equal(last.request_payload.arguments.sellerpilotPaginationDepth, 5);
    assert.equal(last.request_payload.arguments.sellerpilotPaginationEpoch, 1);
    assert.equal(last.request_payload.arguments.sellerpilotPaginationTrail.length, 50);
    assert.equal(last.request_payload.arguments.sellerpilotHistoryRunId, run.runId);
    assert.equal(last.request_payload.arguments.sellerpilotHistoryItemKey,
      "product:2024-08-01:2024-08-01");
    assert.equal((await checkpoint(db, "2024-08-01", "2024-08-01")).complete, true);

    await db.query(`update sellerpilot_private.channel_gateway_jobs
      set request_payload=jsonb_set(request_payload,
        '{arguments,sellerpilotPaginationEpoch}','2'::jsonb,false)
      where id=$1`, [current]);
    await assert.rejects(start(db, "2024-08-01", "2024-08-01"),
      /SMARTSTORE_EXACT_HISTORY_EXISTING_SCOPE_MISMATCH/);
    assert.deepEqual((await db.query(`select job_id,claim_token,worker_token_id
      from sellerpilot_private.gateway_completion_receipts order by job_id`)).rows, receiptsBefore);
  } finally {
...
const retryReceiptProposalSql = await readFile(new URL(
  "../supabase/migrations/20260909112831_cs_smartstore_retry_receipt_retirement.sql",
  import.meta.url,
), "utf8");
test("canonical v7 receipt retirement preserves audit evidence and permits exact same-job completion", async () => {
  const db = await fixture("2024-08-03 00:05:00+09");
  try {
    assert.doesNotMatch(retryReceiptProposalSql, /create\s+(?:or\s+replace\s+)?function\s+public\./iu);
    assert.doesNotMatch(retryReceiptProposalSql, /\bgrant\s+/iu);
    await db.exec(retryReceiptProposalSql);
    const run = await start(db, "2024-08-01", "2024-08-01");
    const initial = await roots(db, run.runId);
    const customer = initial.find(job => job.request_payload.arguments.kind === "customer");
    const product = initial.find(job => job.request_payload.arguments.kind === "product");
    ...
    await recordSucceededPage(db, product.id, true);
    const child = await insertContinuation(db, product.id, { status: "failed", attemptCount: 1 });
    const oldClaim = "00000000-0000-4000-9000-000000000098";
    await db.query(
      "insert into sellerpilot_private.gateway_completion_receipts values($1,$2,$3)",
      [child, oldClaim, tokenId],
    );
    const unrelatedReceipts = (await db.query(
      "select job_id,claim_token,worker_token_id from sellerpilot_private.gateway_completion_receipts where job_id<>$1 order by job_id",
      [child],
    )).rows;
    ...
    assert.deepEqual((await db.query(
      `select job_id,claim_token,history_run_id,history_item_key,owner_id,
              credential_id,environment,receipt_snapshot->>'claim_token' archived_claim,
              receipt_sha256=encode(extensions.digest(receipt_snapshot::text,'sha256'),'hex') receipt_hash_valid,
              failed_job_sha256=encode(extensions.digest(failed_job_snapshot::text,'sha256'),'hex') job_hash_valid,
              contract
         from sellerpilot_private.smartstore_history_retired_completion_receipts`,
    )).rows, [{
      job_id: child,
      claim_token: oldClaim,
      history_run_id: run.runId,
      history_item_key: "product:2024-08-01:2024-08-01",
      owner_id: seller,
      credential_id: credential,
      environment: "production",
      archived_claim: oldClaim,
      receipt_hash_valid: true,
      job_hash_valid: true,
      contract: "smartstore_history_retry_receipt_v1",
    }]);
    assert.equal((await db.query(
      "select count(*)::integer count from sellerpilot_private.gateway_completion_receipts where job_id=$1",
      [child],
    )).rows[0].count, 0);
    assert.deepEqual((await db.query(
      "select job_id,claim_token,worker_token_id from sellerpilot_private.gateway_completion_receipts where job_id<>$1 order by job_id",
      [child],
    )).rows, unrelatedReceipts);

    await recordSucceededPage(db, child, false);
    const activeReceipt = (await db.query(
      "select claim_token,worker_token_id from sellerpilot_private.gateway_completion_receipts where job_id=$1",
      [child],
    )).rows[0];
    assert.notEqual(activeReceipt.claim_token, oldClaim);
    assert.equal(activeReceipt.worker_token_id, tokenId);
    assert.equal((await refresh(db, run.runId)).status, "succeeded");
    assert.equal((await checkpoint(db, "2024-08-01", "2024-08-01")).complete, true);

    for (const role of ["anon", "authenticated", "service_role"]) {
      assert.equal((await db.query(
        "select has_table_privilege($1,'sellerpilot_private.smartstore_history_retired_completion_receipts','select') allowed",
        [role],
      )).rows[0].allowed, false);
    }
    await assert.rejects(
      db.query("delete from sellerpilot_private.smartstore_history_retired_completion_receipts where job_id=$1", [child]),
      /SMARTSTORE_HISTORY_RETIRED_RECEIPT_IMMUTABLE/,
    );
  } finally {
    await db.close();
  }
...
test("receipt retirement rejects drift and rolls back without losing the failed attempt", async () => {
  const db = await fixture("2024-08-03 00:05:00+09");
  try {
    await db.exec(retryReceiptProposalSql);
    const run = await start(db,"2024-08-01","2024-08-01");
    ...
    await recordSucceededPage(db,product.id,true);
    const child = await insertContinuation(db,product.id,{status:"failed",attemptCount:1});
    const oldClaim = "00000000-0000-4000-9000-000000000097";
    await db.query("insert into sellerpilot_private.gateway_completion_receipts values($1,$2,$3)",[child,oldClaim,tokenId]);
    await assert.rejects(db.query(`update sellerpilot_private.channel_gateway_jobs set
      status='queued',claim_token=null,worker_token_id=null,lease_expires_at=null,
      completed_at=null,error_message=null,request_payload=request_payload||'{"forged":true}'::jsonb
      where id=$1`,[child]),/SMARTSTORE_HISTORY_RETRY_TRANSITION_INVALID/);
    assert.equal((await db.query("select status from sellerpilot_private.channel_gateway_jobs where id=$1",[child])).rows[0].status,"failed");
    assert.equal((await db.query("select claim_token from sellerpilot_private.gateway_completion_receipts where job_id=$1",[child])).rows[0].claim_token,oldClaim);
    assert.equal((await db.query("select count(*)::int count from sellerpilot_private.smartstore_history_retired_completion_receipts")).rows[0].count,0);
    await db.query("update sellerpilot_private.channel_credentials set status='revoked' where id=$1",[credential]);
    await assert.rejects(db.query(`update sellerpilot_private.channel_gateway_jobs set
      status='queued',claim_token=null,worker_token_id=null,lease_expires_at=null,
      completed_at=null,error_message=null where id=$1`,[child]),/SMARTSTORE_HISTORY_RETRY_SCOPE_INVALID/);
    assert.equal((await db.query("select claim_token from sellerpilot_private.gateway_completion_receipts where job_id=$1",[child])).rows[0].claim_token,oldClaim);
