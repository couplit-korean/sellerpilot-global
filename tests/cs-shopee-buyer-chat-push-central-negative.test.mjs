...
function authorizationFor(rawBody) {
  const bytes = typeof rawBody === "string" ? Buffer.from(rawBody) : Buffer.from(rawBody);
  return createHmac("sha256", partnerKey).update(`${callbackUrl}|`).update(bytes).digest("hex");
}

function signedRequest(rawBody, authorization = authorizationFor(rawBody)) {
...
function streamedRequest(chunks, contentLength) {
  ...
  const headers = { authorization: "0".repeat(64), "content-type": "application/json" };
  if (contentLength !== undefined) headers["content-length"] = String(contentLength);
  return {
    request: new Request(callbackUrl, { method: "POST", headers, body, duplex: "half" }),
...
for (const mode of ["bom_changed_bytes", "missing_direction", "seller_origin"]) {
 test(`central negative ${mode}`, async () => {
  const {db,POST}=await fixture();
  try {
   let raw=rawPush();
   let auth=createHmac("sha256",partnerKey).update(`${callbackUrl}|${raw}`).digest("hex");
   let body;
   if(mode==="bom_changed_bytes") body=Buffer.concat([Buffer.from([0xef,0xbb,0xbf]),Buffer.from(raw)]);
   else {
    const obj=JSON.parse(raw);
    if(mode==="missing_direction") {delete obj.data.content.from_shop_id;delete obj.data.content.to_shop_id;}
    else {obj.data.content.from_shop_id=Number(shopId);delete obj.data.content.to_shop_id;}
    body=JSON.stringify(obj);auth=createHmac("sha256",partnerKey).update(`${callbackUrl}|${body}`).digest("hex");
   }
   const response=await POST(signedRequest(body,auth));
   const n=(await db.query("select count(*)::int n from sellerpilot_private.cs_shopee_buyer_chat_messages")).rows[0].n;
   assert.ok(response.status>=400 && n===0, `status=${response.status}, canonical buyer rows=${n}`);
  } finally {await db.close();}
 });
}

test("actual route signs the received bytes and rejects invalid UTF-8 before DB lookup", async () => {
  const { db, POST, rpcCalls } = await fixture();
  try {
    const raw = rawPush();
    const changedBytes = Buffer.concat([Buffer.from(raw), Buffer.from(" ")]);
    assert.equal((await POST(signedRequest(changedBytes, authorizationFor(raw)))).status, 401);
    const invalidUtf8 = Buffer.concat([Buffer.from(raw.slice(0, -1)),
      Buffer.from([0xc3, 0x28]), Buffer.from("}")]);
    assert.equal((await POST(signedRequest(invalidUtf8))).status, 400);
    assert.equal(rpcCalls.length, 1);
    assert.equal((await db.query(`select count(*)::int n
      from sellerpilot_private.cs_shopee_buyer_chat_messages`)).rows[0].n, 0);
  } finally { await db.close(); }
});

for (const declaredLength of [undefined, 1]) {
  test(`actual route stops an oversize stream with ${declaredLength === undefined
    ? "no" : "false"} Content-Length before DB lookup`, async () => {
    const { db, POST, rpcCalls } = await fixture();
    try {
      const streamed = streamedRequest([
        new Uint8Array(65_536), new Uint8Array([1]), new Uint8Array([2]),
      ], declaredLength);
      assert.equal((await POST(streamed.request)).status, 400);
...
test("actual route rejects cross-shop direction and records explicit seller origin as seller", async () => {
  const { db, POST } = await fixture();
  try {
    const crossShop = JSON.parse(rawPush());
    crossShop.data.content.to_shop_id = Number(shopId) + 1;
    assert.equal((await POST(signedRequest(JSON.stringify(crossShop)))).status, 400);
    assert.equal((await db.query(`select count(*)::int n
      from sellerpilot_private.cs_shopee_buyer_chat_messages`)).rows[0].n, 0);

    const seller = JSON.parse(rawPush());
    seller.data.content.from_shop_id = Number(shopId);
    seller.data.content.to_shop_id = 0;
    seller.data.content.content.text = "Seller reply";
    assert.equal((await POST(signedRequest(JSON.stringify(seller)))).status, 204);
    await db.query("select set_config('request.jwt.claim.sub',$1,false)", [admin]);
    const read = (await db.query(`select public.sellerpilot_read_cs_shopee_buyer_chat_v1(
      $1,$2,'709122092476686867',null,100
    ) result`, [credentialId, shopId])).rows[0].result;
    const ledger = shopeeBuyerChatReadSchema.parse(read);
    assert.equal(ledger.shops[0].messages[0].senderRole, "seller");
    assert.equal(ledger.shops[0].messages[0].body, "Seller reply");
  } finally { await db.close(); }
});
