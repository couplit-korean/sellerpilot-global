...
test("product research identifies the Vercel OIDC server path without erasing legacy CLI compatibility", async () => {
  ...
  assert.match(page, /disabled=\{researchingProduct \? false : recoveringProductResearch \|\| intake\.researchInput\.trim\(\)\.length < 2 \|\| !mainPhoto \|\| photoSelectionsProcessing \|\| running \|\| !studioWorkerAvailable\}/);
