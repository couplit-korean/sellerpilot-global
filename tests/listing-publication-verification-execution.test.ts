...
test("Coupang post-price verifier binds the original CREATE content and exact live price without a write", async () => {
  const exactJobId = "71000000-0000-4000-8000-000000000061";
  const exactSourceJobId = "25adf712-1e9a-432b-8b0d-09cf35a826c5";
  const exactSellerProductId = "16375780938";
  const exactVendorItemId = "96027942778";
  const exactProductId = "9725220700";
  const exactItemId = "29102903416";
  const exactFingerprint = "6".repeat(64);
  const exactBody = {
    sellerProductName: "파스퇴르 순우유맛으로 채우는 간식 시간",
    displayProductName: "파스퇴르 순우유맛으로 채우는 간식 시간",
    items: [{
      externalVendorSku: "AUTO-780720401E2D4E4EA45F",
      salePrice: 3190,
      maximumBuyCount: 1,
      itemName: "파스퇴르 순우유맛으로 채우는 간식 시간",
      contents: coupangContents,
    }],
  };
  const exactSellerResponse = {
    code: "SUCCESS",
    data: {
      sellerProductId: exactSellerProductId,
      productId: exactProductId,
      sellerProductName: exactBody.sellerProductName,
      displayProductName: exactBody.displayProductName,
      requested: false,
      statusName: "승인완료",
      items: [{
  ...
  const exactSourceArguments = publicationArguments({
    publicationIntent: "live",
    publicationStateContract: "verified_remote_state_v1",
    publicationExpectedLocale: "ko-KR",
    publicationExpectedFingerprint: exactFingerprint,
    publicationExpectedImageCount: 8,
    body: exactBody,
  });
  const publicationAssetBinding = listingPublicationProviderAssetEvidence({
    channel: "coupang",
    remoteId: exactSellerProductId,
    sourceArguments: exactSourceArguments,
    providerArguments: exactSourceArguments,
  });
  assert.ok(publicationAssetBinding);
  const exactSource = {
    contract: "listing_publication_verification_source_v1" as const,
    verificationJobId: exactJobId,
    sourceJobId: exactSourceJobId,
    sourceOperation: "listing.create" as const,
    sourceArguments: exactSourceArguments,
    sourceResponsePayload: {
      ok: false,
      ...
      remoteState: {
        ...
        resources: {
          sellerProductId: exactSellerProductId,
          vendorItemIds: [exactVendorItemId],
        },
      },
    },
    sourceFingerprint: exactFingerprint,
    expectedRemoteId: exactSellerProductId,
    expectedLocale: "ko-KR",
    expectedImageCount: 8 as const,
    market: "",
    targetId: "",
  ...
  const exactArguments = {
    sellerpilotReadOnly: true,
    publicationReviewSourceJobId: exactSourceJobId,
    remoteId: exactSellerProductId,
    market: "",
    targetId: "",
    publicationIntent: "live",
    ...
    publicationExpectedFingerprint: exactFingerprint,
    publicationExpectedImageCount: 8,
    sellerpilotPublicationSource: exactSource,
    sellerpilotCoupangPostPriceVerification: {
      contract: "coupang_exact_post_price_publication_v1",
      verifierJobId: exactJobId,
      priceRepairJobId: "36fcb808-a2f1-42b7-a6c9-264d884f25fb",
      priceRepairAttemptId: "05508966-7665-4873-a89b-89fda8ea8a25",
      sellerProductId: exactSellerProductId,
      vendorItemId: exactVendorItemId,
      productId: exactProductId,
      itemId: exactItemId,
      desiredPrice: 3190,
      currency: "KRW",
      priceRepairResponseSha256: "7".repeat(64),
    },
  };
  const originalFetch = globalThis.fetch;
  const calls: Array<{ method: string; url: string }> = [];
  globalThis.fetch = async (request, init) => {
    ...
    const method = String(init?.method ?? (request instanceof Request ? request.method : "GET"));
    calls.push({ method, url });
    if (url.includes(`/vendor-items/${exactVendorItemId}/inventories`)) {
      return Response.json({
        code: "SUCCESS",
        data: {
          sellerItemId: exactVendorItemId,
          onSale: true,
          amountInStock: 1,
          salePrice: 3190,
        },
    ...
    return Response.json(exactSellerResponse);
  };
  try {
    ...
    assert.equal(result.remoteState?.evidence.observedPrice, 3190);
    assert.equal(result.remoteState?.evidence.observedStock, 1);
    assert.equal(result.remoteState?.evidence.currency, "KRW");
    assert.equal(result.remoteState?.evidence.providerMutationPerformed, false);
    assert.equal(result.remoteState?.resources.productId, exactProductId);
    assert.equal(result.remoteState?.resources.itemId, exactItemId);
    assert.equal(
      result.remoteState?.evidence.providerPublicUrl,
      `https://www.coupang.com/vp/products/${exactProductId}?vendorItemId=${exactVendorItemId}`,
    );
    ...
    for (const invalidReadback of [
      ...
      globalThis.fetch = async (request) => {
        const url = String(request instanceof Request ? request.url : request);
        if (url.includes(`/vendor-items/${exactVendorItemId}/inventories`)) {
          return Response.json({ code: "SUCCESS", data: invalidReadback.vendor });
        }
        return Response.json(invalidReadback.seller);
      };
      const invalidResult = await executeListingPublicationVerification({
        channel: "coupang",
        operation: "listing.publication.verify",
        payload: {
          vendor_id: "A00012345",
      ...
      assert.equal(invalidResult.remoteState, undefined);
      assert.equal(invalidResult.steps.at(-1)?.ok, false);
    }
      ...
      executeListingPublicationVerification({
        ...
        arguments: {
          ...
          sellerpilotPublicationSource: {
            ...exactSource,
            sourceJobId: "72000000-0000-4000-8000-000000000099",
          },
        },
        environment: "production",
      }),
      /COUPANG_POST_PRICE_PUBLICATION_CONTEXT_INVALID/,
    );
