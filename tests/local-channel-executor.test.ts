...
test("SmartStore category enqueue retains the existing Mac heartbeat gate after gaining a local claim tuple", async () => {
  ...
  assert.match(route, /if \(localExecutorAccess && !smartstoreCategoryRead && !smartstoreApprovedAiLocalCreate\s*&& !elevenstApprovedAiLocalCreate && !temuApprovedAiLocalCreate\)/);
...
test("the local executor has an exact read/write operation whitelist", () => {
  ...
  assert.equal(localChannelExecutorAccess("shopee", "shops.get"), "read");
  assert.equal(localChannelExecutorAccess("ebay", "shops.get"), null);
  assert.equal(localChannelExecutorAccess("shopee", "shops.update"), null);
