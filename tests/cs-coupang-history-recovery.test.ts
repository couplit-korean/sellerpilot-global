...
test("Coupang 30-day recovery creates five fixed windows and five non-overlapping scopes", () => {
  ...
  assert.equal(batch.expectedInitialJobs, 25);
  assert.equal(batch.requests.length, 25);
  ...
  for (const kinds of windows.values()) {
    ...
    assert.equal(kinds.filter((kind) => kind === "call-center").length, 1);
  }
});

test("Coupang seven-day recovery remains one complete five-scope unit", () => {
  const batch = coupangHistoryRecoveryBatch("2024-02-29", 7);
  assert.equal(batch.fromDate, "2024-02-23");
  assert.equal(batch.expectedInitialJobs, 5);
  assert.equal(batch.requests.length, 5);
});
...
test("Coupang interrupted or failed recovery replays the identical end date", () => {
  for (const run of [
    successfulRun({ status: "running", queuedJobs: 1, succeededJobs: 24, completedAt: null }),
    successfulRun({ status: "failed", succeededJobs: 24, failedJobs: 1, completedAt: null }),
  ]) {
...
test("Coupang recovery rejects inconsistent job ledgers", () => {
  ...
  assert.throws(
    () => coupangHistoryRecoveryCheckpoint(successfulRun({ expectedInitialJobs: 30 })),
    /COUPANG_HISTORY_RUN_COUNTS_INVALID/u,
  );
  assert.throws(
