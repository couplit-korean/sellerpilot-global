...
const { withShopeeHistoryContinuation } = await import(
  pathToFileURL(resolve(sourceRoot, "lib/channels/cs/shopee/history-event-evidence.ts")).href
);
...
async function claim(db, runId, shopId, claimToken) {
  const selected = (await db.query(`select id,request_payload from sellerpilot_private.channel_gateway_jobs
    where status='queued' and request_payload#>>'{arguments,sellerpilotShopeeHistoryRunId}'=$1
      ...
      and request_payload#>>'{arguments,shopId}'=$2 order by created_at,id limit 1`, [runId, shopId])).rows[0];
  assert.ok(selected);
...
function reviewResult(shopId, commentId, continuation = null) {
  return {
    ok: true, channel: "shopee", operation: "inquiries.list",
    steps: [{ name: "inquiries", ok: true, status: 200, data: {
      sellerpilotProviderContext: { shopId }, response: { item_comment_list: [{
...
test("canonical common completion chain feeds owner GET with per-country partial, authorization and resume truth", async () => {
  ...
  try {
    ...
    const continuation = withShopeeHistoryContinuation(sg.request.arguments, {
      ...sg.request.arguments, cursor: "opaque-next", sellerpilotPaginationDepth: 1,
      sellerpilotPaginationEpoch: 0, sellerpilotPaginationTrail: ["a".repeat(64)],
    });
    const sgFirst = await runWorker(db, sg, async () => reviewResult(shops[0].id, 7301, continuation));
    ...
    const sgFinal = await runWorker(db, sgResume, async () => reviewResult(shops[0].id, 7302));
