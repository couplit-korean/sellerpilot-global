...
function productQnaOverflow(): ChannelOperationResult {
  return {
    ok: false,
    channel: "elevenst",
    operation: "inquiries.list",
    ...
    safeMessage: "11st Product Q&A row limit was exceeded.",
  };
}
    ...
    if (name === "sellerpilot_service_elevenst_cs_account_identity_v1") {
      return {
        data: {
          ...
          version: 1,
          verifiedAt: "2026-09-08T07:59:00.000Z",
        },
        error: null,
      };
    }
...
test("11st runOne records Product Q&A overflow as incomplete 501-row evidence", async () => {
  const calls: RpcCall[] = [];
  const response = await runOneServerlessCsGatewayJob({
    staticEgressChannels: ["elevenst"],
    rpc: rpcFixture(calls),
    executeProvider: async () => productQnaOverflow(),
  }, deriveServerlessCsGatewayCredentials(CRON_SECRET).gatewayTokenHash);
  ...
  assert.equal(observation.providerRows, 501);
  assert.equal(observation.parseIncomplete, true);
  assert.equal(observation.parserMarker, "sellerpilot-elevenst-product-qna-parser/1");
});
