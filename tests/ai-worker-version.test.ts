...
test("new AI claims reject worker 1.46 and accept worker 1.47", () => {
  assert.equal(minimumResultUploadWorkerVersion, "sellerpilot-cli-worker/1.47");
  ...
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.47"), true);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.47.0"), true);
