...
test("new AI claims reject worker 1.45 and accept worker 1.46", () => {
  assert.equal(minimumResultUploadWorkerVersion, "sellerpilot-cli-worker/1.46");
  ...
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.46"), true);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.46.0"), true);
