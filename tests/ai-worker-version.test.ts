...
test("new AI claims reject worker 1.57 and accept worker 1.58 or newer", () => {
  assert.equal(minimumResultUploadWorkerVersion, "sellerpilot-cli-worker/1.58");
  ...
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.58"), true);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.58.0"), true);
