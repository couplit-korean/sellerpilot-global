...
test("new AI claims reject worker 1.48 and accept worker 1.49", () => {
  assert.equal(minimumResultUploadWorkerVersion, "sellerpilot-cli-worker/1.49");
  ...
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.49"), true);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.49.0"), true);
