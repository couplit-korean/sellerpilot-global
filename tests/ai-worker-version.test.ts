...
test("new AI claims reject worker 1.58 and accept worker 1.59 or newer", () => {
  assert.equal(minimumResultUploadWorkerVersion, "sellerpilot-cli-worker/1.59");
  ...
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.56"), false);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.56.0"), false);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.57"), false);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.57.0"), false);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.58"), false);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.58.0"), false);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.59"), true);
  assert.equal(supportsLiveResultUploadAuthorization("sellerpilot-cli-worker/1.59.0"), true);
