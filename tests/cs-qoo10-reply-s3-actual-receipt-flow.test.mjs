...
const integratedCoupangProductMigration =
  "supabase/migrations/20260909135332_cs_coupang_product_reply_readback.sql";
const coupangProductMigration = await access(path.join(repoRoot, integratedCoupangProductMigration))
  .then(() => integratedCoupangProductMigration)
  .catch(() => "docs/cs-review-20260909/coupang/20260909130500_cs_coupang_product_reply_readback_proposal.sql");
...
const [{ runOneServerlessCsGatewayJob }, { POST: completeWorkerPost }, { completeCsWorker, completeCsWorkerRetry }, { executeCsOperation }] = await Promise.all([
  ...
  import(pathToFileURL(path.join(repoRoot, "lib/cs/operations/worker-completion.ts")).href),
  import(pathToFileURL(path.join(repoRoot, "lib/cs/operations/execute.ts")).href),
]);
...
const sql = Object.fromEntries(await Promise.all(Object.entries({
  ...
  delivery: "supabase/migrations/20260831033000_add_cs_message_delivery_ledger.sql",
  observation: "supabase/migrations/20260907232000_add_cs_reply_remote_observation.sql",
  ...
  coupangProduct: coupangProductMigration,
  coupangTicketIdentity: "supabase/migrations/20260909160547_cs_coupang_vendor_ticket_identity.sql",
  ...
  requery: "supabase/migrations/20260909153338_cs_qoo10_reply_s3_requery.sql",
}).map(async ([key, relative]) => [
  key,
  await readFile(path.join(repoRoot, relative), "utf8"),
])));
const deliveryTriggerSql = sql.delivery.slice(
  sql.delivery.indexOf("create or replace function sellerpilot_private.sync_inquiry_reply_delivery_ledger()"),
  sql.delivery.indexOf("-- Backfill reply jobs created before this trigger."),
);
assert.ok(deliveryTriggerSql.startsWith("create or replace function"));
assert.match(deliveryTriggerSql,/create trigger sync_inquiry_reply_delivery_ledger/u);
const observationCoreSql = `${sql.observation.slice(
  0,
  sql.observation.indexOf("create or replace function public.sellerpilot_get_inquiry_reply_delivery"),
)}\ncommit;`;
assert.match(observationCoreSql,/sellerpilot_service_observe_inquiry_replies_v1/u);
...
const ids = {
  ...
  coupangProductTicket: "61000000-0000-4000-8000-000000000016",
  coupangProductInbound: "61000000-0000-4000-8000-000000000017",
  coupangProductReplyJob: "61000000-0000-4000-8000-000000000018",
  coupangProductReplyClaim: "61000000-0000-4000-8000-000000000019",
  coupangProductReadClaim: "61000000-0000-4000-8000-000000000020",
  coupangProductNewInbound: "61000000-0000-4000-8000-000000000021",
};
const qooSellerKey = "a".repeat(64);
const coupangSellerKey = "b".repeat(64);
const replyBody = "synthetic product reply fixture";
const replyFingerprint = createHash("sha256").update(replyBody).digest("hex");
const qooInboundKey = "qoo10:inbound:actual-receipt-fixture";
const coupangInboundKey = `coupang:${"d".repeat(64)}`;
const coupangProductInboundKey = `coupang:${"8".repeat(64)}`;
const coupangProductNewInboundKey = `coupang:${"9".repeat(64)}`;
...
function coupangProductReplyRequest() {
  return {
    arguments: { kind: "product", inquiryId: "3201", reply: replyBody },
    sellerpilotTicketId: ids.coupangProductTicket,
    sellerpilotInboundKey: coupangProductInboundKey,
    sellerpilotReplyFingerprint: replyFingerprint,
  };
}

function coupangProductReplyResponse() {
  return {
    ok: true,
    channel: "coupang",
    operation: "inquiries.reply",
    steps: [{
      name: "replyOnlineInquiry",
      ok: true,
      status: 200,
      data: { sellerpilotReplyAcceptance: {
        contract: "sellerpilot-reply-acceptance/1",
        level: "provider_accepted",
        channel: "coupang",
        kind: "product",
...
function coupangProductReadbackResult() {
  return {
    ok:true,channel:"coupang",operation:"inquiries.list",
    steps:[{name:"inquiries",ok:true,status:200,data:{
      sellerpilotInquiryKind:"product",
      data:{content:[{inquiryId:9999,content:"unrelated buyer question",
        inquiryAt:"2025-01-15T08:00:00+09:00",commentDtoList:[{
          inquiryCommentId:9998,inquiryId:9999,content:"unrelated seller answer",
          inquiryCommentAt:new Date().toISOString(),
        }]},{inquiryId:3201,content:"synthetic buyer question",
        inquiryAt:"2025-01-15T08:00:00+09:00",commentDtoList:[{
          inquiryCommentId:9201,inquiryId:3201,content:replyBody,
          inquiryCommentAt:new Date().toISOString(),
        }]}],pagination:{currentPage:1,totalPages:1,totalElements:1,countPerPage:50}},
    }}],safeMessage:"synthetic Coupang product readback",
  };
}
...
async function createFixture() {
  ...
  await db.exec(`
    ...
    create table sellerpilot_private.support_inbound_messages (
      ...
      body text not null default 'synthetic inbound',
  ...
  await db.exec(deliveryTriggerSql);
             ...
             or (token.scope='serverless_cs' and (
               exists(select 1 from sellerpilot_private.channel_gateway_jobs job
                 where job.id=p_job_id and job.worker_token_id=token.id
                   and job.claim_token=p_claim_token and job.channel in('qoo10','coupang')
               ...
               or exists(select 1 from sellerpilot_private.gateway_completion_receipts receipt
                ...
                where receipt.job_id=p_job_id and receipt.worker_token_id=token.id
                  and receipt.claim_token=p_claim_token and job.channel in('qoo10','coupang')
                  and job.operation in('inquiries.list','inquiries.reply'))
             ))
           )
      )
  ...
  `);
  await db.exec(observationCoreSql);
  await db.exec(`
  ...
  await db.exec(sql.coupangProduct);
  await db.exec(sql.common);
  await db.exec(sql.compat);
  await db.exec(sql.requery);
  return db;
}

async function applyCoupangTicketIdentity(db) {
  await db.exec(`
    ...
    create unique index channel_credentials_one_active_lazada_pending_owner_idx
      ...
      where status='pending' and channel='lazada';
    ...
    returns jsonb language sql stable security definer set search_path='' as $$
      select jsonb_build_object(
        'id',ticket.id,'channel_key',ticket.channel_key,
        'external_ticket_id',ticket.external_ticket_id,
        'latest_inbound_key',ticket.latest_inbound_key
      ) from sellerpilot_private.support_tickets ticket where ticket.id=p_id
    $$;
  `);
  await db.exec(sql.coupangTicketIdentity);
}
...
async function insertQooReply(db) {
  ...
  await db.query(`insert into sellerpilot_private.support_inbound_messages(
    id,ticket_id,owner_id,channel_key,inbound_key,sender_role,received_at
  ) values($1,$2,$3,'qoo10',$4,'customer','2026-09-07T15:30:00Z')`,
...
async function insertCoupangProductReply(db) {
  await db.query(`insert into sellerpilot_private.support_tickets(
    id,owner_id,source_credential_id,channel_key,ticket_kind,external_ticket_id,
    latest_inbound_key,seller_account_key,demo,status
  ) values($1,$2,$3,'coupang','conversation','product:3201',$4,$5,false,'open')`, [
    ids.coupangProductTicket, ids.owner, ids.coupangCredential,
    coupangProductInboundKey, coupangSellerKey,
  ]);
  await db.query(`insert into sellerpilot_private.support_inbound_messages(
    id,ticket_id,owner_id,channel_key,inbound_key,sender_role,received_at
  ) values($1,$2,$3,'coupang',$4,'customer','2025-01-15T08:00:00+09:00')`, [
    ids.coupangProductInbound, ids.coupangProductTicket, ids.owner, coupangProductInboundKey,
  ]);
  await db.query(`insert into sellerpilot_private.channel_gateway_jobs(
    id,credential_id,channel,operation,environment,request_payload,status,
    created_by,seller_account_key,worker_token_id,claim_token,lease_expires_at,
    provider_mutation_started_at,started_at
  ) values($1,$2,'coupang','inquiries.reply','production',$3::jsonb,'running',
    $4,$5,$6,$7,clock_timestamp()+interval '10 minutes',clock_timestamp(),clock_timestamp())`, [
    ids.coupangProductReplyJob, ids.coupangCredential,
    JSON.stringify(coupangProductReplyRequest()), ids.owner, coupangSellerKey,
    ids.gatewayWorker, ids.coupangProductReplyClaim,
  ]);
}

async function completeCoupangProductReply(db) {
  const completion = {
    tokenHash: gatewayTokenHash,
    jobId: ids.coupangProductReplyJob,
    claimToken: ids.coupangProductReplyClaim,
    status: "succeeded",
    response: coupangProductReplyResponse(),
  };
...
async function coupangProductReadbackChild(db) {
  return (await db.query(`select job.*
    from sellerpilot_private.coupang_product_reply_readback_links link
    join sellerpilot_private.channel_gateway_jobs job on job.id=link.child_job_id
    where link.source_job_id=$1`,[ids.coupangProductReplyJob])).rows[0];
}

async function executeCoupangProductPage(child,responsePayload){
  const originalFetch=globalThis.fetch;
  globalThis.fetch=async()=>Response.json(responsePayload);
  try{
    return await executeCsOperation({
...
async function dbRpc(db, name, args) {
  const calls = {
    ...
    sellerpilot_service_qoo10_reply_s3_requery_context_v1: {
      sql: "select public.sellerpilot_service_qoo10_reply_s3_requery_context_v1($1,$2,$3) value",
      values: [args.p_token_hash,args.p_job_id,args.p_claim_token],
    },
    sellerpilot_service_apply_qoo10_reply_s3_requery_v1: {
      sql: `select public.sellerpilot_service_apply_qoo10_reply_s3_requery_v1(
        $1,$2,$3,$4,$5,$6,$7::jsonb
      ) value`,
      values: [args.p_token_hash,args.p_job_id,args.p_claim_token,args.p_state,
        args.p_reason,args.p_matching_rows,args.p_descriptor],
    },
    sellerpilot_service_requeue_coupang_product_reply_readback_v1: {
      sql: `select public.sellerpilot_service_requeue_coupang_product_reply_readback_v1(
        $1,$2,$3,$4::jsonb,$5,$6,$7,$8,$9
      ) value`,
      values: [args.p_token_hash,args.p_job_id,args.p_claim_token,
        JSON.stringify(args.p_retry_arguments),args.p_retry_count,args.p_retry_after_seconds,
        args.p_deferred_count,args.p_replay_count,args.p_provider_status],
    },
    sellerpilot_service_observe_inquiry_replies_v1: {
      sql:`select public.sellerpilot_service_observe_inquiry_replies_v1($1,$2,$3::jsonb) value`,
      values:[args.p_credential_id,args.p_channel,JSON.stringify(args.p_observations)],
    },
  };
  if(name==="sellerpilot_service_record_cs_history_page_v1")return {data:{
    contract:"cs_history_coverage_v1",status:"completed",jobId:args.p_job_id,
  },error:null};
...
function coupangProductRetry(child) {
  return {
    reason: "retryable_read_failure",
    arguments: {
      ...child.request_payload.arguments,
      sellerpilotCoupangReadbackAttempt: 1,
    },
    retryCount: 1,
...
function coupangProductRetryResult(child) {
  return {
    ok: false,
    channel: "coupang",
    operation: "inquiries.list",
    steps: [{ name: "inquiries", ok: false, status: 503, data: {} }],
    retryContinuation: coupangProductRetry(child),
  };
}

function coupangProductServerlessRpc(db, child) {
  let claimed=false;
  return async (name,args={})=>{
    if(name==="sellerpilot_claim_serverless_gateway_job"){
      if(claimed)return {data:null,error:null};
      claimed=true;
      await claimChild(db,child.id,ids.serverlessWorker,ids.coupangProductReadClaim);
      return {data:{
        id:child.id,claim_token:ids.coupangProductReadClaim,credential_id:ids.coupangCredential,
        channel:"coupang",operation:"inquiries.list",environment:"production",
        request:child.request_payload,credential:{vendor_id:"synthetic"},attempt_count:1,
      },error:null};
    }
...
test("actual canonical completion creates one product readback child and replays without duplication",async()=>{
  const db=await createFixture();
  try{
    await insertCoupangProductReply(db);
    const {first,replay}=await completeCoupangProductReply(db);
    assert.equal(first.status,"completed");
    assert.equal(replay.status,"completed");
    assert.equal(replay.replayed,true);
    const delivery=(await db.query(`select status,verification_status,reconciliation_reason
      from sellerpilot_private.support_reply_deliveries where gateway_job_id=$1`,
      [ids.coupangProductReplyJob])).rows[0];
    assert.deepEqual(delivery,{status:"succeeded",verification_status:"provider_accepted",reconciliation_reason:null});
    const child=await coupangProductReadbackChild(db);
    assert.ok(child);
    assert.equal(child.request_payload.arguments.kind,"product-reply-readback");
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.coupang_product_reply_readback_links
      where source_job_id=$1`,[ids.coupangProductReplyJob])).rows[0].count,1);
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.channel_gateway_jobs where operation='inquiries.reply'`)).rows[0].count,1);
  }finally{await db.close();}
});

test("account-scoped native product identity completes the actual readback path",async()=>{
  const db=await createFixture();
  try{
    await insertCoupangProductReply(db);
    await applyCoupangTicketIdentity(db);
    const ticket=(await db.query(`select external_ticket_id,provider_external_ticket_id,
      latest_inbound_key from sellerpilot_private.support_tickets where id=$1`,
      [ids.coupangProductTicket])).rows[0];
    assert.match(ticket.external_ticket_id,/^coupang:account:[a-f0-9]{64}$/u);
    assert.equal(ticket.provider_external_ticket_id,"product:3201");
    assert.match(ticket.latest_inbound_key,/^coupang:[a-f0-9]{64}$/u);
    await db.query(`update sellerpilot_private.channel_gateway_jobs set request_payload=
      jsonb_set(request_payload,'{sellerpilotInboundKey}',to_jsonb($2::text),true)
      where id=$1`,[ids.coupangProductReplyJob,ticket.latest_inbound_key]);
    ...
    const child=await coupangProductReadbackChild(db);
    assert.ok(child);
    assert.equal(child.request_payload.arguments.expectedInboundKey,ticket.latest_inbound_key);
    await claimChild(db,child.id,ids.gatewayWorker,ids.coupangProductReadClaim);
    const readbackResult=await executeCoupangProductPage(child,{
      code:"200",data:coupangProductReadbackResult().steps[0].data.data,
    });
    let lastRpcError=null;
    const response=await completeCsWorker({
      serviceClient:{rpc:async(name,args)=>{
        const result=await dbRpc(db,name,args);
        if(result.error)lastRpcError={name,...result.error};
        return result;
      }},tokenHash:gatewayTokenHash,
      job:{...child,request:child.request_payload,status:"running",claim_token:ids.coupangProductReadClaim,
        normalization_timestamp:"2026-09-09T04:10:00.000Z"},
      completion:{jobId:child.id,claimToken:ids.coupangProductReadClaim,status:"succeeded",
        result:readbackResult},
    });
    assert.equal(response.status,200,JSON.stringify(lastRpcError));
...
test("actual page continuation keeps lineage through two SQL retries and terminal reconciliation",async()=>{
  const db=await createFixture();
  try{
    await insertCoupangProductReply(db);
    await completeCoupangProductReply(db);
    const pageOne=await coupangProductReadbackChild(db);
    await claimChild(db,pageOne.id,ids.gatewayWorker,ids.coupangProductReadClaim);
    const pageOneResult=await executeCoupangProductPage(pageOne,{code:"200",data:{content:[
      {inquiryId:9999,content:"unrelated buyer question",inquiryAt:"2025-01-15T08:00:00+09:00",commentDtoList:[]},
      {inquiryId:3201,content:"synthetic buyer question",inquiryAt:"2025-01-15T08:00:00+09:00",commentDtoList:[]},
    ],pagination:{currentPage:1,totalPages:2,totalElements:2,countPerPage:50}}});
    assert.equal(pageOneResult.continuation.arguments.query.pageNum,2);
    const firstCompletion=await callCompletion(db,{
      tokenHash:gatewayTokenHash,jobId:pageOne.id,claimToken:ids.coupangProductReadClaim,
      status:"succeeded",response:pageOneResult,
    });
    const pageTwo=(await db.query(`select * from sellerpilot_private.channel_gateway_jobs
      where request_payload->>'continuationOf'=$1`,[pageOne.id])).rows[0];
    assert.equal(firstCompletion.continuationJobId,pageTwo.id);
    assert.equal(pageTwo.request_payload.arguments.query.pageNum,2);
    assert.equal(pageTwo.seller_account_key,coupangSellerKey);
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.coupang_product_reply_readback_links
      where source_job_id=$1`,[ids.coupangProductReplyJob])).rows[0].count,2);

    await claimChild(db,pageTwo.id,ids.gatewayWorker,ids.coupangProductReadClaim);
    const pageTwoResult=await executeCoupangProductPage(pageTwo,{code:"200",data:{content:[],
      pagination:{currentPage:2,totalPages:2,totalElements:2,countPerPage:50}}});
    assert.equal(pageTwoResult.retryContinuation.retryCount,1);
    assert.equal((await completeCsWorkerRetry({rpc:(name,args)=>dbRpc(db,name,args)},gatewayTokenHash,{
      jobId:pageTwo.id,claimToken:ids.coupangProductReadClaim,status:"failed",error:"pending",
      result:pageTwoResult,retryContinuation:pageTwoResult.retryContinuation,
    })).status,202);

    let retryJob=(await db.query("select * from sellerpilot_private.channel_gateway_jobs where id=$1",[pageTwo.id])).rows[0];
    assert.equal(retryJob.request_payload.arguments.query.pageNum,1);
    assert.equal(retryJob.request_payload.arguments.sellerpilotCoupangReadbackAttempt,1);
    await claimChild(db,retryJob.id,ids.gatewayWorker,ids.coupangProductReadClaim);
    const retryOneResult=await executeCoupangProductPage(retryJob,{code:"200",data:{content:[],
      pagination:{currentPage:1,totalPages:1,totalElements:0,countPerPage:50}}});
    assert.equal((await completeCsWorkerRetry({rpc:(name,args)=>dbRpc(db,name,args)},gatewayTokenHash,{
      jobId:retryJob.id,claimToken:ids.coupangProductReadClaim,status:"failed",error:"pending",
      result:retryOneResult,retryContinuation:retryOneResult.retryContinuation,
    })).status,202);

    retryJob=(await db.query("select * from sellerpilot_private.channel_gateway_jobs where id=$1",[pageTwo.id])).rows[0];
    assert.equal(retryJob.request_payload.arguments.sellerpilotCoupangReadbackAttempt,2);
    await claimChild(db,retryJob.id,ids.gatewayWorker,ids.coupangProductReadClaim);
    const terminalResult=await executeCoupangProductPage(retryJob,{code:"200",data:{content:[],
      pagination:{currentPage:1,totalPages:1,totalElements:0,countPerPage:50}}});
    assert.equal(terminalResult.retryContinuation,undefined);
    assert.equal(terminalResult.steps.at(-1).data.sellerpilotReconciliationRequired,true);
    const terminalResponse=await completeCsWorker({
      serviceClient:{rpc:(name,args)=>dbRpc(db,name,args)},tokenHash:gatewayTokenHash,
      job:{...retryJob,status:"running",claim_token:ids.coupangProductReadClaim,
        normalization_timestamp:new Date().toISOString()},
      completion:{jobId:retryJob.id,claimToken:ids.coupangProductReadClaim,
        status:"reconciliation_required",result:terminalResult,error:"COUPANG_PRODUCT_REPLY_READBACK_EXHAUSTED"},
    });
    assert.equal(terminalResponse.status,200);
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.coupang_product_reply_readback_retries
      where child_job_id=$1`,[pageTwo.id])).rows[0].count,2);
    ...
    assert.equal((await db.query(`select count(*)::integer count from sellerpilot_private.channel_gateway_jobs
      where operation='inquiries.reply'`)).rows[0].count,1);
  }finally{await db.close();}
});

test("actual product readback completion reaches the canonical reply observation ledger",async()=>{
  const db=await createFixture();
  try{
    await insertCoupangProductReply(db);
    await completeCoupangProductReply(db);
    ...
    await db.query(`insert into sellerpilot_private.support_inbound_messages(
      id,ticket_id,owner_id,channel_key,inbound_key,sender_role,received_at
    ) values($1,$2,$3,'coupang',$4,'customer',clock_timestamp())`,[
      ids.coupangProductNewInbound,ids.coupangProductTicket,ids.owner,coupangProductNewInboundKey,
    ]);
    await db.query(`update sellerpilot_private.support_tickets set
      latest_inbound_key=$2,status='open',provider_status='waiting' where id=$1`,[
      ids.coupangProductTicket,coupangProductNewInboundKey,
    ]);
    await claimChild(db,child.id,ids.gatewayWorker,ids.coupangProductReadClaim);
    const response=await completeCsWorker({
      serviceClient:{rpc:(name,args)=>dbRpc(db,name,args)},tokenHash:gatewayTokenHash,
      job:{...child,status:"running",claim_token:ids.coupangProductReadClaim,
        normalization_timestamp:"2026-09-09T04:10:00.000Z"},
      completion:{jobId:child.id,claimToken:ids.coupangProductReadClaim,status:"succeeded",
        result:coupangProductReadbackResult()},
    });
    assert.equal(response.status,200);
    const delivery=(await db.query(`select status,verification_status,verification_contract,
      provider_message_id from sellerpilot_private.support_reply_deliveries
      where gateway_job_id=$1`,[ids.coupangProductReplyJob])).rows[0];
    assert.deepEqual({...delivery,provider_message_id:undefined},{
      status:"succeeded",verification_status:"remote_observed",
      verification_contract:"sellerpilot-reply-observation/1",provider_message_id:undefined,
    });
    assert.match(delivery.provider_message_id,/^coupang:answer-observation:[a-f0-9]{64}$/u);
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.support_inbound_messages where ticket_id=$1 and sender_role='seller'`,
      [ids.coupangProductTicket])).rows[0].count,1);
    assert.deepEqual((await db.query(`select latest_inbound_key,status
      from sellerpilot_private.support_tickets where id=$1`,[ids.coupangProductTicket])).rows[0],{
      latest_inbound_key:coupangProductNewInboundKey,status:"open",
    });
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.support_tickets where external_ticket_id='product:9999'`)).rows[0].count,0);
  }finally{await db.close();}
});

test("actual serverless product completion is evidence-only and preserves a newer buyer generation",async()=>{
  const db=await createFixture();
  try{
    await insertCoupangProductReply(db);
    await completeCoupangProductReply(db);
    ...
    const executed=await executeCoupangProductPage(child,{code:"200",data:
      coupangProductReadbackResult().steps[0].data.data});
    const response=await runOneServerlessCsGatewayJob({
      staticEgressChannels:["coupang"],rpc:coupangProductServerlessRpc(db,child),
      executeProvider:async()=>executed,logError:()=>{},
    },serverlessTokenHash);
    assert.equal(response.status,200);
    assert.deepEqual((await db.query(`select latest_inbound_key,status
      from sellerpilot_private.support_tickets where id=$1`,[ids.coupangProductTicket])).rows[0],{
...
test("new inbound after provider mutation preserves accepted completion and reconciles without resend",async()=>{
  const db=await createFixture();
  try{
    await insertCoupangProductReply(db);
    await db.query(`insert into sellerpilot_private.support_inbound_messages(
      id,ticket_id,owner_id,channel_key,inbound_key,sender_role,received_at
    ) values($1,$2,$3,'coupang',$4,'customer','2026-09-09T13:00:00+09:00')`,[
      ids.coupangProductNewInbound,ids.coupangProductTicket,ids.owner,coupangProductNewInboundKey,
    ]);
    await db.query("update sellerpilot_private.support_tickets set latest_inbound_key=$2 where id=$1",[
      ids.coupangProductTicket,coupangProductNewInboundKey,
    ]);
    const {first,replay}=await completeCoupangProductReply(db);
    assert.equal(first.status,"completed");
    assert.equal(replay.replayed,true);
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.gateway_completion_receipts where job_id=$1`,
      [ids.coupangProductReplyJob])).rows[0].count,1);
    assert.deepEqual((await db.query(`select status,verification_status,reconciliation_reason
      from sellerpilot_private.support_reply_deliveries where gateway_job_id=$1`,
      [ids.coupangProductReplyJob])).rows[0],{
      status:"reconciliation_required",verification_status:"reconciliation_required",
      reconciliation_reason:"COUPANG_PRODUCT_READBACK_TICKET_STALE",
    });
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.coupang_product_reply_readback_links
      where source_job_id=$1`,[ids.coupangProductReplyJob])).rows[0].count,0);
    assert.equal((await db.query(`select count(*)::integer count
      from sellerpilot_private.channel_gateway_jobs where operation='inquiries.reply'`)).rows[0].count,1);
  }finally{await db.close();}
});

test("actual external worker retry entrypoint records the gateway-scoped SQL receipt",async()=>{
  const db=await createFixture();
  try{
    await insertCoupangProductReply(db);
    await completeCoupangProductReply(db);
    const child=await coupangProductReadbackChild(db);
    await claimChild(db,child.id,ids.gatewayWorker,ids.coupangProductReadClaim);
    const retry=coupangProductRetry(child);
    const response=await completeCsWorkerRetry({rpc:(name,args)=>dbRpc(db,name,args)},gatewayTokenHash,{
      jobId:child.id,claimToken:ids.coupangProductReadClaim,status:"failed",error:"pending",
      result:coupangProductRetryResult(child),retryContinuation:retry,
    });
    assert.equal(response.status,202);
    assert.deepEqual((await db.query(`select job.status,retry.worker_token_id
      from sellerpilot_private.channel_gateway_jobs job
      join sellerpilot_private.coupang_product_reply_readback_retries retry on retry.child_job_id=job.id
      where job.id=$1`,[child.id])).rows[0],{status:"queued",worker_token_id:ids.gatewayWorker});
  }finally{await db.close();}
});

test("actual serverless retry entrypoint records the serverless_cs-scoped SQL receipt",async()=>{
  const db=await createFixture();
  try{
    await insertCoupangProductReply(db);
    await completeCoupangProductReply(db);
    ...
    const response=await runOneServerlessCsGatewayJob({
      staticEgressChannels:["coupang"],rpc:coupangProductServerlessRpc(db,child),
      executeProvider:async()=>coupangProductRetryResult(child),logError:()=>{},
    },serverlessTokenHash);
    assert.equal(response.status,503);
    assert.equal((await response.json()).retryScheduled,true);
    assert.deepEqual((await db.query(`select job.status,retry.worker_token_id
      from sellerpilot_private.channel_gateway_jobs job
      join sellerpilot_private.coupang_product_reply_readback_retries retry on retry.child_job_id=job.id
      where job.id=$1`,[child.id])).rows[0],{status:"queued",worker_token_id:ids.serverlessWorker});
  }finally{await db.close();}
});
