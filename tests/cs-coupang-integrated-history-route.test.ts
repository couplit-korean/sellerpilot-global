...
const state = {
  ...
  channels: ["coupang"], expectedInitialJobs: 25, totalJobs: 25, queuedJobs: 0, runningJobs: 0,
  succeededJobs: 24, failedJobs: 1, progressPercent: 100,
...
test("actual sync GET keeps failed history on the same window and advances only all-success jobs", async () => {
  ...
  result = await route({ ...state, status: "succeeded", expectedInitialJobs: 40,
    totalJobs: 40, succeededJobs: 40, failedJobs: 0 }).handlers.GET(request());
  assert.equal(result.status, 200);
  assert.deepEqual((await result.json()).checkpoint, { canAdvance: true, replayEndDate: "2024-02-29", nextEndDate: "2024-01-30" });
});
...
test("actual sync POST verifies the requested channel, dates and initial scopes in the returned receipt", async () => {
  for (const data of [{ ...state, channels: ["elevenst"] }, { ...state, toDate: "2024-02-28" }, { ...state, expectedInitialJobs: 24 }, { ...state, expectedInitialJobs: 30 }]) {
    assert.equal((await route(data).handlers.POST(request("POST"))).status, 502);
  }
  const result = await route({ ...state, status: "queued", queuedJobs: 25, succeededJobs: 0, failedJobs: 0, completedAt: null }).handlers.POST(request("POST"));
