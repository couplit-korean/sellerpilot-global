...
test("product list, detail, and calculator label only ledger-backed values", async () => {
  ...
  assert.match(calculator, /inputs: \{[\s\S]{0,120}\.\.\.selectedResult\.engineInput,[\s\S]{0,120}productId: selectedProduct\.id/);
  assert.match(calculator, /currency: selectedResult\.currency,[\s\S]{0,120}rateToKrw: selectedResult\.rateToKrw,[\s\S]{0,120}rateBasis,/);
