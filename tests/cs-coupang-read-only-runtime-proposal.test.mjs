...
import { isLocalChannelExecutorTuple, localChannelExecutorAccess } from "../lib/channels/local-channel-executor";
...
test("Coupang live-read proposal widens only inquiries.list as read", async () => {
  ...
  for (const operation of ["inquiries.reply", "orders.list", "shipment.confirm"]) {
    assert.equal(isLocalChannelExecutorTuple("coupang", operation), false);
  }
});
