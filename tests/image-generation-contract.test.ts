...
test("protected products keep source-pixel compositing except dedicated prepared-food slots and preserve legacy input compatibility", async () => {
  ...
  assert.match(worker, /const generationOutputFile = backgroundOnly \? backgroundPlateFile : outputFile/);
  assert.match(worker, /buildAssetImagePrompt\(\s*result,\s*generationOutputFile,/);
  assert.match(worker, /normalizeGeneratedAsset\(generationOutputFile, generationPreset\)/);
  assert.match(worker, /assertIdentityBackgroundPlate\(generated, generationPreset, backgroundContactMode\)/);
  assert.match(worker, /if \(backgroundOnly\) \{[\s\S]*normalizeIdentityBackgroundPlate\(generated, generationPreset\)[\s\S]*writeFile\(backgroundPlateFile, generated, \{ mode: 0o600 \}\)[\s\S]*assertIdentityBackgroundPlate\(generated, generationPreset, backgroundContactMode\)/);
  ...
  assert.match(worker, /executeSourceProductCutout\("background", "", "", \[\{ file: backgroundPlateFile \}\]/);
  assert.match(worker, /auditGeneratedIdentityBackground\(\{\s*outputFile: backgroundPlateFile,/);
  assert.doesNotMatch(worker, /writeFile\(outputFile, generated/);
  assert.match(worker, /async function publishGeneratedAsset\(outputFile, normalized\)[\s\S]*writeFile\(candidateFile, normalized, \{ flag: "wx", mode: 0o600 \}\)[\s\S]*rename\(candidateFile, outputFile\)/);
  assert.match(worker, /normalized = await compositeIdentityForeground\([\s\S]*if \(!duplicate\) \{\s*if \(backgroundOnly\) await publishGeneratedAsset\(outputFile, normalized\)/);
