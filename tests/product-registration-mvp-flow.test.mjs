...
test("the primary final-authoring action requires a succeeded first draft and never falls back to manual MVP", async () => {
  ...
  assert.match(action, /if \(!firstDraftGenerated \|\| !isProductResearchJobId\(sourceResearchJobId\)\)/);
...
test("a succeeded research job becomes Studio lineage and changing only the source input invalidates it", async () => {
  ...
  assert.match(page, /setSourceResearchJobId\(jobId\);\s*setFirstDraftGenerated\(true\)/);
  assert.match(page, /if \(key === "researchInput"\) \{\s*setFirstDraftGenerated\(false\);\s*setSourceResearchJobId\(""\)/);
