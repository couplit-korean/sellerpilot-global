...
const normalState = `sellerpilot-lazada-my-${'n'.repeat(32)}`;
...
for (const scenario of ['prepare', 'no-worker', 'auth-mismatch', 'bad-origin', 'prepare-failure', 'ready']) {
  test(`actual Lazada exact UI start ${scenario}`, async () => {
    const sent = [], redirects = [], toasts = [], stored = [];
    const prepared = scenario === 'prepare' || scenario === 'prepare-failure' ? null : { credentialId: credential, sessionId: session, actorId: actor, expiresAt: Date.now() + 60_000 };
    const deps = {
      userId: actor,
      lazadaExactStarting: { current: false },
      createSupabaseClient: () => ({ auth: { getSession: async () => ({ data: { session: { user: { id: scenario === 'auth-mismatch' ? 'other' : actor }, access_token: 'fixture-token' } } }) } }),
      ...
      requestChannelConnection: async (url, init) => {
        const body = JSON.parse(init.body); sent.push({ url, body });
        if (scenario === 'prepare-failure') return { response: { ok: false }, payload: { status: 'blocked' } };
        return { response: { ok: true }, payload: scenario === 'prepare' ? { status: 'executor_required', sessionId: session } : scenario === 'no-worker' ? { status: 'executor_required' } : { status: 'ready', authorizationUrl: `${scenario === 'bad-origin' ? 'https://evil.invalid' : 'https://auth.lazada.com'}/oauth/authorize?state=${exactState}` } };
      },
      URL,
    };
...
const completeJs = ts.transpileModule(`${source.slice(completeStart, completeEnd)}\nreturn completeChannelOAuth;`, { compilerOptions: { target: ts.ScriptTarget.ES2022 } }).outputText;

for (const scenario of ['normal', 'stale-exact', 'exact']) {
  test(`actual Lazada callback ${scenario}`, async () => {
    const calls = [], removed = [], toasts = [];
    const state = scenario === 'exact' ? exactState : normalState;
    const exact = scenario === 'normal' ? null : { sessionId: session, credentialId: credential, actorId: actor, expiresAt: Date.now() + 60_000, state: exactState };
    const deps = {
      pendingChannelOAuth: { channel: 'lazada', state, code: 'fixture-code' }, userId: actor,
      createSupabaseClient: () => ({ auth: { getSession: async () => ({ data: { session: { user: { id: actor }, access_token: 'fixture-token' } } }) } }),
      readShopeeExactBrowserSession: () => null,
      readLazadaExactBrowserSession: () => exact,
      requestChannelConnection: async (url, init) => { calls.push({ url, body: JSON.parse(init.body) }); return { response: { ok: true }, payload: url.endsWith('/exact') ? { status: 'bound' } : { message: 'stored' } }; },
      setOAuthToastMessage: value => toasts.push(value),
      setPendingChannelOAuth() {},
      window: { sessionStorage: { removeItem: key => removed.push(key) } },
      shopeeExactBrowserKey: 'shopee', lazadaExactBrowserKey: 'lazada',
    };
    ...
    assert.equal(removed.includes('lazada'), scenario !== 'normal');
  });
}

test('credential center routes Lazada through normal authorize while preserving explicit exact support', () => {
  const a = center.indexOf('const startOAuth = async');
  const b = center.indexOf('\n\n  return (', a);
  const section = center.slice(a, b);
  assert.doesNotMatch(section, /credential\.channel === "lazada"[\s\S]{0,180}sellerpilot:lazada-exact-start/);
  assert.match(section, /channel-credentials\/\$\{credential\.channel\}\/authorize/);
  assert.ok(source.includes('sellerpilot:lazada-exact-start'));
  assert.ok(source.includes('exact?.state === pendingChannelOAuth.state'));
});
