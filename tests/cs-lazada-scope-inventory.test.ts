...
test("Lazada review and after-sales expose the local read-only implementation without entering the live denominator", () => {
  ...
  assert.equal(lazadaSupplementalCsSurfaces.every((surface) =>
    surface.implementation === "read_only_adapter_storage_ui"
    && surface.historyStorage === "implemented_local"
    && surface.automaticReadEnabled === false
    && surface.livePermissionObserved === false), true);
});
