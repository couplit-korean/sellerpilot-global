...
const routeUrl = new URL(
  "../app/api/admin/products/[id]/qoo10-s1-release/route.ts",
  import.meta.url,
);

test("exact Qoo10 S1 release route authenticates before parsing any target", async () => {
...
test("exact Qoo10 S1 release route preserves no-store on authentication errors", async () => {
  const route = await readFile(routeUrl, "utf8");
...
test("exact Qoo10 S1 release route exposes only the fenced v2 service RPCs", async () => {
  const route = await readFile(routeUrl, "utf8");

  assert.match(route, /sellerpilot_service_get_exact_qoo10_localization_release_status/u);
  assert.match(route, /sellerpilot_service_enqueue_exact_qoo10_localization_verifier/u);
  assert.match(route, /sellerpilot_service_enqueue_exact_qoo10_localization_activation/u);
