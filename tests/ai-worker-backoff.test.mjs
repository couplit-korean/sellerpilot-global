...
test("AI claim route compensates every post-claim preparation failure", async () => {
  ...
  assert.match(route, /safeReason: "cross_product_comparison_lookup_failed"/);
  assert.match(route, /safeReason: "invalid_cross_product_comparison_contract"/);
  assert.match(route, /safeReason: "cross_product_comparison_signing_failed"/);
  assert.match(route, /safeReason: "cross_product_comparison_signing_incomplete"/);
  assert.ok(
