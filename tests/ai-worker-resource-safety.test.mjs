...
test("AI worker dispatches support replies and preserves uploaded assets across ambiguous completion", async () => {
  const [source, claimRoute] = await Promise.all([
...
test("Codex subprocesses use a bounded FIFO gate and measure timeout only after dequeue", async () => {
  ...
  assert.match(source, /timeoutMs: studioMasterTimeoutMs/);
