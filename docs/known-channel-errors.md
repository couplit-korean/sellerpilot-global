...
| Shopee 숍 조회 대기·완료 HTTP400 | 현재 SG 작업 `39c870a8`은 v91 Mac 큐 queued/attempt0. 과거 IP403과 별개. 구형 완료 로그에는 작업ID/시각/필드가 없어 어느 문의의 오류인지 단정 불가 | 동일 SG 작업의 결과를 재사용하는164000 DB 적용 완료. API/UI 대기 처리와 민감값 없는 완료 오류 코드·필드·작업ID 로깅은 구현·집중 검사 완료, 배포 전 | 숍 조회 POST 중복, 로그에 없는 원인 추측, 문의 실패를 IP 오류로 재분류 |
| eBay Inventory 설명 거절·잘못된 원격ID 표시 | 실제 Inventory PUT400/25718, offer/publish 미전송. 설명 길이 수정은 a071 운영 포함 | 165000으로 정확한 거절 증거를 보존하며 SKU를 remote_id로 잘못 저장한 한 행만 정정 완료. 새 코드의 같은 오류 저장 방지도 구현됨, 배포 전. 정상 새 시도는 아직 안 함 | 이전 job 재전송, 실패를 게시 성공 처리, 정책·계정 재진단 |
| SmartStore 승인된 AI 등록 Mac 수령·공유 관리자 저장 | 승인 draft 검사 수정 운영 완료. 현재 `540d7294`는 provider 미전송 queued. 클라우드 차단은 기존 IP 결정; Mac에 일반 AI 승인 상품 수령 조건이 빠지고 source/stage/complete의 소유자 기준이 불일치 | 170000/171000 수정·집중검사 완료, 원자 적용 검토 중. 대기 중 source가04:53:56UTC 만료됐고 정규 재조회는 revision을 변경하므로 기존 요청을 임의 수정하지 않음 | 클라우드 재허용, TTL 연장, 외부 상세로 위장, 중복 신규등록 |
