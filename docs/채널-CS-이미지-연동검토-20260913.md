...
후속 구조 정리: 네 채팅 작업 준비에서 브라우저의 이미지 요청/merge 함수는 `app/_publishing/use-first-draft-images.ts`, 타일 표시와 역할 라벨은 `app/_publishing/first-draft-image-review.tsx`로 이동했다. 아래 위치는 검토 당시 `57bc85f` 기준이다. 동작 결함은 아직 남아 있으며 [네 작업 분담](parallel-tasks/README.md)에 따라 해결한다.
