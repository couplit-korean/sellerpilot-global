...
`docs/통합상품등록-구현원장-20260907.md`의 기능·검증·DB 적용을 먼저 읽는다. intake 초안/공통값 예외/typed category/Coupang 고시/publish CAS 저장/편집 context를 이미 구현했고 신규 DB023000·024000을 적용했다. 같은 테이블이나 입력 화면을 다시 만들지 않는다. 사용자 우선순위는 이 상품 등록 기능이며, 배포 화면의 실제 저장·복원을 마감한 후 아래 운영 복구로 이어간다. 편집용200은 strict 승인 복구나 상품 업로드 완료가 아니다.
