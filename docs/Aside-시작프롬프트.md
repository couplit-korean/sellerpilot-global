...
- `docs/통합상품입력-자동등록-설계-20260907.md`: 사용자 제안에 따른 AI 초안→공통/상품군/채널별 입력→사람 검토→자동 전송 구조. 입력 화면·저장·검증·payload를 같은 항목 정의로 연결한다. 기존 AUTO-01~08의 구현 기준이며 별도 중복 작업이 아니다.
- `docs/Codex-직접실행-20260907.md`: 실행 이력. 최신 절이 우선.
- `docs/Codex-잔여목록-20260907.md`: 현재 복구 작업과 완료 조건.

## 현재까지 반영한 입력 기능

`docs/통합상품등록-구현원장-20260907.md`의 기능·검증·DB 적용을 먼저 읽는다. intake 초안/공통값 예외/typed category/Coupang 고시/publish CAS 저장/편집 context를 이미 구현했고 신규 DB023000·024000 및 실제403을 고친31500을 적용했다. 03시 로그인 후 공통 저장/새로고침 복원·채널 예외 보존을 실제 UI에서 확인했다. 로그인 요청이나 이 DB 적용을 반복하지 않는다. 같은 테이블이나 입력 화면을 다시 만들지 않는다. 사용자 우선순위는 이 상품 등록 기능이며, 배포 화면의 실제 저장·복원을 마감한 후 아래 운영 복구로 이어간다. 편집용200은 strict 승인 복구나 상품 업로드 완료가 아니다.

## 이어서 할 작업

1. AUTO-01: 현재 상품의 승인 source 불일치를 정상 계약으로 복구한다. 상품 updated_at은 2026-09-06T13:08:23.846181Z, approved v2는 03:19:01.757195Z다. strict `publish-context`는409다. 새 `mode=draft` 편집 화면은 이 상태에서도 입력할 수 있으므로 작업대 전체를 막는 옛 UI를 다시 만들지 않는다. 222500은 미래 completion 예방만 적용했고 기존 불일치를 복구하지 않았다. 승인 manifest·실제 상품 사실·내용 해시를 검증하고 내용/운영 메타데이터를 분리한다. 전체행 commitment가 이미 불일치했으므로 timestamp를 그냥 되감거나 비교 필드를 임의 제외하지 않는다. 같은 이미지8장 재제작 금지. GET200 및 내용변경 차단/상태갱신 허용 회귀가 완료 조건이다.
...
3. AUTO-03: 쿠팡 공식 카테고리59631·상품사실·고시·315g×1개·출고/배송값을 SellerPilot 프로그램에 영속 저장한다. 입력 UI와 channelDrafts 서버 저장 기반은 구현됐다. 01:40 감사에서는 실제 Coupang assignment/판매값 저장이 없었으므로 현재 상품의 검토된 값이 실제 저장됐는지를 먼저 읽는다. WING에 입력된 것과 program DB 저장은 다르다. 기존 조사를 재사용해 정상 UI/API→job→provider→동일 SKU readback을 검증한다. WING 수동 등록은 실행하지 않는다.
...
- 입력 구현은 미완성 초안 저장부터 가능하게 하고, 지원하는 채널의 필수·선택·조건부 항목을 타입별 폼으로 다룬다. 사람이 보완한 값은 재분석/새로고침에도 보존한다. 기존 이미지·분류·원격 ID를 재사용하며 입력 폼 확장이 승인 충돌·worker·인증 문제의 해결을 대신하지 않는다. 상세 설계의 소유권/의존 순서를 따른다.
...
첫 응답에서는 최신 구현 원장과 배포 상태를 확인한 뒤 실제 남은 담당/수정만 짧게 말한다. 이미 끝낸 분석을 반복하거나 수동 등록으로 목표를 바꾸지 않는다.
