> **MVP 배포 인도:** 운영 코드 `e6813f1`의 Vercel·DB·Mac 일치, 등록 어댑터 8개, 기존 CREATE 경로 8개 버전/동일 판매자 결속을 확인했다. Lazada 정책 migration 적용 완료. 사용자 실제 검증 대기이며 Shopee 토큰·Temu 일반 채팅·Lazada 추가 국가 권한은 제한으로 남는다. [배포 근거](../../reviews/20260913-mvp-deployment-handoff.md). 주문·배송 추가 작업 없음.

> **현재 실행 범위 변경:** [상품 등록·CS MVP](../../MVP-완료기준.md)가 아래 과거 작업 목록보다 우선한다. 주문/배송·11번가 발송은 보류하고 기존 코드는 유지한다. 실제 채널 검증은 사용자 담당이다. 최신 배포 결과는 최상단 인도 기록을 따른다.

> 2026-09-13 20:17 KST 병렬 통합 검토: 기존 HEAD 403a01c의 동적 RPC 분류를 끝냈으며 실제 호출 이름 385개 중 운영 누락은 0이다. 과거 96/71/38 미확인 수치를 재작업 목록으로 사용하지 않는다. Lazada IM 5개국 전용 연결·국가별 저장·로컬 경로 승계 코드를 구현했고 DB 변경 2개를 운영 적용·해시 검증했다. 관련 회귀·최종 빌드는 통과했으며 새 앱 배포·실승인은 남아 있다. Shopee 저장 메인 토큰 복구 경로, 11번가 발주/발송, Temu 일반 구매자 상담의 실제 구현 공백을 확인했다. 전체 완료 시각은 아직 확정할 수 없다. [병렬 범위·작업 예산·현재 근거](../../reviews/20260913-parallel-critical-path.md).
