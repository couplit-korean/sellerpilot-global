> **MVP 등록 경로 수정 진행:** 8개 DB CREATE와 TS 수령 허용 목록의 불일치, Lazada 로컬 정책 CHECK 누락만 수정. 1차 7c3a4dc 배포 완료, 후속 코드 배포 및 동일 판매자 route metadata 갱신 진행 중. 사용자 실제 검증 대기. 배송·주문 작업 없음.

> **현재 실행 범위 변경:** [상품 등록·CS MVP](../../MVP-완료기준.md)가 아래 과거 작업 목록보다 우선한다. 주문/배송·11번가 발송은 보류하고 기존 코드는 유지한다. 실제 채널 검증은 사용자 담당이다. 이번에는 설계/지침만 변경했으며 새 배포를 수행하지 않았다.
