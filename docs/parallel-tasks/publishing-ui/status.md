> **2026-09-13 검색 기준 표시:** 유사상품 요청에 사용한 상품명·검색어·보조 검색어를 등록 화면에 추가했다. stale 상태는 다음 검색 예정어로 표시하여 입력 변경을 이미 실행한 검색으로 오인하지 않게 했다. 기존 1차 이미지 6개 역할·임시 이미지 판정을 유지했다. 관련 로직 48개·렌더링 4개 통과. 최종 통합 배포 진행 중.
