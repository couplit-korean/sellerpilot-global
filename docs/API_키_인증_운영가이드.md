...
비밀키 원문은 Git, 브라우저 저장소, 화면, 감사 로그에 남기지 않는다. Vercel에는 Supabase 서버 접근용 Secret Key만 서버 전용 환경변수로 보관하고, 판매 채널과 OpenAI 키는 웹 관리 화면에서 Vault로 저장한다.
...
| OpenAI | Project API Key, 선택 Project ID | 플랫폼 만료 없음, 내부 90일 교체 권장 | `GET /v1/models/gpt-5.6` 무과금 권한 검사 후 실제 생성 |
...
- OpenAI: Couplit 조직 종량제 플랜 확인. API 잔액이 음수여서 호출 중단 상태이며 Project API Key는 아직 생성·저장하지 않음. 조직 월 한도는 현재 `$100`이므로 결제 전 사용자가 원하는 더 낮은 운영 한도를 확정해야 함.
- Supabase Vault: OpenAI 키 타입, 만료·교체·경고·감사 수명주기와 서버 전용 복호화 경로를 운영 DB에 적용. 익명·일반 사용자는 복호화 함수를 실행할 수 없음.
...
- [x] OpenAI Project API Key Vault 등록·교체·만료·권한검사 UI 구현
...
- [ ] OpenAI 운영비 한도·API 크레딧 확정 후 Project Key 생성, Vault 등록, 텍스트·이미지 실호출 통과
