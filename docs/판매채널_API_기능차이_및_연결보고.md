...
현재 실계정 판정은 **Qoo10 생성·재조회 PoC 1/7, Shopee 과거 8개 숍 UNLIST E2E 이력 보유(현재 재인증 필요), 나머지 5개 미통과**다. 과거 성공 이력과 지금 호출 가능한 상태를 구분해서 표시하며, 다음 외부 조건은 코드만으로 완료할 수 없다.
...
`API 키 · 인증` 화면의 **API 실행 검수**는 Shopee를 포함한 6개 채널의 공식 핵심 경로를 실제로 호출할 수 있는 관리자 도구다. 상품 등록 센터는 공통 상품 초안에서 채널별 필수 카테고리·속성·이미지·가격·재고를 최종 점검하고, 준비된 채널을 최대 8개까지 병렬 실행하는 화면과 작업 원장을 연결했다. 다만 아래 기능은 채널별 운영 자격증명과 실계정 검수가 남아 있다.
...
따라서 현재 판정은 **핵심 API 실행 계층 6/7 코드 준비, Qoo10 쓰기 PoC 현재 통과, Shopee 과거 8개 숍 이력/현재 OAuth 차단, Lazada OAuth 차단**이다. 11번가는 로그인 전용 판매자 명세를 확보한 뒤 별도 계약 테스트를 추가해야 한다.
