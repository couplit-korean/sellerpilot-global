...
기준일: 2026-08-16
대상: Qoo10 Japan, Lazada, 쿠팡, 11번가, 네이버 스마트스토어, eBay
제외: Shopee
비활성: Alibaba.com, 1688

## 결론

6개 채널을 하나의 공통 기능표와 자격증명 Vault로 관리하도록 확장했다. Qoo10 구 OpenApiService 호출은 현재 QAPI로 교체했고, 쿠팡 HMAC, 네이버 Commerce OAuth 서명, eBay User OAuth와 자동 갱신을 구현했다. Lazada 기존 OAuth·갱신 흐름은 유지하면서 공통 기능표에 편입했다.
...
다만 **실계정 API E2E 통과는 현재 0/6**이다. 키가 없는 상태에서 성공으로 표시하지 않았으며, 다음 외부 조건은 코드만으로 완료할 수 없다.
...
| 보안 저장 | 6개 채널 Vault 저장, 버전·지문·만료·경고·교체·감사 기록 |
...
| 운영 화면 | 6개 키 카드, 키 수명·교체일·경고일, 연결 검사, OAuth 재연결, 보호된 API 실행 검수 콘솔 |
...
`API 키 · 인증` 화면의 **API 실행 검수**는 5개 채널의 공식 핵심 경로를 실제로 호출할 수 있는 관리자 도구다. 그러나 상품 등록 센터의 공통 상품 초안을 각 채널의 완전한 등록 payload로 자동 변환해 일괄 게시하는 업무 UI는 아직 이 실행 API와 연결되지 않았다. 다음 기능도 기능표와 설계만 있고 실행 어댑터·운영 검수가 남아 있다.

- 채널별 이미지 업로드·이전, 옵션/세트상품 상세 변환과 출고지·반품지 기준정보 동기화
- 취소·반품·교환, 고객 문의·답변, 정산 조회
- Lazada Push와 eBay Notification 수신·서명검증, 폴링 체크포인트 스케줄러
- 429/5xx 지수 백오프, 실패 대기열, 부분 성공 보상과 운영자 재처리
- 공통 상품·채널 ID 매핑, 중앙 재고 예약/차감/복구 트랜잭션

따라서 현재 판정은 **핵심 API 실행 계층 5/6 코드 준비, 실계정 E2E 0/6**이다. 11번가는 로그인 전용 판매자 명세를 확보한 뒤 별도 계약 테스트를 추가해야 한다.
