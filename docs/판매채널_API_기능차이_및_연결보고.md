...
기준일: 2026-08-20
활성 대상: Qoo10 Japan, Shopee, Lazada, 쿠팡, 네이버 스마트스토어, eBay, Temu
추가 검증 대기: 11번가
비활성: Alibaba.com, 1688

## 결론

7개 활성 채널을 하나의 공통 기능표와 자격증명 Vault로 관리하도록 확장했다. Qoo10 QAPI, Shopee Main account 8개 숍 OAuth, Lazada OAuth, 쿠팡 HMAC, 네이버 Commerce OAuth 서명, eBay User OAuth와 자동 갱신, Temu Partner API 서명·고정 송신 IP 보호를 구현했다.

또한 관리자 전용 `POST /api/admin/channel-operations` 실행 계층을 추가했다. 7개 활성 채널의 카테고리, 상품 생성/수정/중지, 가격, 재고, 주문, 발주확인, 송장 경로를 서버 허용목록으로 고정했다. 외부 쓰기는 명시 확인과 멱등키가 없으면 거부하며, 같은 요청의 중복 실행은 DB 작업 원장에서 차단한다. 호출 인수는 [판매채널 실행 API 계약](./판매채널_실행_API_계약.md)에 정리했다.

현재 실계정 판정은 **7개 활성 채널 중 판매 전 과정을 현재 시점에 E2E 통과한 채널은 0개**다. Qoo10은 상품 생성·재조회 PoC가 통과했고 Shopee는 과거 8개 숍 UNLIST E2E 이력이 있지만, 둘 다 전체 주문·배송·CS까지 통과한 상태는 아니다. 과거 성공 이력과 지금 호출 가능한 상태를 구분해서 표시하며, 다음 외부 조건은 코드만으로 완료할 수 없다.

- Qoo10: 운영 QAPI 키·실상품 읽기·전체 3,264개 카테고리 조회·테스트상품 생성/재조회 통과, 수정·판매중지와 주문 폴링 검수 필요
- Shopee: Merchant·8개 숍 UNLIST 발행/재조회 이력은 있으나 2026-08-17 연결 검사에서 토큰 갱신 HTTP 422, OAuth 재승인 필요
- Lazada: 운영 앱 정보는 있으나 현재 연결 검사에서 필수 OAuth 값 누락/만료, 판매자 재승인 필요
...
- Temu: Partner App Key·Secret, 판매자 Access Token과 허용된 고정 송신 IP
- 11번가: 활성 채널로 추가하기 전에 180일 유효 Open API Key와 로그인 전용 판매자 상세 XML 명세를 확보하고 읽기·쓰기 계약 테스트를 별도로 통과해야 함
...
| 11번가 | 공식 기능 범위만 문서화. 운영키·판매자 상세 명세를 확보하기 전에는 활성 채널·연결 완료·쓰기 가능으로 표시하지 않음 |
...
- 11번가는 상품·재고 기능을 제공하지만 서비스 코드와 XML 필드는 로그인 문서 버전에 고정해야 한다. 확인 전에는 활성 채널에 넣거나 쓰기 버튼을 노출하지 않는다.
...
`연결 상태 · 채널 연결` 화면의 **API 실행 검수**는 7개 활성 채널의 공식 핵심 경로를 호출하는 관리자 도구다. 상품 등록 센터는 현재 상품 1건의 공통 초안에서 채널별 필수 카테고리·속성·이미지·가격·재고를 최종 점검하고 선택 채널별 결과를 작업 원장에 기록한다. 다만 아래 기능은 채널별 운영 자격증명과 실계정 검수가 남아 있다.
...
따라서 현재 판정은 **7개 활성 채널의 실행 계층 구현, Qoo10 쓰기 PoC 통과, 전체 판매흐름 API E2E 0/7**이다. Shopee·Lazada는 OAuth 재승인, 쿠팡·네이버·eBay·Temu는 운영키 또는 판매자 승인 보완이 필요하다. 11번가는 로그인 전용 판매자 명세와 운영키를 확보한 뒤 별도 계약 테스트를 추가해야 한다.
