...
- 담당 작업: 01a075d8-40c6-7102-a7e3-526de202bcb4
- 작업 폴더: /Users/kimchangheemac/dev/sellerpilot-product-coupang-20260909
- 브랜치: codex/product-coupang-20260909
- 기준 SHA: e8d821587a29933aebf31771672e56f515995245
- dev 포트: 3215; NEXT_DIST_DIR은 자기 작업 폴더 내 경로만 사용한다.
...
먼저 새 폴더의 AGENTS.md, docs/channel-connections/coupang.md, docs/channel-connections/README.md, docs/channel-connections/product-recovery-retirement-20260909.md와 관련 Next.js 로컬 가이드를 읽는다. 계정/키/상품번호를 코드 상수로 박지 않는다.

## 채널 목표

이 작업은 이제 쿠팡 상품등록 전담이다. 이전의 상품등록 0번 중앙 역할·다른 7채널 지시·자동화 관리는 현재 중앙 작업으로 이관됐다. Wing 판매자와 vendorId/API 권한·등록 IP를 확인한다. 사용자가 최근 API 키를 갱신했다고 했으므로 현재 활성 credential 버전과 실제 키 적용 여부를 확인하고 필요한 교체는 비밀을 드러내지 말고 0번에 요청한다. 카테고리·옵션별 필수속성/고시·인증·정확한 용량/포장·출고/반품센터·배송비·재고·가격을 일반 SKU로 등록하고 sellerProductId/옵션 ID/상태 GET을 검증한다. 기존 실제 상품번호를 하드코딩해 우회하지 않는다.

## 수정 소유권

직접 수정 가능: lib/product-registration/channels/coupang.ts, coupang 전용 lib/channels/coupang-*.ts 중 상품등록 파일, 새 lib/product-registration/coupang/ 모듈, 자기 상품등록 전용 테스트, docs/product-channel-parallel/reports/coupang/ 문서와 제안. CS/OAuth 공용/배송 파일은 이 패턴에 맞더라도 자동 소유가 아니다.
