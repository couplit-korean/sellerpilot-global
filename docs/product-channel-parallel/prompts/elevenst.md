...
- 담당 작업: 01a0816b-ef3d-7810-8ba2-1b8b9e6a2b07
- 작업 폴더: /Users/kimchangheemac/dev/sellerpilot-product-elevenst-20260909
- 브랜치: codex/product-elevenst-20260909
- 기준 SHA: e8d821587a29933aebf31771672e56f515995245
- dev 포트: 3216; NEXT_DIST_DIR은 자기 작업 폴더 내 경로만 사용한다.
...
먼저 새 폴더의 AGENTS.md, docs/channel-connections/elevenst.md, docs/channel-connections/README.md, docs/channel-connections/product-recovery-retirement-20260909.md와 관련 Next.js 로컬 가이드를 읽는다. 계정/키/상품번호를 코드 상수로 박지 않는다.

## 채널 목표

올바른 판매자 ID는 couplit이다. 셀러오피스 비밀번호와 OPEN API Key를 혼동하지 않는다. 키·서비스권한·호출 IP·정식 XML 응답을 확인한다. 카테고리/고시·sellerPrdCd·prdNo·무료/유료 배송필드·반품/교환비·출고/반품 주소를 연결한다. 현재 유료배송 검증 구현을 보존하고, 일반 CREATE→상품번호 확보→GET 품목/가격/재고/배송조건 확인을 완결한다. HTML/빈404를 상품 부재로 보지 않는다. 재고 동기화 전체 구현은 별도 범위로 보고 신규등록 완료와 혼합하지 않는다.

## 수정 소유권

직접 수정 가능: lib/product-registration/channels/elevenst.ts, elevenst 전용 lib/channels/elevenst-*.ts 중 상품등록 파일, 새 lib/product-registration/elevenst/ 모듈, 자기 상품등록 전용 테스트, docs/product-channel-parallel/reports/elevenst/ 문서와 제안. CS/OAuth 공용/배송 파일은 이 패턴에 맞더라도 자동 소유가 아니다.
