...
- 담당 작업: 01a0816d-7401-7a63-b9ee-023a3ca03bf7
- 작업 폴더: /Users/kimchangheemac/dev/sellerpilot-product-temu-20260909
- 브랜치: codex/product-temu-20260909
- 기준 SHA: e8d821587a29933aebf31771672e56f515995245
- dev 포트: 3213; NEXT_DIST_DIR은 자기 작업 폴더 내 경로만 사용한다.
...
먼저 새 폴더의 AGENTS.md, docs/channel-connections/temu.md, docs/channel-connections/README.md, docs/channel-connections/product-recovery-retirement-20260909.md와 관련 Next.js 로컬 가이드를 읽는다. 계정/키/상품번호를 코드 상수로 박지 않는다.

## 채널 목표

KR 판매자 앱의 현재 심사·활성상태·토큰·goods 권한을 공식 화면에서 읽는다. 이 중앙 작업에서는 사용자가 승인한 compliance 설문을 제출해 Reviewing을 관측한 이력이 있으므로 과거 Rejected 문구만 보고 재제출하지 않는다. 실제 승인은 별도 조회해야 한다. 실제 카테고리·템플릿·SKU 옵션·창고/배송·속성·가격단위·이미지 제약에 맞춰 일반 CREATE를 완결한다. 심사 대기여도 로컬 검증/필수값 계약을 개발하고 대기 원인과 해결 증거를 기록한다. 삭제한 특정 기존 상품 UPDATE/채택을 복원하지 않는다.

## 수정 소유권

직접 수정 가능: lib/product-registration/channels/temu.ts, temu 전용 lib/channels/temu-*.ts 중 상품등록 파일, 새 lib/product-registration/temu/ 모듈, 자기 상품등록 전용 테스트, docs/product-channel-parallel/reports/temu/ 문서와 제안. CS/OAuth 공용/배송 파일은 이 패턴에 맞더라도 자동 소유가 아니다.
