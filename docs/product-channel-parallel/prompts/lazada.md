...
- 담당 작업: 01a0816d-1e18-7a92-94da-a0fa0d6dda46
- 작업 폴더: /Users/kimchangheemac/dev/sellerpilot-product-lazada-20260909
- 브랜치: codex/product-lazada-20260909
- 기준 SHA: e8d821587a29933aebf31771672e56f515995245
- dev 포트: 3212; NEXT_DIST_DIR은 자기 작업 폴더 내 경로만 사용한다.
...
먼저 새 폴더의 AGENTS.md, docs/channel-connections/lazada.md, docs/channel-connections/README.md, docs/channel-connections/product-recovery-retirement-20260909.md와 관련 Next.js 로컬 가이드를 읽는다. 계정/키/상품번호를 코드 상수로 박지 않는다.

## 채널 목표

실제 OAuth app/seller/country/shop 상태를 읽고 MY를 첫 대상으로 연결한다. 최신 token/redirect URI/서비스권한을 확인하되 계정연결이 끝난 상태를 과거 실패 기록만 보고 초기화하지 않는다. leaf category·SKU 속성·브랜드·이미지·KRW 원가와 MYR 확정 가격·재고·배송 필수조건을 연결한다. SellerSku 부재검사→CreateProduct→item/SKU GET 재조회와 내부 완료를 완결한다.

## 수정 소유권

직접 수정 가능: lib/product-registration/channels/lazada.ts, lazada 전용 lib/channels/lazada-*.ts 중 상품등록 파일, 새 lib/product-registration/lazada/ 모듈, 자기 상품등록 전용 테스트, docs/product-channel-parallel/reports/lazada/ 문서와 제안. CS/OAuth 공용/배송 파일은 이 패턴에 맞더라도 자동 소유가 아니다.
