...
- 담당 작업: 01a0816b-5b09-7893-9dca-d0870785eac1
- 작업 폴더: /Users/kimchangheemac/dev/sellerpilot-product-qoo10-20260909
- 브랜치: codex/product-qoo10-20260909
- 기준 SHA: e8d821587a29933aebf31771672e56f515995245
- dev 포트: 3217; NEXT_DIST_DIR은 자기 작업 폴더 내 경로만 사용한다.
...
먼저 새 폴더의 AGENTS.md, docs/channel-connections/qoo10.md, docs/channel-connections/README.md, docs/channel-connections/product-recovery-retirement-20260909.md와 관련 Next.js 로컬 가이드를 읽는다. 계정/키/상품번호를 코드 상수로 박지 않는다.

## 채널 목표

Qoo10 Japan 판매자/키·ShippingNo·카테고리·SellerCode와 JPY 가격/재고·이미지·일본어 필수값을 확인한다. 일반 GetItemDetailInfo SellerCode 중복 검사→상품 생성→ItemCode 기반 재조회→판매 상태와 내부 완료 경로를 완결한다. 202/처리중은 completion으로 보고하지 말고 동일 요청 lineage로 조회한다. 기존 고정 상품의 S1/배송 정규화 복구 경로를 다시 구현하지 않는다. TracX 발송 연동은 이번 상품등록 소유에서 제외한다.

## 수정 소유권

직접 수정 가능: lib/product-registration/channels/qoo10.ts, qoo10 전용 lib/channels/qoo10-*.ts 중 상품등록 파일, 새 lib/product-registration/qoo10/ 모듈, 자기 상품등록 전용 테스트, docs/product-channel-parallel/reports/qoo10/ 문서와 제안. CS/OAuth 공용/배송 파일은 이 패턴에 맞더라도 자동 소유가 아니다.
