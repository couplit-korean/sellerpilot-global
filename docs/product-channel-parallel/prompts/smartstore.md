...
- 담당 작업: 01a0816b-af8b-7893-a1b9-5f84564a3e90
- 작업 폴더: /Users/kimchangheemac/dev/sellerpilot-product-smartstore-20260909
- 브랜치: codex/product-smartstore-20260909
- 기준 SHA: e8d821587a29933aebf31771672e56f515995245
- dev 포트: 3214; NEXT_DIST_DIR은 자기 작업 폴더 내 경로만 사용한다.
...
먼저 새 폴더의 AGENTS.md, docs/channel-connections/smartstore.md, docs/channel-connections/README.md, docs/channel-connections/product-recovery-retirement-20260909.md와 관련 Next.js 로컬 가이드를 읽는다. 계정/키/상품번호를 코드 상수로 박지 않는다.

## 채널 목표

판매자 계정·커머스 API 앱·서명/token 권한을 확인한다. 일반 상품의 원상품/채널상품 ID를 구분하고 카테고리·고시·인증·브랜드·용량·배송/반품/출고지·옵션·재고·가격 입력을 검증한다. SellerPilot 신규 요청→중복 검색→등록→원상품 및 채널상품 재조회→내부 완료를 완결한다. 검색 오류를 부재로 처리하거나 기존 SKU를 자동 PUT하지 않는다. 기존 상품번호 확인은 신규 등록 성공에 포함하지 않는다.

## 수정 소유권

직접 수정 가능: lib/product-registration/channels/smartstore.ts, smartstore 전용 lib/channels/smartstore-*.ts 중 상품등록 파일, 새 lib/product-registration/smartstore/ 모듈, 자기 상품등록 전용 테스트, docs/product-channel-parallel/reports/smartstore/ 문서와 제안. CS/OAuth 공용/배송 파일은 이 패턴에 맞더라도 자동 소유가 아니다.
