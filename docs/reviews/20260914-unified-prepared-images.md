...
The user then requested all six new Downloads photos. First-stage submission now includes every selected role/additional photo and binds pending retries to a digest of the full ordered selection. Corrected residual six-image labels. Server-research tests: 44 passed; UI submission/lifecycle tests: 16 passed; TypeScript passed. Deployed `e6df20c7c29ac083928396263201838fe5df8d81` (`dpl_FBczG8Bpb8dvAU5AFiMvV8Lv4KZk`); production canaries passed on the new release, six schedules activated, listing routes and local runtime aligned. Actual generated result verification has not passed.

Live evidence: Aside input count was 6; DB request `8d0930cf-6070-48b0-868d-a3c5176a1447` contains all six original filenames with roles main/back/left/right/label/barcode and their original byte lengths. It failed with `source_product_identity_mismatch` at 18:28:24 UTC, despite visual inspection showing the same Narangd Zero 500ml bottle. The mismatching photo/reason is not exposed in the stored error and remains unresolved. Retry job `d584602e-35ba-4e6c-bde6-3187e875b06a` returned to queued at 18:35:32 UTC; no generated-image request or verified output exists. Separate same-model diagnostic via the project's OIDC returned HTTP 429 free-tier rate limiting. This is evidence of a provider limitation, not proof of the exact reason for every queued production attempt. Input photos remain in Aside. No marketplace publication was performed. Temporary pulled environment and project bypass files were removed.
