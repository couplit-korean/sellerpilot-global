...
## 2026-09-14 11:53 KST — eBay 저장 토큰 실조회 및 11번가 라벨 확인

- eBay v210의 미확정 자동 갱신 기록 c9d6431c는 보존했다. 승인된 기존 refresh 권한으로 토큰을 한 번 발급하고 GetUser로 동일 판매자를 확인한 뒤, 저장 RPC의 신규 스키마 캐시 오류는 저장된 동일 응답의 Management 경로 재개로 처리했다. 공급자 갱신을 재전송하지 않았다.
- 활성 credential v211 `0aca8183-7440-4076-ba0f-679c20259a81` 저장, 기존 미시작 조회 307건의 credential만 재결속, 강제 공급자 작업 시작 0건을 확인했다. 이후 저장된 토큰으로 기존 fulfillment/payment/return 정책과 inventory location 조회가 모두 HTTP 200 및 기대 ID 일치로 4/4 통과했다. 이는 상품 게시 성공을 의미하지 않는다.
- 11번가 기존 셀러오피스의 couplit 로그인과 공식 주소 API를 확인했다. 출고지 2번·반품지 3번이며 주소/연락처 원문은 공개 기록에 남기지 않았다. 필수 주소 입력란 누락 수정은 ee9e309 후보에 포함됐다.
- Aside 파일 선택창에서 Downloads의 2026-09-14 나랑드 원본 001~006 정확한 6장을 선택했다. 실제 화면은 ‘라벨 사진 6장 확인 · 파일은 이 기기에서만 읽습니다.’로 완료됐다. 파일 입력의 접근성 이름에는 이전 확인 중 문구가 남았지만 실제 해시 처리는 성공했으므로 관련 코드를 다시 수정하지 않았다.
- ee9e309 후보 빌드는 READY, 무실행 후보 canary 6/6 통과했다. 운영 승격 전 Qoo10 완료 후 이력 충돌을 일시 장애로 재시도하는 결함을 추가 확인하여 좁은 후속 수정과 통합한다. 현재 활성 웹/Mac은 192a212다.
- 가공 이미지 8장·최종 자산 16개·승인 상세 v1은 보존했다. 카테고리 6/8, 신규 상품 게시 0/8. 판매 배송/반품 비용은 사용자 답변 대기이며, Shopee SG 미확정 갱신과 Lazada 정확한 브랜드는 별도 미해결이다.


## 2026-09-14 12:01 KST — Qoo10 완료 후 불필요 재시도 수정

- Qoo10 8월17일 S1 이력 창은 이전 job 55aba216로 이미 완성됐다. 새 조회 0519541d의 기본 결과는 succeeded였지만, 기존 다른 job의 이력 증거와 충돌하는 RPC가 SQLSTATE40001을 발생시켜 HTTP503 및 최대30분 동일결과 저장 재시도로 이어졌다. 공급자를 30분 재호출한 것은 아니다.
- migration155000은 정확한 두 이력 증거 충돌만 PT409로 변경한다. 완료 API도 해당 오류만409로 반환해 기존 worker의 정상 종료 경로를 사용한다. 일반 일시 장애는 재시도를 보존한다. 기존 이력 증거와 현재 job의 성공 결과를 변경하지 않는다.
