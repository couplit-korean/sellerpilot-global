> **범위 변경:** 이 보고서는 상품 등록·CS MVP로 좁히기 이전의 감사·복구 기록이다. 11번가 배송과 주문/배송 전체는 현재 작업에서 제외한다. 당시 관측은 보존하되 작업 우선순위와 시간 예측은 현재 계획으로 사용하지 않는다. [최종 MVP 설계](../MVP-완료기준.md)를 우선한다.
