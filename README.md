...
Qoo10 Japan, Shopee, Lazada, 쿠팡, 네이버 스마트스토어, eBay, Temu의 상품 등록, 판매, 주문, 재고와 CS를 한곳에서 관리하는 AI 커머스 운영 서비스입니다. 11번가는 공식 Open API의 상품·주문·클레임 범위까지 확인했지만 판매자 전용 운영키와 상세 XML 명세를 아직 실계정으로 검증하지 못해 활성 채널 수에 포함하지 않습니다. Alibaba와 1688은 비활성 준비 채널입니다.
...
- 원가·물류비·수수료·환율·광고비를 반영해 7개 활성 채널의 순이익, 손익분기점과 목표 마진 판매가를 비교하고 운영 DB에 저장하는 마진 계산
- 통합 주문·출고·배송 화면
- 다국어 CS 통합함과 AI 답변 초안
- Qoo10, Shopee, Lazada, 쿠팡, 네이버 스마트스토어, eBay, Temu 채널별 운영 페이지
- Supabase Vault 기반 7개 활성 판매채널의 키 버전, 만료일, 교체주기, 사전경고, 롤백 유예, 연결검사, 감사기록 관리
...
- 7개 활성 채널의 공식 카테고리·상품·가격·재고·주문·배송 실행 계층, 관리자 검수 UI와 멱등키 기반 중복 방지 원장. 운영키나 승인 범위가 없는 채널은 실행 전에 차단하고 실제 연결로 표시하지 않음
