...
export async function GET(request:Request){
 ...
 const filters=archiveFiltersSchema.safeParse({query:params.get("query")??"",channel:params.get("channel"),status:params.get("status"),from:params.get("from"),to:params.get("to"),
  accountId:params.get("accountId"),shopId:params.get("shopId"),ticketKind:params.get("ticketKind"),source:params.get("source")});
 ...
 const {query,channel,status,from,to,accountId,shopId,ticketKind,source}=filters.data;
 const {data,error}=await admin.userClient.rpc("sellerpilot_search_cs_archive_v2",{p_query:query,p_channel:channel,p_status:status,p_from_date:from,p_to_date:to,
  p_account_id:accountId,p_shop_id:shopId,p_ticket_kind:ticketKind,p_source:source,p_limit:25,
