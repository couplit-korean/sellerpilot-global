...
export async function POST(request: NextRequest) {
  ...
  if (!textValue(nextSecret, "access_token")) return NextResponse.json({ message: "Lazada Access Token이 없습니다. OAuth를 다시 진행해 주세요." }, { status: 400 });
  ...
  return NextResponse.json({ message: "Lazada OAuth 연결과 Vault 저장이 완료됐습니다." }, { headers: { "cache-control": "no-store, max-age=0" } });
}
