...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!productId.success || !exactTarget(productId.data, listingId)) {
    return NextResponse.json({
      message: "Qoo10 exact 상품·게시 원장 식별값이 일치하지 않습니다.",
    }, noStore(400));
  }
  ...
  if (error || !data) {
    return NextResponse.json({
      message: "Qoo10 S1 검증·활성화 상태를 읽지 못했습니다.",
      mode: "qoo10_exact_release_status_unavailable",
    }, noStore(503));
  }
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!productId.success || !body.success
    ...
    return NextResponse.json({
      message: "Qoo10 exact S1 요청의 상품·게시 원장·작업 식별값을 확인해 주세요.",
    }, noStore(400));
  }
  const runtimeRelease = resolveRuntimeReleaseIdentity();
  if (runtimeRelease.status !== "valid") {
  ...
  if (error || !data) {
    return NextResponse.json({
      message: body.data.action === "verify"
        ? "현재 S1 원격 상태와 새 현지화 update 작업의 결속을 확인하지 못해 verifier를 만들지 않았습니다."
        : "2분 이내의 exact S1 관측과 one-use permit을 확인하지 못해 활성화를 만들지 않았습니다.",
      mode: body.data.action === "verify"
        ? "qoo10_exact_s1_verifier_precondition_failed"
        : "qoo10_exact_s1_activation_precondition_failed",
    }, noStore(409));
  }
