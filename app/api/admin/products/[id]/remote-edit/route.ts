...
import { bindQoo10RollbackUpdateRecoveryArguments, centralProductEditFieldSupport, channelProductEditFieldSupport, listingUpdateServerCandidate, listingUpdateMutablePaths, listingUpdateRemoteIdentity, legacyEbayListingUpdateCandidate, prepareListingUpdateArguments, productEditRemotePlan, qoo10RollbackListingUpdateCandidate, remoteProductEditIdempotencyKey, type ListingUpdateReference, type Qoo10RollbackUpdateRecoveryBinding } from "../../../../../../lib/channels/listing-update";
...
const remoteEditSchema = z.object({
  ...
  arguments: z.record(z.string(), z.unknown())
    .refine((value) => JSON.stringify(value).length <= 128000, "payload too large"),
});
const qoo10RollbackIdentitySchema = z.object({
  ...
  expectedState: z.object({
    ...
    retailPriceJpy: z.number().int().min(1).max(999999999),
    sellPriceJpy: z.number().int().min(1).max(999999999),
    quantity: z.number().int().min(1).max(99999999),
    shippingNo: z.string().regex(/^\d{1,20}$/u),
    biContentsNo: z.number().int().min(100000).max(Number.MAX_SAFE_INTEGER),
  }).strict().refine((value) => value.retailPriceJpy >= value.sellPriceJpy, "Qoo10 retail price must not be below its sell price"),
}).strict();
...
function listingAvailability(listing: ListingRecord) {
  ...
  const executionBlock = listingExecutionBlock(listing, false);
  ...
  return {
    ...
    mode: release.available
      ? executionBlock?.mode ?? release.mode
      : release.mode,
    reason: release.available
      ? executionBlock?.message ?? release.reason
      : release.reason,
...
async function productContext(request: Request, productId: string) {
  const admin = await authenticateAdminRequest(request, { timeoutMs: 10000 });
...
export async function GET(request: Request, context: {
  ...
  return NextResponse.json({
    ...
    listings: listings.map(listingAvailability),
...
export async function POST(request: Request, context: {
  ...
  if (qoo10RollbackListingUpdateCandidate(listing.channel, reference)) {
    const { data: identityData, error: identityError } = await loaded.admin.serviceClient.rpc("sellerpilot_service_get_qoo10_rollback_update_identity", {
  ...
  if (legacyEbayListingUpdateCandidate(listing.channel, reference)) {
    const { data: identityData, error: identityError } = await loaded.admin.serviceClient.rpc("sellerpilot_service_get_ebay_listing_update_identity", {
      p_listing_id: listing.id,
      p_credential_id: body.data.credentialId,
      p_product_id: productId.data,
      p_market: typeof listing.market === "string" ? listing.market : "",
  ...
  try {
    ...
    if (boundQoo10RollbackUpdateRecovery) {
      argumentsValue = bindQoo10RollbackUpdateRecoveryArguments(argumentsValue, boundQoo10RollbackUpdateRecovery);
    }
  }
  catch {
    return NextResponse.json({
  ...
  if (listing.channel === "lazada") {
    ...
    if (!lazadaPricePolicy
      ...
      || Math.abs(sourcePrice - lazadaPricePolicy.sourcePriceKrw) > 0.000001
