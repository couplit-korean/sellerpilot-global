...
export async function POST(request: Request) {
  ...
  if (!supabaseUrl || !secretKey) {
    console.error("AI worker heartbeat server configuration is unavailable", {
  ...
  const { data, error } = await serviceClient.rpc("sellerpilot_touch_ai_job", {
    ...
    p_worker_version: parsed.data.version ?? "sellerpilot-cli-worker/1.18",
  });
  if (error) {
    const status = workerRpcErrorStatus(error);
    console.error("AI worker heartbeat RPC failed", { code: error.code ?? "unknown", status });
  ...
  if (data !== "running") {
    return NextResponse.json({ message: "AI 작업 실행 권한 또는 lease가 만료됐습니다." }, { status: 409 });
  }
