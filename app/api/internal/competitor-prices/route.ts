...
async function runCompetitorPrices(serviceClient: NonNullable<ReturnType<typeof serverClient>>) {
  ...
  const due = dueData.flatMap((item) => {
    if (!item || typeof item !== "object" || typeof item.product_id !== "string" || typeof item.query !== "string") {
      invalidDueRows += 1;
      return [];
    }
  ...
  for (const product of due) {
    ...
    try {
      searched = await searchCompetitorProviders(registry, product.query, product.aliases, 30);
    } catch {
    ...
    try {
      ...
      results.push({ productId: product.product_id, ok: true, count: savedCount, providers: searched.providers });
    } catch {
      infrastructureFailures += 1;
      results.push({ productId: product.product_id, ok: false, count: 0, providers: searched.providers });
    }
  ...
  return NextResponse.json({
    ok: infrastructureFailures === 0 && results.every((item) => item.ok),
    checked: results.length,
    infrastructureFailures,
    results,
  }, {
    status: infrastructureFailures > 0 ? 503 : results.some((item) => !item.ok) ? 207 : 200,
