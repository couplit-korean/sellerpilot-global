...
export function CompetitorPriceSlots({
  ...
  const gridItems = compact ? [...exactItems, ...probableItems] : exactItems;
  ...
  const priceFollow = productId ? competitorTotalKrw === null
    ...
    : null;
  const groups: Array<{ marketplace: CompetitorMarketplaceId; items: CompetitorV3DisplayItem[] }> = marketplaceOrder.map((marketplace) => ({ marketplace, items: gridItems.filter((item) => (item.marketplace || "other") === marketplace).slice(0, 3) }));
  const marketplaceOrderSet = new Set<string>(marketplaceOrder);
  const otherItems = gridItems.filter((item) => !marketplaceOrderSet.has(item.marketplace || "other")).slice(0, 3);
  ...
  return <div className={`competitor-market-groups ${compact ? "compact" : ""}`}>
    ...
    {gridItems.length === 0 && state !== "loading" && state !== "pending" && <p className="competitor-exact-empty"><Search size={15} /><span><b>{compact ? "유사 상품 없음" : "확정 동일상품 없음"}</b><small>{compact ? "상품명이 비슷하거나 같은 마켓 상품을 찾지 못했습니다." : "probable 후보가 있으면 아래 사람 검토 목록에서만 확인합니다."}</small></span></p>}
    {state === "loading" && <div className="competitor-loading"><LoaderCircle className="spin" size={17} />{compact ? "비슷한 상품을 채널별로 찾고 있습니다." : "동일 상품 가격을 채널별로 찾고 있습니다."}</div>}
    ...
    {marketplaceGroups.map((group) => {
      ...
      return <section key={group.marketplace}><header><b>{marketplaceLabels[group.marketplace] ?? group.marketplace}</b><small>{group.items.length > 0 ? `${compact ? "유사" : "exact"} ${group.items.length}개` : groupSummary}</small></header><div className="competitor-price-grid">{Array.from({ length: 3 }, (_, index) => {
