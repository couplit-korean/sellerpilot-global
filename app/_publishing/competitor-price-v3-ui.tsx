...
export function CompetitorPriceSlots({
  ...
  return <div className={`competitor-market-groups ${compact ? "compact" : ""}`}>
    ...
    {providers.length > 0 && <div className="competitor-provider-summary" aria-label="가격 검색 공급자 상태">{providers.map((provider) => <span className={provider.status} key={provider.provider}><b>{providerLabels[provider.provider]}</b><em>{provider.status === "searched" ? `조회 완료 · 후보 ${provider.count}건` : provider.status === "pending" ? "조회 진행 중" : provider.status === "failed" ? "응답 실패" : "미연결"}</em><small>{lastCheckedAt ? `마지막 조회 ${formatTimestamp(lastCheckedAt)}` : "마지막 조회 미확인"}</small></span>)}</div>}
