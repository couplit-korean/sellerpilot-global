...
function PublishingPage({ notify, channelMetrics, pipeline, authenticatedFetch, initialProduct, onStartAnother, onShowHistory, onManualProductCreated }: { notify: (message: string) => void; channelMetrics: OperationsSnapshot["channelMetrics"]; pipeline: OperationsSnapshot["pipeline"] | null; authenticatedFetch: (input: string, init?: RequestInit) => Promise<Response>; initialProduct?: { id: string; name: string } | null; onStartAnother: () => void; onShowHistory: () => void; onManualProductCreated: () => void }) {
  ...
  const setIntakeField = <Key extends keyof ProductIntakeDraft>(key: Key, value: ProductIntakeDraft[Key]) => {
    ...
    if (shouldInvalidateCompetitorResearch(String(key), currentIntake[key], value)) {
      ...
      if (key === "researchInput") setFirstDraftImages([]);
      setResearchCompetitors([]);
      setCompetitorProviders([]);
      setCompetitorFetchedAt(null);
        ...
        <article className="panel upload-panel">
          ...
          <section className={`product-research-panel ${manualErrors.researchInput ? "field-error" : ""}`}>
            ...
            {firstDraftImages.length > 0 && <section className="first-draft-image-review" aria-label="1차 생성 이미지 6개">
              <header><span><ImageIcon size={16} /><b>1차 생성 이미지</b><small>상세페이지 제작 전에 사람이 확인하는 서버 결과입니다.</small></span><em>{firstDraftImages.length} / 6장</em></header>
...
function CsPage({ authenticatedFetch, notify, displayTickets, displayOrders, onSend, onDeliveryStatus, onDraft, onStatus, onSync, onBackfill, syncing, syncStatus, historyBackfill, initialQuery = "", initialTicketId = null, initialChannel = "all", initialStatus = "open", onFilterChange }: {
  ...
  onSync: () => Promise<void>;
  onBackfill: (channel: "coupang" | "smartstore") => Promise<void>;
  authenticatedFetch: (input: string, init?: RequestInit) => Promise<Response>;
  syncing: boolean;
  syncStatus: OperationsSnapshot["syncStatus"];
...
function DashboardShell({ onLogout, onIdleLogout, userEmail, userId, freshLogin, oauthToastMessage, onOAuthToastQueued }: { onLogout: () => Promise<void>; onIdleLogout: () => Promise<void>; userEmail: string; userId: string; freshLogin: boolean; oauthToastMessage: string; onOAuthToastQueued: () => void }) {
  ...
  const syncOrders = useCallback(async (silent = false, historyDays?: number, historyChannel?: "coupang" | "smartstore") => {
    ...
    try {
      const response = await authenticatedOperationsFetch("/api/operations/sync", {
        ...
        body: JSON.stringify(historyDays
          ? { channels: historyChannel ? [historyChannel] : ["coupang", "smartstore"], historyDays }
