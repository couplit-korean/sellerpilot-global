...
export const channelReadiness: ChannelReadiness[] = [
  ...
  },
  {
    ...
    appState: "Couplit 앱 Online · Merchant·8개 숍 OAuth·Vault 연결 · 글로벌→SG 실등록 검증",
    overall: "partial",
    summary: "운영 Merchant와 8개 숍을 OAuth로 재승인하고 숍별 Access/Refresh Token을 Vault에 저장했습니다. 실제 글로벌 카테고리·필수속성·이미지 5장을 검증한 뒤 SG 로컬 상품을 UNLIST 상태로 생성하고 다시 읽어 등록 결과를 확인했습니다.",
    checks: [
      ...
      { label: "OAuth 서버 토큰", state: "verified", evidence: "Merchant·8개 숍 토큰 교환·Vault 저장·get_shop_info 호출 가능" },
    ...
    blockers: ["만료 전 Partner Key 교체 일정 등록", "운영 Push 콜백·이벤트 구독", "실판매 전 국가별 세금·관세·마진 승인"],
    nextAction: "8개 숍 UNLIST 상품 운영자 검수 → 국가별 세금·관세·마진 승인 → Push 구독 → 선택 숍부터 실판매 전환",
  },
  {
    key: "lazada",
    code: "L",
    ...
    appState: "Couplit Commerce Online · 권한·콜백·MY 실셀러 확인 · 공급사 화이트리스트 인덱스 차단",
    overall: "partial",
    summary: "Couplit Commerce 앱의 운영 상태, 상품·카탈로그·크로스보더 권한, 운영 콜백과 MY 실셀러 코드를 확인했습니다. 허용목록을 성공적으로 다시 추가했지만 OAuth가 동일 셀러 코드를 invalid로 반환하여, 현재 차단점은 Vercel IP가 아닌 Lazada 공급사 허용목록 인덱싱·계정 분류입니다.",
    checks: [
      ...
      { label: "토큰 교환", state: "blocked", evidence: "OAuth 수임 코드 발급 전 셀러 화이트리스트 검증에서 차단" },
    ...
    blockers: ["Lazada 공급사의 MY 셀러 화이트리스트 인덱싱·크로스보더 계정 분류 수정", "새 판매자 OAuth 승인과 토큰 교환", "ID 실셀러 스토어 확보", "Push Mechanism 이벤트 구독"],
    nextAction: "Lazada 지원창구에 앱·MY 셀러 코드·Authorize 시각·오류 문구로 인덱싱 수정 요청 → OAuth·Vault 저장 → 6개 국가 카테고리·필수속성·Inactive 상품 순차 검수",
  },
  {
    key: "coupang",
    code: "C",
