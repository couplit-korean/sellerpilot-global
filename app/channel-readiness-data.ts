...
export const channelReadiness: ChannelReadiness[] = [
  ...
  },
  {
    ...
    appState: "판매자 계정·Seller API 재검증 필요 · 실상품 등록 미검증",
    overall: "blocked",
    consoleVerified: false,
    apiReadPassed: false,
    summary: "공개 ProductSearch 응답만으로 판매자 계정 연결이나 상품 등록 권한을 확인할 수 없습니다. 현재 CHANGHEE 프로필의 Seller Office 로그인이 확인되지 않았고 11번가 실상품 생성·재조회 성공 이력도 없으므로, 계정과 OPEN API 서비스의 일치 여부를 다시 검증하기 전까지 연결 완료로 판정하지 않습니다.",
    checks: [
      { label: "판매자센터 접근", state: "blocked", evidence: "CHANGHEE 프로필에서 Seller Office 인증 세션을 확인하지 못함" },
      { label: "OPEN API 계정", state: "blocked", evidence: "운영 판매자 계정과 OPEN API 서비스의 일치 여부 재검증 필요" },
      ...
      { label: "서비스 등록", state: "blocked", evidence: "현재 판매자 계정 기준 서비스 등록·고정 IP 상태 재확인 필요" },
      { label: "운영 API Key", state: "partial", evidence: "Vault 참조는 있으나 현재 판매자 계정의 키인지 검증되지 않음" },
      { label: "SellerPilot 읽기 진단", state: "partial", evidence: "공개 ProductSearch 응답은 판매자 상품 등록 권한의 증거가 아님" },
      { label: "실상품 등록·재조회", state: "blocked", evidence: "11번가 상품 생성 성공 및 원격 상품번호 재조회 이력이 없음" },
      { label: "주문 동기화", state: "blocked", evidence: "Seller Office 실주문과 통합 원장 대조가 필요" },
      { label: "문의 동기화", state: "blocked", evidence: "공식 개발 가이드/API 검색에 판매자 문의 조회 API가 제공되지 않음" },
    ],
    blockers: ["CHANGHEE 프로필의 Seller Office 로그인 필요", "판매자 계정과 OPEN API 서비스 일치 여부 확인", "승인된 테스트상품 생성·재조회 검증"],
    nextAction: "Seller Office 로그인 확인 → OPEN API 서비스·키 매핑 대조 → 승인된 테스트상품 1건 생성·재조회",
    officialDocs: [
