...
export const channelReadiness: ChannelReadiness[] = [
  {
    ...
    market: "일본",
    console: "QSM 판매자 센터",
    appState: "QAPI 운영 키 Vault 연결 · 실상품 읽기·전체 카테고리 조회 성공",
    overall: "partial",
    summary: "QSM 실계정의 유효한 QAPI 키를 Couplit Supabase Vault에 저장하고 실상품 읽기와 전체 카테고리 API를 직접 검증했습니다. 키 원문은 화면·소스·로그에 남기지 않습니다.",
    checks: [
    ...
    blockers: ["쓰기 테스트상품 범위 승인", "주문 폴링 체크포인트 실검증"],
    nextAction: "카테고리·필수속성 검증 → 승인된 테스트상품 1건 등록·조회·중지 → 주문 폴링 체크포인트 검수",
  },
  {
    key: "shopee",
    code: "S",
    ...
    appState: "Couplit 앱 Online · 8개 숍 365일 재승인 완료 · IP 정책 결정 대기",
    overall: "partial",
    summary: "운영 앱의 Redirect Domain과 Live Partner ID·Key를 확인하고 메인 계정의 8개 숍을 365일로 재승인했습니다. 토큰 교환은 앱 IP 화이트리스트가 현재 실행 IP를 차단해 대기 중입니다.",
    checks: [
      ...
      { label: "OAuth 서버 토큰", state: "blocked", evidence: "source_ip_undeclared · 기존 White IP와 현재 요청 IP 불일치" },
    ...
    blockers: ["운영 Vercel용 Shopee IP 허용 정책 결정", "8개 숍 Access/Refresh Token 교환", "만료 전 Partner Key 교체 일정 등록"],
    nextAction: "IP 정책 반영 → 8개 숍 토큰 교환·Vault 저장 → get_shop_info 읽기 → 카테고리·상품·주문 제한 검수",
  },
  {
    key: "lazada",
    code: "L",
    ...
    appState: "Couplit Commerce Online · 개발자 로그인 복구 · IP 정책 결정 대기",
    overall: "partial",
    summary: "Lazada 개발자 로그인을 복구해 Couplit Commerce 앱, 운영 콜백, 활성 권한과 판매자 허용목록을 확인했습니다. 토큰 교환은 앱 IP 화이트리스트가 현재 실행 IP를 차단해 대기 중입니다.",
    checks: [
      ...
      { label: "판매자 허용 범위", state: "verified", evidence: "MY·PH·SG·TH·VN 판매자 5개 허용목록 확인" },
      ...
      { label: "토큰 교환", state: "blocked", evidence: "AppWhiteIpLimit · 기존 White IP와 현재 요청 IP 불일치" },
      { label: "IP 허용목록", state: "blocked", evidence: "운영 Vercel은 고정 출발 IP가 보장되지 않아 정책 결정 필요" },
    ...
    blockers: ["운영 Vercel용 Lazada IP 허용 정책 결정", "새 판매자 OAuth 승인과 토큰 교환", "Push Mechanism 이벤트 구독"],
    nextAction: "IP 정책 반영 → 새 OAuth 승인·Vault 저장 → /seller/get·category/suggestion 실검수 → Push 구독",
  },
  {
    key: "coupang",
    code: "C",
