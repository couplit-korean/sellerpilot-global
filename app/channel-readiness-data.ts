...
export const channelReadiness: ChannelReadiness[] = [
  ...
  },
  {
    ...
    appState: "운영 API Key Vault 연결 · 고정 IP 주문 주기수집 연결",
    overall: "partial",
    consoleVerified: true,
    apiReadPassed: true,
    summary: "11번가 판매자센터와 OPEN API 센터 로그인, 서비스 등록, 개발자 이메일·고정 실행 IP 저장과 운영 API Key의 Vault 연결을 완료했습니다. 등록 IP에서 ProductSearch 진단과 결제완료 주문의 5분 주기수집을 실행합니다. 판매자 문의 조회 API는 공식 개발 가이드와 API 검색에서 제공되지 않아 미지원으로 명확히 표시합니다.",
    checks: [
      ...
      { label: "주문 동기화", state: "verified", evidence: "결제완료 주문을 7일 단위 두 구간으로 조회해 통합 원장에 적재" },
      { label: "문의 동기화", state: "blocked", evidence: "공식 개발 가이드/API 검색에 판매자 문의 조회 API가 제공되지 않음" },
    ],
    blockers: ["11번가가 판매자 문의 조회 API를 제공하면 CS 통합함 어댑터 추가", "실주문 발생 시 판매자센터와 원장 건수 대조"],
    nextAction: "5분 주문 주기수집 유지 → 실주문 발생 시 원장 누락 대조 → 문의 API 제공 여부 정기 확인",
    officialDocs: [
