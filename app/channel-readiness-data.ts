...
export const channelReadiness: ChannelReadiness[] = [
  {
    ...
    market: "일본",
    console: "QSM 판매자 센터",
    appState: "QAPI 운영 키 유효 · 전체 카테고리 실조회 성공 · Vault 저장 대기",
    overall: "partial",
    summary: "QSM 실계정의 유효한 QAPI 키를 확인하고 전체 카테고리 API를 직접 호출해 3,264건을 정상 수신했습니다. 키 원문은 화면·소스·로그에 남기지 않았습니다.",
    checks: [
      ...
      { label: "Vault 자격증명", state: "blocked", evidence: "SellerPilot 서버의 안전한 비밀 참조에 아직 저장하지 않음" },
    ...
    blockers: ["기존 QAPI 키의 Couplit Vault 저장 승인", "검사 상품번호 지정", "쓰기 테스트상품 범위 승인"],
    nextAction: "Vault 키 연결 → 기존 실상품 1건 읽기 → 카테고리·필수속성 검증 → 승인된 테스트상품 1건 등록·조회·중지",
  },
  {
    key: "shopee",
    code: "S",
    ...
    appState: "Couplit 앱 Online · 운영 키 확인 · 판매자 연결 존재 · 숍 토큰 저장 대기",
    overall: "partial",
    summary: "운영 앱과 Redirect Domain, Live Partner ID·Key, 판매자 연결 상태를 실제 콘솔에서 확인했습니다. 현재 남은 것은 숍별 Access/Refresh Token을 Couplit Vault에 저장하고 읽기 API를 검증하는 단계입니다.",
    checks: [
      ...
      { label: "운영 판매자 연결", state: "verified", evidence: "운영 앱에 Unlink 가능한 기존 판매자 권한 연결 존재" },
    ...
    blockers: ["Live Partner Key의 Couplit Vault 저장 승인", "연결된 Main account의 shop_id·Access/Refresh Token 확보", "만료 전 Partner Key 교체 일정 등록"],
    nextAction: "Vault 키 입력 → SellerPilot 승인 링크 재실행 → 숍 토큰 교환 → get_shop_info 읽기 → 카테고리·상품·주문 제한 검수",
  },
  {
    key: "lazada",
    code: "L",
    ...
    appState: "판매자 승인 화면 확인 · 기존 코드는 만료 추정 · 개발자 콘솔 재로그인 필요",
    overall: "partial",
    summary: "Couplit Commerce 판매자 승인 화면과 운영 콜백 수신 이력은 확인했습니다. 기존 일회성 코드는 재사용하지 않고 개발자 콘솔 로그인 뒤 새 승인·토큰 교환을 진행해야 합니다.",
    checks: [
      ...
      { label: "OAuth 판매자 승인", state: "partial", evidence: "과거 운영 콜백 코드 수신 이력은 있으나 새 코드 발급 필요" },
      { label: "토큰 교환", state: "blocked", evidence: "새 Authorization Code와 App Secret이 Couplit Vault에 없음" },
      { label: "IP 허용목록", state: "blocked", evidence: "이전 토큰 교환에서 AppWhiteIpLimit 응답 · 고정 출구 IP 재확인 필요" },
    ...
    blockers: ["Lazada ISV 개발자 콘솔 재로그인", "고정 송신 IP와 White IP 일치 확인", "새 판매자 OAuth 승인과 토큰 교환"],
    nextAction: "개발자 콘솔 로그인 → White IP 확인 → 새 OAuth 승인·토큰 교환 → /seller/get·category/suggestion 실검수",
  },
  {
    key: "coupang",
    code: "C",
