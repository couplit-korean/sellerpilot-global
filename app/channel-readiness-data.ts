...
export const channelReadiness: ChannelReadiness[] = [
  ...
  },
  {
    ...
    consoleVerified: true,
    apiReadPassed: true,
    summary: "2026-09-07 상품 Q&A·고객문의 단독30일 수집 두 작업이 HTTP 200으로 완료됐고 조회 결과는 모두 0건입니다. 판매자센터 고객문의 전체 상태·유형에서도 3개월 범위 0건을 확인했습니다. 실제 문의·답변 왕복과 전체 과거 답변 복구 완료는 별도로 검증하며 최신 건수는 LIVE 원장으로 판정합니다.",
    checks: [
    ...
    blockers: ["실제 고객 문의·답변 왕복 및 재문의 검증", "공급자 제공 범위 내 과거 답변·첨부 이력 대조"],
  ...
  },
  {
    ...
    checks: [
      ...
      { label: "일반 메시지 권한", state: "blocked", evidence: "2026-09-07 04:29 KST Commerce Message API 최소 읽기 HTTP 403 · 현재 commerce.message scope 미기록 · 전체 대화 미수집" },
    ],
    blockers: ["Commerce Message API 일반 대화 권한 HTTP 403 해소", "일반 메시지 전체 페이지·첨부 수집과 원장 연결", "실제 ASQ·일반 대화 수신과 사용자 승인 답변 E2E"],
    nextAction: "상시 작업자 연결 상태 유지 → 일반 메시지 commerce.message 권한 확인·승인 → 전체 대화 수집 → 사용자 승인 답변 E2E",
