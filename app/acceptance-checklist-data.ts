...
const rawSections: Array<Omit<AcceptanceSection, "items"> & { items: RawItem[] }> = [
  ...
  { code: "G", title: "Qoo10·Lazada 상품등록 연결", intent: "공식 API로 등록·수정·가격·재고를 처리하고 부분 실패를 복구합니다.", pptSlides: "5, 9, 10, 24", items: [
    ["채널 계정 안전 연결"], ["토큰 자동 갱신"], ["카테고리 자동 동기화"], ["필수속성 사전검사"], ["채널 이미지 업로드"], ["신규 상품 자동 등록"], ["기존 상품 자동 수정"], ["판매중지·품절 처리"], ["가격 자동 변경"], ["재고 자동 변경"], ["옵션·세트상품 등록"], ["호출제한·재시도·중복방지"], ["부분 성공 처리"], ["Qoo10 단계형 QAPI 처리"], ["Shopee 글로벌상품·국가게시 (사용자 요청 범위 제외)", "후속"], ["Lazada 이미지·창고·웹훅 처리"],
  ] },
  ...
  { code: "P", title: "테스트·검수·인수인계", intent: "실계정·실상품·장애 상황까지 증거로 검수합니다.", pptSlides: "2, 25, 28, 30, 31", items: [
    ["정식 Excel과 항목 연결"], ["기존 코드·인증정보 선점검"], ["Qoo10·Lazada API PoC 선검증"], ["실제 E2E 상품흐름 검수"], ["오류·중복·호출제한 검수"], ["10,000건 동시주문 시뮬레이션"], ["24시간 채널장애 복구시험"], ["이미지·OCR 품질검수"], ["상품매칭 정답 500건 검수"], ["가격·마진 자동시험"], ["30~100개 SKU 제한운영"], ["문서·소스·계정·교육 인수"],
  ] },
...
const excludedIds = new Set(["G-15"]);
