export type DevelopmentStatus = "done" | "partial" | "not_started";
export type VerificationStatus = "passed" | "pending" | "external";
...
const rawSections: Array<Omit<AcceptanceSection, "items"> & { items: RawItem[] }> = [
  { code: "A", title: "착수 전 범위와 준비", intent: "채널·국가·카테고리·계정·데이터·계약 범위를 확정합니다.", pptSlides: "1, 2, 5, 29, 31", items: [
    ["1차 판매채널을 3개로 고정", "결정"], ["1차 판매국가 확정", "결정"], ["무인 운영의 뜻 확정", "결정"], ["자동 등록 허용 카테고리 확정", "결정"], ["규제상품 자동 제외 범위 확정", "결정"], ["실계정과 테스트상품 준비"], ["개발자 앱 권한 확보"], ["카카오 알림톡 준비", "조건"], ["공급사 상품자료 준비"], ["소유권과 유지보수 범위 확정", "결정"],
  ] },
  ...
  { code: "G", title: "3개 채널 상품등록 연결", intent: "공식 API로 등록·수정·가격·재고를 처리하고 부분 실패를 복구합니다.", pptSlides: "5, 9, 10, 24", items: [
    ["채널 계정 안전 연결"], ["토큰 자동 갱신"], ["카테고리 자동 동기화"], ["필수속성 사전검사"], ["채널 이미지 업로드"], ["신규 상품 자동 등록"], ["기존 상품 자동 수정"], ["판매중지·품절 처리"], ["가격 자동 변경"], ["재고 자동 변경"], ["옵션·세트상품 등록"], ["호출제한·재시도·중복방지"], ["부분 성공 처리"], ["Qoo10 단계형 QAPI 처리"], ["Shopee 글로벌상품·국가게시"], ["Lazada 이미지·창고·웹훅 처리"],
  ] },
  ...
  { code: "I", title: "주문과 공통 재고", intent: "중앙 재고원장을 기준으로 모든 채널을 수렴시킵니다.", pptSlides: "18, 22", items: [
    ["통합 주문함"], ["Shopee·Lazada 주문 웹훅"], ["Qoo10 주문 주기조회"], ["웹훅 누락 보정조회"], ["주문형식 통일"], ["중복 주문 제거"], ["주문상태 이력"], ["주문상품과 내부 SKU 연결"], ["중앙 재고원장"], ["주문 즉시 재고 예약"], ["한 번에 차감하고 전파예약"], ["전 채널 재고 갱신"], ["안전재고·임계치·자동품절"], ["세트 구성품 연동차감"], ["재고 대조와 동시주문 검증"],
  ] },
  ...
  { code: "P", title: "테스트·검수·인수인계", intent: "실계정·실상품·장애 상황까지 증거로 검수합니다.", pptSlides: "2, 25, 28, 30, 31", items: [
    ["정식 Excel과 항목 연결"], ["기존 코드·인증정보 선점검"], ["3채널 API PoC 선검증"], ["실제 E2E 상품흐름 검수"], ["오류·중복·호출제한 검수"], ["10,000건 동시주문 시뮬레이션"], ["24시간 채널장애 복구시험"], ["이미지·OCR 품질검수"], ["상품매칭 정답 500건 검수"], ["가격·마진 자동시험"], ["30~100개 SKU 제한운영"], ["문서·소스·계정·교육 인수"],
  ] },
...
export const acceptanceSections: AcceptanceSection[] = rawSections.map((section) => ({
  ...
  items: section.items.map(([title, priority = "필수"], index) => {
    ...
    return {
      ...
      title,
      priority,
      development: partialDevelopmentIds.has(id) ? "partial" : "not_started",
      verification: externalVerificationIds.has(id) ? "external" : "pending",
    };
  }),
}));
