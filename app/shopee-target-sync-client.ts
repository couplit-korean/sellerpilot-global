...
export async function syncExistingShopeeTarget({ accessToken, selection, checkOnly = false, signal, fetcher = fetch }: {
  ...
  const request = async (method: "GET" | "POST", credentialId?: string) => {
    const response = await fetcher(url, {
      ...
      ...(method === "POST" ? { body: JSON.stringify({ channel: "shopee", credentialId, ...target }) } : {}),
    });
  ...
  if (before.response.status !== 409 || before.body?.channel !== "shopee"
      || typeof before.body.code !== "string" || !before.body.code.startsWith("SHOPEE_TARGET_CACHE_")
  ...
  let postMessage = "";
  try {
    const posted = await request("POST", credentialId);
