...
const syncStateLabel: Record<EbayCaseDisputeHistorySyncResponse["state"], string> = {
  complete: "수집 완료",
  unavailable: "수집 불가",
  interrupted: "수집 중단 · 이어서 가능",
  page_limit_reached: "페이지 한도 도달 · 이어서 가능",
};
...
export function EbayCaseDisputeHistory({ authenticatedFetch }: { authenticatedFetch: EbayCaseDisputeAuthenticatedFetch }) {
  ...
  const [resume, setResume] = useState<HistoryResume | null>(null);
  ...
  async function syncHistory() {
    ...
    setSyncReceipt(null);
    try {
      const matchingResume = resume?.credentialId === credentialId && resume.resourceKind === resourceKind
        ? resume
        : null;
      const now = Date.now();
      const startTime = matchingResume?.startTime
        ?? (resourceKind === "resolution_case" ? new Date(now - 30 * 86_400_000).toISOString() : undefined);
      const endTime = matchingResume?.endTime
        ?? (resourceKind === "resolution_case" ? new Date(now).toISOString() : undefined);
      const receipt = await syncEbayCaseDisputeHistoryUiResponse({
        authenticatedFetch,
        credentialId,
        resourceKind,
        ...(startTime && endTime ? { startTime, endTime } : {}),
        ...(matchingResume ? { resumeOffset: matchingResume.resumeOffset } : {}),
        signal: abort.signal,
      });
      if (abort.signal.aborted || run !== generation.current) return;
      setSyncReceipt(receipt);
      setResume(receipt.resumeOffset === null ? null : {
        credentialId,
        resourceKind,
        ...(startTime && endTime ? { startTime, endTime } : {}),
        resumeOffset: receipt.resumeOffset,
      });
      if (receipt.observedCount !== null && receipt.completedPages > 0) {
  ...
  return <details className={`panel ${styles.viewer}`}>
    ...
    <div className={styles.filters}>
      ...
      {accounts ? <>
        ...
        <button type="button" className="filter-button" disabled={loading || !credentialId} onClick={() => void syncHistory()}>{resume ? `이어서 저장 (${resume.resumeOffset})` : "최신 내역 가져와 저장"}</button>
      </> : null}
    </div>
    {accounts?.length === 0 ? <p>운영 공간에 연결된 활성 eBay 키가 없습니다.</p> : null}
    {loading ? <p role="status">eBay 케이스·분쟁 이력을 조회하고 있습니다…</p> : null}
    {error ? <p role="alert" className={styles.error}>{error}</p> : null}
    {syncReceipt ? <p role="status">
      {syncStateLabel[syncReceipt.state]} · 시도 {syncReceipt.pages}페이지·완료 {syncReceipt.completedPages}페이지
      {syncReceipt.resumeOffset === null ? "" : ` · 재개 위치 ${syncReceipt.resumeOffset}`}
      {syncReceipt.total === null ? " · 전체 건수 미확인" : ` · 전체 ${syncReceipt.total}개`}
      {syncReceipt.observedCount === null ? " · 저장 안 함" : ` · 확정 관측 ${syncReceipt.observedCount}개·신규 이력 ${syncReceipt.insertedCount}개`}
      {syncReceipt.partialPageMayExist ? " · 중단된 현재 페이지는 일부 저장됐을 수 있어 같은 위치부터 안전하게 다시 확인합니다." : ""}
    </p> : null}
    {page ? <>
