...
import {
  ...
  elevenstReadAccountsSchema,
  type ElevenstReadAccount,
...
import styles from "./read-state.module.css";
...
export async function fetchElevenstReadState(
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(credentialId)) {
    throw new Error("ELEVENST_READ_SELECTION_INVALID");
  }
  const params = new URLSearchParams({ view: "state", credentialId });
  const response = await authenticatedFetch("/api/admin/cs/channels/elevenst/read-state?" + params, {
    cache: "no-store",
    signal,
  });
  if (!response.ok) throw new Error(`ELEVENST_READ_STATE_HTTP_${response.status}`);
  const state = elevenstReadStateSchema.parse(await response.json());
  if (state.credentialId !== credentialId) throw new Error("ELEVENST_READ_ACCOUNT_MISMATCH");
  return state;
}

export async function fetchElevenstReadAccounts(
  authenticatedFetch: AuthenticatedFetch,
  signal?: AbortSignal,
) {
  const response = await authenticatedFetch(
    "/api/admin/cs/channels/elevenst/read-state?view=accounts",
    { cache: "no-store", signal },
  );
  if (!response.ok) throw new Error("ELEVENST_READ_ACCOUNTS_HTTP_" + response.status);
  return elevenstReadAccountsSchema.parse(await response.json()).accounts;
}
...
export function ElevenstReadStatePanel({ authenticatedFetch }: { authenticatedFetch: AuthenticatedFetch }) {
  const [accounts, setAccounts] = useState<ElevenstReadAccount[]>([]);
  ...
  const loadAccounts = async () => {
    ...
    setError("");
    try {
      const next = await fetchElevenstReadAccounts(authenticatedFetch, abort.signal);
      if (!abort.signal.aborted) {
        setAccounts(next);
        setCredentialId((current) => next.some((account) => account.credentialId === current)
          ? current
          : next[0]?.credentialId ?? "");
      }
    } catch {
      if (!abort.signal.aborted) {
        setAccounts([]);
        setCredentialId("");
        setError("활성 11번가 계정 목록을 조회하지 못했습니다.");
      }
    } finally {
      if (!abort.signal.aborted) setAccountsLoading(false);
    }
  };
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const next = await fetchElevenstReadState(authenticatedFetch, credentialId, abort.signal);
  ...
  return <details className={`panel ${styles.panel}`} onToggle={(event) => {
    if (event.currentTarget.open) {
      if (!accounts.length && !accountsLoading) void loadAccounts();
      return;
    }
    controller.current?.abort();
    setState(null);
    ...
    <label className={styles.accountSelector}>
      <span>연결 계정</span>
      <select value={credentialId} disabled={accountsLoading || !accounts.length} onChange={(event) => {
        controller.current?.abort();
        setLoading(false);
        setError("");
        ...
        {accounts.length ? accounts.map((account) => <option
          key={account.credentialId}
          value={account.credentialId}
        >{account.label}</option>) : <option value="">활성 계정 없음</option>}
      </select>
    </label>
    <button type="button" className="filter-button" disabled={loading || accountsLoading || !credentialId} onClick={() => void load()}>
